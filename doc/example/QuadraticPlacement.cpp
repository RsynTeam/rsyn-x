...
bool QuadraticPlacementExample::run(const Rsyn::Json &params) {
	...
	const std::string model = params.count("model")? params["model"] : "hybrid";
	...
	if (model == "b2b") {
		...
		std::cout << "Warning: Invalid quadratic placement model '" << model << "'. "
				"Doing nothing...\n";
	} // end else
...
void QuadraticPlacementExample::runQuadraticPlacementUsingHybridNetModel() {
	...
	buildLinearSystemUsingHybridNetModel(movable, A, bx, by, mapCellToIndex, numStarNodes);
	watchBuilder.finish();
...
void QuadraticPlacementExample::runQuadraticPlacementUsinbBound2BoundNetModel() {
	std::cout << "*** Quadratic Placement (Bound2Bound Net Model) ***\n";
	
	// Note: This value defines the minimum allowed distance between two nodes.
	//
	// A reasonable value is very important for convergence. The main cause for
	// weak convergence is when one mix large and small connectivity weights.
	// If epsilon is two small, a very large weight may be generated, which will
	...
	for (Rsyn::Instance instance : module.allInstances()) {
		Rsyn::Cell cell = instance.asCell(); // TODO: hack, assuming that the instance is a cell
		if (!instance.isFixed() && !instance.isLCB() && !instance.isSequential()) {
			movable.insert(cell);
		} // end if
	} // end for
	...
	// Construct linear systems.
	Stepwatch watchBuilder("Building linear system");
	buildLinearSystemUsingBound2BoundModel(mapCellToIndex, mapIndexToCell, Ax, Ay, bx, by, epsilon);
	watchBuilder.finish();
	...
	for (int i = 0; i < mapIndexToCell.size(); i++) {
		...
		if(instance.getType() != Rsyn::CELL )
			continue;
		Rsyn::PhysicalCell phCell = phDesign.getPhysicalCell(instance.asCell());
	...
	// Solve linear systems.
	Stepwatch watchSolver("Solving linear system");
	std::thread tx([&](){ ::SolveByConjugateGradientWithDiagonalPreConditioner(Ax, bx, px, 1000); });
	std::thread ty([&](){ ::SolveByConjugateGradientWithDiagonalPreConditioner(Ay, by, py, 1000); });
	tx.join();
	ty.join();
	watchSolver.finish();
...
void QuadraticPlacementExample::buildLinearSystemUsingHybridNetModel(
	...
	for (Rsyn::Net net : nset) {
		...
		if (useStarModel) {
			const double weight = net.getNumPins() / 
			...
			for (Rsyn::Pin pin0 : net.allPins()) {
				...
				const int index0 = (it0 != mapCellToIndex.end()) ? it0->second : -1;
			...
			for (Rsyn::Pin pin0 : net.allPins()) {
				...
				for (Rsyn::Pin pin1 : net.allPins()) {
					...
					const auto it1 = mapCellToIndex.find(cell1);
					const int index1 = (it1 != mapCellToIndex.end()) ? it1->second : -1;

					if ((index0 != -1 && index1 != -1) && index0 >= index1)
						continue; 

					const double weight = 1.0 / double(std::max(1, (net.getNumPins()-1)));
...
void QuadraticPlacementExample::buildLinearSystemUsingBound2BoundModel_AddConnections(
	...
	for (int i = 1; i < numPins-1; i++) {
		...
		buildLinearSystemUsingBound2BoundModel_Add(numPins, lindex, lpos, iindex, ipos, epsilon, dscp, b, hpwl);
		buildLinearSystemUsingBound2BoundModel_Add(numPins, uindex, upos, iindex, ipos, epsilon, dscp, b, hpwl);
	} // end for
} // end method
...
void QuadraticPlacementExample::buildLinearSystemUsingBound2BoundModel(
	...
	for (Rsyn::Net net : nset) {
		clsRelaxedPinPos->getNetPinsSortedByRelaxedPinPositions(net, X, xPins);
		clsRelaxedPinPos->getNetPinsSortedByRelaxedPinPositions(net, Y, yPins);
		...
		buildLinearSystemUsingBound2BoundModel_AddConnections(mapCellToIndex, epsilon, net, xPins, dscpx, bx, netB2BHPWLx);
		buildLinearSystemUsingBound2BoundModel_AddConnections(mapCellToIndex, epsilon, net, yPins, dscpy, by, netB2BHPWLy);
	...
	std::cout << "HPWLx == B2B HPWLx? " << FloatingPoint::approximatelyEqual(totalHPWLx, totalB2BHPWLx) << "\n";
	std::cout << "HPWLy == B2B HPWLy? " << FloatingPoint::approximatelyEqual(totalHPWLy, totalB2BHPWLy) << "\n";
...
void QuadraticPlacementExample::buildLinearSystemUsingBound2BoundModel_CheckConsistency(
