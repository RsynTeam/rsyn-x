...
void LayoutOverlay::renderTracks(PhysicalCanvasGL * canvas) {
	...
	for(Rsyn::PhysicalTrack phTrack : phDesign.allPhysicalTracks()) {
		Rsyn::PhysicalTrackDirection direction = phTrack.getDirection();
		Rsyn::PhysicalTrackDirection reverse = 
			direction == Rsyn::TRACK_HORIZONTAL ? Rsyn::TRACK_VERTICAL : Rsyn::TRACK_HORIZONTAL;
		const DBU space = phTrack.getSpace();
		for (Rsyn::PhysicalLayer phLayer : phTrack.allLayers()) {
			if (!clsViewTrackLayer[phLayer.getIndex()])
				continue;

			if ((direction == Rsyn::TRACK_HORIZONTAL && phLayer.getDirection() == Rsyn::HORIZONTAL) ||
					(direction == Rsyn::TRACK_VERTICAL && phLayer.getDirection() == Rsyn::VERTICAL)) {
			...
			p0[direction] = phTrack.getLocation();
			p1[direction] = phTrack.getLocation();
