...
void Report::start(const Rsyn::Json &params) {
	...
	{ // reportCell
		...
		clsSession.registerCommand(dscp, [&](const ScriptParsing::Command & command) {
			...
			Rsyn::Instance cell = clsDesign.findCellByName(cellName);

			if (!cell || cell.getType() != Rsyn::CELL) {
				std::cout << "Cell " << cellName << " not found.\n";
				return;
			} // end if
	...
	{ // reportNet
		...
		clsSession.registerCommand(dscp, [&](const ScriptParsing::Command & command) {
			...
			if (!net) {
				std::cout << "Net " << netName << " not found.\n";
				return;
			} // end if
	...
	{ // reportPin
		...
		clsSession.registerCommand(dscp, [&](const ScriptParsing::Command & command) {
			...
			Rsyn::Pin pin = clsDesign.findPinByName(pinName, separator[0]);

			if (!pin) {
				std::cout << "Pin " << pinName << " not found.\n";
				return;
			} // end if
	...
	{ // reportTree
		...
		dscp.setDescription("Report informations about the routing tree of a given net.");
...
void Report::reportPin_Data(Rsyn::Pin pin, const TimingMode mode) {
	...
	std::cout << std::setw(5) << Rsyn::Global::getDirectionShortName(pin.getDirection()) << " ";
	std::cout << std::right;
	...
	if (net && pin.isDriver()) {
		...
		const double pinLoad = clsRoutingEstimator->getRCTree(net).getLoadCap().getMax();
		const double wireLoad = clsRoutingEstimator->getRCTree(net).getTotalWireCap();
	...
	std::cout << std::setw(9) << clsTimer->getPinArrivalTime(pin, mode, FALL) << " ";
	std::cout << std::setw(9) << clsTimer->getPinArrivalTime(pin, mode, RISE) << " ";
	std::cout << std::setw(9) << clsTimer->getPinRequiredTime(pin, mode, FALL) << " ";
	std::cout << std::setw(9) << clsTimer->getPinRequiredTime(pin, mode, RISE) << " ";
...
void Report::reportArc_Data(Rsyn::Arc arc, const TimingMode mode) {
	std::cout
		<< std::left
		<< std::setw(10) << arc.getName() << " "
		<< std::right
		<< std::setw(9) << clsTimer->getArcDelay(arc, mode, FALL) << " "
		<< std::setw(9) << clsTimer->getArcDelay(arc, mode, RISE) << " "
		<< std::setw(9) << clsTimer->getArcOutputSlew(arc, mode, FALL) << " "
		<< std::setw(9) << clsTimer->getArcOutputSlew(arc, mode, RISE) << " ";

	if (arc.getLibraryArc()) {
		// Cell arc.
		std::cout
			<< std::setw(9) << clsLibraryCharacterizer->getDriverResistance(arc, mode, FALL) << " "
			<< std::setw(9) << clsLibraryCharacterizer->getDriverResistance(arc, mode, RISE) << "\n";
	} else {
		// Net arc.
		std::cout
			<< std::setw(9) << "N/A" << " "
...
void Report::reportCell_TimingInformation(Rsyn::Cell cell, const TimingMode mode) {
	std::cout << "\n";
	...
	// General
	std::cout << "Criticality : " << clsTimer->getCellCriticality(cell, mode) << "\n";
...
void Report::reportNet_TimingInformation(Rsyn::Net net, const TimingMode mode) {
	...
	std::cout << "Wirelength  : " << clsRoutingEstimator->getNetWirelength(net) << "\n";
...
void Report::reportPin(Rsyn::Pin pin, const bool late, const bool early) {
	...
	std::cout << "Library Cell : " << (pin.getInstanceType() == Rsyn::CELL ? pin.getInstance().asCell().getLibraryCellName() : std::string("N/A")) << "\n";
	std::cout << "Position     : " << clsPhysicalDesign.getPinPosition(pin) << "\n";
	std::cout << "Displacement : " << clsPhysicalDesign.getPinDisplacement(pin) << "\n";
...
void Report::reportCell(Rsyn::Cell cell, const bool late, const bool early) {
	const Rsyn::PhysicalCell &phCell = clsPhysicalDesign.getPhysicalCell(cell);
	...
	for (Rsyn::Pin pin : cell.allPins()) {
		...
		std::cout << std::left << std::setw(9) << Rsyn::Global::getDirectionShortName(pin.getDirection()) << " ";
...
void Report::reportTree(Rsyn::Net net) {
	...
	for (int i = 0; i < numNodes; i++) {
		...
		std::cout << std::setw(7) << clsRoutingEstimator->getWireCapToParentNode(tree, i) << " ";
		std::cout << std::setw(7) << clsRoutingEstimator->getDownstreamCapacitanceIgnoringParentWire(tree, i) << " ";
		std::cout << (tag.getPin() ? (tag.getPin().getFullName()) : std::string("-")) << "\n";
	} // end for
