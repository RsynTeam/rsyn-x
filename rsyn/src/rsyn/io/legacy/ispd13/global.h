...
namespace ISPD13 {
	...
	public:
		...
		const SpefNet &getNet(const int i) const {
			return SPEFNets[i];
		};

		int getSize() const {
			return SPEFNets.size();
		};
	...
	public:
		...
		TimingStruct(string port, double earlyR, double earlyF, double lateR, double lateF) {
			portName = port;
			earlyRise = earlyR;
			earlyFall = earlyF;
			lateRise = lateR;
	...
	public:
		...
		CommandStruct(string pin, string net, string file, string gate, string cell, CommandTypeEnum cmdType) {
			netName = net;
			pinName = pin;
			fileName = file;
			gateName = gate;
	...
	public:
		...
		QueryStruct(string pin, bool isEarly, bool isRise, int numPaths, QueryTypeEnum qType) {
			pinName = pin;
			early = isEarly;
			rise = isRise;
			paths = numPaths;
