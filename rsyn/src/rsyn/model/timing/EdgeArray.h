...
public:
	...
	EdgeType() {clsValue = -1;}
	EdgeType(const TimingTransition edgeTypeEnum) {clsValue = edgeTypeEnum; }
...
class EdgeArray {
	...
	friend EdgeArray<T> abs( const EdgeArray<T> v ) { return EdgeArray<T>(std::abs(v[RISE]),std::abs(v[FALL])); }
	friend EdgeArray<T> pow( const EdgeArray<T> v, const double exp ) { return EdgeArray<T>(std::pow(v[RISE], exp),std::pow(v[FALL], exp)); }
	...
	friend EdgeArray<T> sqrt( const EdgeArray<T> v) { return EdgeArray<T>(std::sqrt(v[RISE]),std::sqrt(v[FALL])); }
	friend EdgeArray<T> exp( const EdgeArray<T> v) { return EdgeArray<T>(std::exp(v[RISE]), std::exp(v[FALL])); }
...
public:
	...
	// This constructor is explicit to avoid unintended assignment from scalar
	// to EdgeArray.
	...
	TimingTransition getMaxEdge() const { return (clsValue[RISE] > clsValue[FALL])? RISE : FALL; }
	TimingTransition getMinEdge() const { return (clsValue[RISE] < clsValue[FALL])? RISE : FALL; }
