...
public:
	...
	RCTreeNodeNotFoundException() : Exception("RCTree: Node not found in RC tree") {
	}
};
	...
	struct Resistor {
		...
		int getOtherNode(const int n) const {
			if (n == propNode0)
				...
				return propNode0;
			else throw RCTreeOtherNodeNotFound();
		} // end method
...
public:
	...
	// TODO: Add description.
	void addResistor(const NameType &sourceNode, const NameType &targetNode, const Number resistance);
...
public:
	...
	struct Node {
		...
		// Stores the cap related to design pins (e.g. gate inputs or primary
		// output pins).
		...
		EdgeArray<Number> propDownstreamCapDelay; // used to slew computation (ICCAD 2014 Contest)
		...
		EdgeArray<Number> getTotalCap() const {
			return EdgeArray<Number>(propWireCap, propWireCap) + propPinCap;
		} // end method

		// Returns only the wire cap associated to this node.

		Number getWireCap() const {
			return propWireCap;
		} // end method
		...
		// Returns the downstream cap i.e. the sum of the wire and pin cap of 
		// this node and all nodes driven, direct or indirectly, by this node.

		EdgeArray<Number> getDownstreamCap() const {
			return propDownstreamCap;
		} // end method
		
		// Returns the upstream res i.e. the sum of the wire segment resistance
...
protected:
	...
	static bool nearlyEqual(const Number x, const Number y, const Number precision);
	...
	// If removeLoops is false, throws RCTreeLoopException exception if a loop
	// is detected.
	void buildTopology(const RCTreeDescriptorTemplate<NameType, TagType> &dscp, 
			const int root, 
...
public:
	...
	// If removeLoops is true, it returns false if a loop is detected and true
	// otherwise.
	// If removeLoops is false, throw RCTreeLoopException exception if a loop
	// is detected.
	bool build(const RCTreeDescriptorTemplate<NameType, TagType> &dscp, 
			const NameType &rootNodeName, 
			const bool removeLoops = false,
	...
	// of Large ASICs.
	template<class RCTreeDriver>
	void simulate(const RCTreeDriver &driver, const Number epsilon = 1e-6, const int maxIterations = 100);

	// [PAPER] Modeling the Effective Capacitance for the RC Interconnect of
	// CMOS Gates

	template<class RCTreeDriver>
	EdgeArray<Number> computeEffectiveCapacitanceBasedOnJessica(const RCTreeDriver &driver, const Number epsilon = 1e-6, const int maxIterations = 100);

	// [PAPER] Performance Computation for Pre-characterized
	// CMOS Gates with RC Loads

	template<class RCTreeDriver>
	EdgeArray<Number> computeEffectiveCapacitanceBasedOnMenezes(const RCTreeDriver &driver, const Number epsilon = 1e-6, const int maxIterations = 10 );

	// [PAPER] Modeling the Driving-Point Characteristic of Resistive
	// Interconnect for Accurate Delay Estimation

	void reduceToPiModel(EdgeArray<Number> &C1, EdgeArray<Number> &R, EdgeArray<Number> &C2);
	...
	// argument. If there are more than one downstream node, the node passed as
	...
	// Returns the index of the upstream steiner point that the node passed as
	// argument connects to. If the node passed as argument is not an endpoint
	// returns the node index itself.
	int getUpstreamSteinerPoint(const int nodeIndex) const;
	
	// Returns the node index of the fist downstream node that spans more than
	...
	std::vector<int>
	allDownstreamNodes(const int startNodeIndex, const bool includeStartNode) const;
}; // end class
...
public:
	...
	RCTreeNodeTag(Rsyn::Pin pin, const DBU x, const DBU y) : pin(pin), x(x), y(y) {}
...
template<class NameType, class TagType>
inline
RCTreeDescriptorTemplate<NameType, TagType>::RCTreeDescriptorTemplate() {
...
inline
int
RCTreeDescriptorTemplate<NameType, TagType>::createNode(const NameType &name) {
...
inline
void
RCTreeDescriptorTemplate<NameType, TagType>::addResistor(const NameType &sourceNode, const NameType &targetNode, const Number resistance) {
...
inline
void
RCTreeDescriptorTemplate<NameType, TagType>::addCapacitor(const NameType &node, const Number capacitance) {
...
inline
int
RCTreeDescriptorTemplate<NameType, TagType>::getNumNodes() const {
...
inline
int
RCTreeDescriptorTemplate<NameType, TagType>::getNumResistors() const {
...
inline
int
RCTreeDescriptorTemplate<NameType, TagType>::getNumCapacitors() const {
...
template<class NameType, class TagType>
inline
const typename RCTreeDescriptorTemplate<NameType, TagType>::Resistor &
RCTreeDescriptorTemplate<NameType, TagType>::getResistor(const int index) const {
...
template<class NameType, class TagType>
inline
const typename RCTreeDescriptorTemplate<NameType, TagType>::Capacitor &
RCTreeDescriptorTemplate<NameType, TagType>::getCapacitor(const int index) const {
...
inline
int
RCTreeDescriptorTemplate<NameType, TagType>::findNode(const NameType &name) const {
...
inline
int
RCTreeDescriptorTemplate<NameType, TagType>::findNodeOrException(const NameType &name) {
	typename std::map<NameType, int>::const_iterator it = clsNodeMap.find(name);

	if (it == clsNodeMap.end())
		throw RCTreeNodeNotFoundException();
	return it->second;
} // end method
...
inline
TagType &
RCTreeDescriptorTemplate<NameType, TagType>::getNodeTag(const NameType nodeName) {
...
inline
const TagType &
RCTreeDescriptorTemplate<NameType, TagType>::getNodeTag(const NameType nodeName) const {
	return clsNodes[findNodeOrException(nodeName)].propTag;
} // end method
...
inline
void
RCTreeDescriptorTemplate<NameType, TagType>::setNodeTag(const NameType nodeName, const TagType &tag) {
...
inline
void
RCTreeDescriptorTemplate<NameType, TagType>::applyDefaultNodeTag(const TagType &tag) {
...
inline
void
RCTreeDescriptorTemplate<NameType, TagType>::applyDefaultCap(const Number cap) {
...
template<class NameType, class TagType>
inline
void RCTreeDescriptorTemplate<NameType, TagType>::print() const {
//		const int numNodes = clsNodes.size();
//		for ( int i = 0; i < numNodes; i++ ) {
//			const auto &node = clsNodes[i];
//
//			std::cout << "node " << node.propName << "\n";
//			for (int k = 0; k < node.propResistors.size(); k++) {
//				const auto &res = clsResistors[node.propResistors[k]];
//				const auto &name0 = clsNodes[res.propNode0].propName;
//				const auto &name1 = clsNodes[res.propNode1].propName;
//
//				std::cout << "\tres " << name0 << "<-->" << name1 << " " << res.propValue;
//				if (node.propName != name0  && node.propName != name1) {
...
//		} // end for
	...
	for (int i = 0; i < numCapacitors; i++) {
		...
		cout
			<< "cap "
			<< clsNodes[cap.propNode].propName << " "
			<< cap.propValue << "\n"
			;
	} // end for
} // end method
...
template<class NameType, class TagType>
inline
RCTreeBaseTemplate<NameType, TagType>::RCTreeBaseTemplate() {
	clear();
} // end constructor
...
inline
bool
RCTreeBaseTemplate<NameType, TagType>::nearlyEqual(const Number x, const Number y, const Number precision) {
...
inline
EdgeArray<Number>
RCTreeBaseTemplate<NameType, TagType>::pow2(const EdgeArray<Number> v) {
	return v*v;
} // end method
...
inline
EdgeArray<Number>
RCTreeBaseTemplate<NameType, TagType>::pow3(const EdgeArray<Number> v) {
	return v * v*v;
} // end method
...
template<class NameType, class TagType>
inline
void RCTreeBaseTemplate<NameType, TagType>::buildTopology(
	...
	while (!q.empty()) {
		...
		const typename RCTreeDescriptorTemplate<NameType, TagType>::Resistor
			&resistorDescriptor = dscp.getResistor(r);
...
template<class RCTreeDriver>
inline
void RCTreeBaseTemplate<NameType, TagType>::stepForward(const RCTreeDriver &driver) {
...
template<class NameType, class TagType>
inline
void RCTreeBaseTemplate<NameType, TagType>::stepBackward() {
	const int numNodes = clsNodes.size();
	...
	for (int n = numNodes - 1; n > 0; n--) { // n > 0 skips root node
		...
		const EdgeArray<Number> RCeff = R1*Ceff1;
		const EdgeArray<Number> K1 = 1.0 - ((2.0 * RCeff) / S0)*(1.0 - exp(-S0 / (2.0 * RCeff)));
...
template<class NameType, class TagType>
inline
void RCTreeBaseTemplate<NameType, TagType>::clear() {
	clsDirty = false;
...
template<class NameType, class TagType>
inline
bool RCTreeBaseTemplate<NameType, TagType>::isIdeal() const {
	return clsIdeal;
} // end method
...
template<class NameType, class TagType>
inline
bool RCTreeBaseTemplate<NameType, TagType>::hasUserSpecifiedWireLoad() const {
...
template<class NameType, class TagType>
inline
bool RCTreeBaseTemplate<NameType, TagType>::hasUserSpecifiedRouting() const {
...
template<class NameType, class TagType>
inline
Number RCTreeBaseTemplate<NameType, TagType>::getUserSpecifiedWireLoad() const {
...
template<class NameType, class TagType>
inline
void RCTreeBaseTemplate<NameType, TagType>::setUserSpecifiedWireLoad(const float load) {
...
template<class NameType, class TagType>
inline
void RCTreeBaseTemplate<NameType, TagType>::resetUserSpecifiedWireLoad() {
	clsUserSpecifiedWireLoad = 0;
	clsHasUserSpecifiedWireLoad = false;
} // end method
...
template<class NameType, class TagType>
inline
void RCTreeBaseTemplate<NameType, TagType>::setIdeal(const bool enable) {
	clsIdeal = enable;
} // end if
...
template<class NameType, class TagType>
inline
bool RCTreeBaseTemplate<NameType, TagType>::build(const RCTreeDescriptorTemplate<NameType, TagType> &dscp,
		const NameType &rootNodeName,
		const bool removeLoops,
	...
	// descriptor.
	bool loopDetected;
	buildTopology(dscp, dscp.findNode(rootNodeName), removeLoops, loopDetected);
...
template<class NameType, class TagType>
inline
void RCTreeBaseTemplate<NameType, TagType>::updateDownstreamCap() {
...
template<class NameType, class TagType>
inline
void RCTreeBaseTemplate<NameType, TagType>::updateDownstreamBranchingCount() {
	const int numNodes = getNumNodes();
	for (int i = 0; i < numNodes; i++) {
		Node &node = clsNodes[i];
...
template<class NameType, class TagType>
inline
void RCTreeBaseTemplate<NameType, TagType>::updateUpstreamRes() {
	const int numNodes = getNumNodes();
	if (numNodes == 0)
		return;
	...
	for (int i = 1; i < numNodes; i++) { // skip root
		Node &node = clsNodes[i];
		const Node &parent = clsNodes[node.propParent];
		node.propUpstreamRes = parent.propUpstreamRes + node.propDrivingResistance;
	} // end for
} // end method
...
template<class NameType, class TagType>
inline
void RCTreeBaseTemplate<NameType, TagType>::updateDrivingPoint() {
	const int numNodes = getNumNodes();
	for (int i = 0; i < numNodes; i++) {
		Node &node = clsNodes[i];
		node.propY1.set(0, 0);
	...
	for (int n = numNodes - 1; n > 0; n--) { // n > 0 skips root node
		...
		const EdgeArray<Number> R = EdgeArray<Number>(node.propDrivingResistance,
			node.propDrivingResistance);
		...
		const EdgeArray<Number> yU2 = yD2 - R * (pow2(yD1) + C * yD1 + (1.0 / 3.0) * pow2(C));
		const EdgeArray<Number> yU3 = yD3 - R * (2 * yD1 * yD2 + C * yD2) +
...
inline
void
RCTreeBaseTemplate<NameType, TagType>::simulate(const RCTreeDriver &driver, const Number epsilon, const int maxIterations) {
	if (clsDirty)
		updateDownstreamCap();
	...
	for (int i = 0; i < numNodes; i++) {
		Node &node = clsNodes[i];
		node.propEffectiveCap = EdgeArray<Number>(clsNodes[i].propDownstreamCap, clsNodes[i].propDownstreamCap);
	} // end for

	bool converged = false;
	EdgeArray<Number> previousRootEffectiveCapacitance = clsNodes[0].propEffectiveCap;
...
template<class RCTreeDriver>
inline
EdgeArray<Number> RCTreeBaseTemplate<NameType, TagType>::computeEffectiveCapacitanceBasedOnJessica(
	...
	for (int i = 0; i < maxIterations; i++) {
		...
		const EdgeArray<Number> shielding = (1 -
			(R * C1) / (tD - tx / 2.0) +
...
inline
EdgeArray<Number>
RCTreeBaseTemplate<NameType, TagType>::computeEffectiveCapacitanceBasedOnMenezes(
		const RCTreeDriver &driver,
		const Number epsilon,
		const int maxIterations
) {
...
//
//		const EdgeArray<Number> td = driver.computeDelay(Ceff);
//		const EdgeArray<Number> deltatd = (driver.computeDelay(Ceff) - driver.computeDelay(Ceff*0.99));
//		const EdgeArray<Number> tf = driver.computeSlew(Ceff);
//
...
//
//		//   cout << "running iteration with f and df defined, over all Number Numbers" << endl;
...
//
//		//Ceff = EdgeArray<Number>(newtonRise.do_iteration( &cout), newtonFall.do_iteration( &cout)) ;
//		Ceff = EdgeArray<Number>(newtonRise.do_iteration( NULL/*&cout*/), newtonFall.do_iteration( NULL/*&cout*/)) ;
//		//cout << "Ceff: " << Ceff << endl;
//
//		if ( nearlyEqual(previousCeff[RISE], Ceff[RISE], epsilon ) &&
//			nearlyEqual(previousCeff[FALL], Ceff[FALL], epsilon )	)
...
template<class NameType, class TagType>
inline
void RCTreeBaseTemplate<NameType, TagType>::reduceToPiModel(EdgeArray<Number> &C1, EdgeArray<Number> &R, EdgeArray<Number> &C2) {
	if (clsDirty)
		updateDownstreamCap();
...
template<class NameType, class TagType>
inline
void RCTreeBaseTemplate<NameType, TagType>::elmore() {
	const int numNodes = clsNodes.size();
	...
	for (int n = 1; n < numNodes; n++) { // 1 => skips root node
		...
		node.propDelay = parent.propDelay +
	...
	for (int i = 0; i < numNodes; i++) {
		Node &node = clsNodes[i];
		node.propDownstreamCapDelay = node.getTotalCap() * node.propDelay;
	} // end for

	for (int n = numNodes - 1; n > 0; n--) { // n > 0 skips root node
		const Node &node = clsNodes[n];
		clsNodes[node.propParent].propDownstreamCapDelay += node.propDownstreamCapDelay;
	} // end for
	...
	for (int n = 1; n < numNodes; n++) { // 1 => skips root node
		...
		node.propSecondMoment = parent.propSecondMoment +
			node.propDrivingResistance * node.propDownstreamCapDelay;

		node.propSlew = sqrt(si +
			abs(2*node.propSecondMoment - pow2(node.propDelay)));
	} // end for
} // end method
...
template<class NameType, class TagType>
inline
void RCTreeBaseTemplate<NameType, TagType>::setNodeLoadCap(const int index, const EdgeArray<Number> cap) {
...
template<class NameType, class TagType>
inline
void RCTreeBaseTemplate<NameType, TagType>::setInputSlew(const EdgeArray<Number> slew) {
...
template<class NameType, class TagType>
inline
int RCTreeBaseTemplate<NameType, TagType>::getNumNodes() const {
	return clsNodes.size();
} // end method
...
template<class NameType, class TagType>
inline
const typename RCTreeBaseTemplate<NameType, TagType>::Node &
...
inline
const NameType &
RCTreeBaseTemplate<NameType, TagType>::getNodeName(const int index) const {
...
template<class NameType, class TagType>
inline
const TagType &RCTreeBaseTemplate<NameType, TagType>::getNodeTag(const int index) const {
...
template<class NameType, class TagType>
inline
TagType &RCTreeBaseTemplate<NameType, TagType>::getNodeTag(const int index) {
	return clsNodeTags[index];
} // end method
...
template<class NameType, class TagType>
inline
EdgeArray<Number> RCTreeBaseTemplate<NameType, TagType>::getLumpedCap() const {
	return clsLumpedCap;
} // end method
...
template<class NameType, class TagType>
inline
EdgeArray<Number> RCTreeBaseTemplate<NameType, TagType>::getLoadCap() const {
	return clsLoadCap;
} // end method
...
template<class NameType, class TagType>
inline
Number RCTreeBaseTemplate<NameType, TagType>::getTotalWireCap() const {
...
template<class NameType, class TagType>
inline
bool RCTreeBaseTemplate<NameType, TagType>::getLoopDetectedAndRemovedFlag() const {
...
template<class NameType, class TagType>
inline
bool RCTreeBaseTemplate<NameType, TagType>::getDirtyFlag() const {
	return clsDirty;
} // end method
...
template<class NameType, class TagType>
inline
int RCTreeBaseTemplate<NameType, TagType>::getDownstreamNodeIndex(const int nodeIndex) const {
	const Node &node = getNode(nodeIndex);
	return node.propSinks.size() == 1?
			node.propSinks[0] : nodeIndex;
} // end method
...
template<class NameType, class TagType>
inline
int RCTreeBaseTemplate<NameType, TagType>::getUpstreamNodeIndex(const int nodeIndex) const {
...
template<class NameType, class TagType>
inline
int RCTreeBaseTemplate<NameType, TagType>::getUpstreamSteinerPoint(const int nodeIndex) const {
	const Node &node = getNode(nodeIndex);
...
template<class NameType, class TagType>
inline
int RCTreeBaseTemplate<NameType, TagType>::getDownstreamBranchingNodeIndex(const int nodeIndex) const {
	int i = nodeIndex;
	while (true) {
...
template<class NameType, class TagType>
inline
int RCTreeBaseTemplate<NameType, TagType>::getUpstreamBranchingNodeIndex(const int nodeIndex) const {
	int i = nodeIndex;
	while (i != -1) {
		const Node &node = getNode(i);
		if (node.propSinks.size() != 1) {
...
inline
std::vector<int>
RCTreeBaseTemplate<NameType, TagType>::allDownstreamNodes(const int startNodeIndex, const bool includeStartNode) const {
