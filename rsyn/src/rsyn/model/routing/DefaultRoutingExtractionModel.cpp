...
void
DefaultRoutingExtractionModel::stop() {
...
void DefaultRoutingExtractionModel::extract(const Rsyn::RoutingTopologyDescriptor<int> &topology, RCTree &tree) {
	...
	for (const RoutingTopologyDescriptor<int>::Segment &segment : topology.allSegments()) {
		const RoutingTopologyDescriptor<int>::Node &node0 = topology.getNode(segment.propNode0);
		const RoutingTopologyDescriptor<int>::Node &node1 = topology.getNode(segment.propNode1);

		const DBU wirelength = DBUxy::computeManhattanDistance(
			node0.propPosition, node1.propPosition);

		if (ENABLE_LONG_WIRE_SLICING && wirelength > longWirelengthThreshold) {
			...
			generateSteinerTree_SliceLongWire(
				longWirelengthThreshold,
				wireCapPerDistanceUnit,
				wireResPerDistanceUnit,
				index0,
				index1,
				x0, y0,
				x1, y1,
			...
			const Number wireHalfCap = (wireCapPerDistanceUnit*wirelength)/2.0f;
			const Number wireRes = wireResPerDistanceUnit*wirelength;

			dscp.addCapacitor(segment.propNode0, wireHalfCap);
			dscp.addResistor(segment.propNode0, segment.propNode1, wireRes);
...
//			} // end for			
		...
		if (node.propPin && node.propPin.isDriver()) {
			if (driver) {
				std::cout << "WARNING: Generating RC-Tree for a multiple-drive net.\n";
			} // end if
			driver = node.propPin;
			root = node.propIndex;
		} // end if
	...
	if (!tree.build(dscp, root, true)) {
		...
		if (!warned) {
			std::cout << "\n[WARNING] A loop was detected (and removed) when building "
					<< "the tree. "
					<< "The tree returned by FLUTE may generate "
					<< "loops when nodes at same position (x, y) are merged. "
					<< "Typically FLUTE will generate several steiner points at a same "
					<< "position for large nets. "
					<< "Note that this is not necessary a bug in the merging scheme, "
					<< "but just a consequence of the tree topology returned by FLUTE. "
	...
	for (int i = 1; i < numRCTreeNodes; i++) { // start @ 1 to skip root node
		...
		if (pin.isPort()) {
			...
			const Number outputCap = clsScenario->getOutputLoad(port, 0);
			load[RISE] = outputCap;
			load[FALL] = outputCap;
		} else {
...
int DefaultRoutingExtractionModel::generateSteinerTree_SliceLongWire(
	const DBU longWirelengthThreshold,
	const Number wireCapPerMicron,
	const Number wireResPerMicron,
	const int index_i,
	const int index_j,
	const DBU xi, const DBU yi,
	const DBU xj, const DBU yj,
	std::deque<SlicingPoint> &slicingPoints,
	RCTreeDescriptor &dscp,
	int &sliceId) const {

	// NOTE. Don't clear the slicing points collection, as a net may have
	// several wires sliced and all slicing points are stored cumulatively in
	// the same collection.
	...
	if (totalWirelength <= longWirelengthThreshold) {
		std::cout << "[BUG] Requesting to slice a wire that is short or equal to the maximum wire segment.\n";
	} // end if

	// Compute the number of slicing points.
	const int numSlicingPoints = totalWirelength/longWirelengthThreshold -
		((totalWirelength % longWirelengthThreshold)? 0 : 1);

	// Create all entire slices, except the last one if that one connects to
	// the endpoint. This happens when the total wirelength is multiple of the
	...
	const Number entireSliceHalfCap = wireCapPerMicron*entireSliceWirelength / 2.0f;
	const Number entireSliceRes = wireResPerMicron*entireSliceWirelength;

	const DBU multiplier[2] = {
			(xi > xj)? +1 : -1,
			(yi > yj)? +1 : -1
	};
	...
	for (int i = 0; i < numSlicingPoints; i++) {
		// Try to move all we can in the current dimension and change direction
		// if necessary.
		...
		if (currDisp[currDim] > size[currDim]) {
			...
			if (reversed) {
				std::cout << "[BUG] Reversing direction a second time.\n";
			} // end if
	...
	if (wl >= totalWirelength) {
		std::cout << "[BUG] Sum of wire segment length is greater or equal to the "
				"total wire segment.\n";
	} // end if

	if (std::abs(xk - xi) + std::abs(yk - yi) != totalWirelength - wl) {
		std::cout << "[BUG] The distance from the last slice point to the endpoint "
				"does not match the total wire length.\n";
		std::cout << "xj, yj = " << xj << ", " << yj << "\n";
		std::cout << "xk, yk = " << xk << ", " << yk << "\n";
		std::cout << "xi, yi = " << xi << ", " << yi << "\n";
		std::cout << "dist(k, i) = " << (std::abs(xk - xi) + std::abs(yk - yi)) << "\n";
	...
	const Number lastSliceHalfCap = wireCapPerMicron*lastSliceWirelength / 2.0f;
	...
	if (FloatingPoint::notApproximatelyEqual(totalCap, wireCapPerMicron * totalWirelength)) {
		std::cout << "[BUG] Total capacitance of sliced interconnection does not match the expected capacitance.\n";
		std::cout << "  sum cap = " << totalCap << "\n";
		std::cout << "total cap = " << (wireCapPerMicron * totalWirelength) << "\n";
	} // end if

	if (FloatingPoint::notApproximatelyEqual(totalRes, wireResPerMicron * totalWirelength)) {
		std::cout << "[BUG] Total resistance of sliced interconnection does not match the expected resistance.\n";
		std::cout << "  sum res = " << totalRes << "\n";
		std::cout << "total res = " << (wireResPerMicron * totalWirelength) << "\n";
	} // end if
