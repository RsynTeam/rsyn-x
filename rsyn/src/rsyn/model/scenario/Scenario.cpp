...
void Scenario::init_Mode(const TimingMode mode, const ISPD13::LIBInfo &lib) {
	for (const ISPD13::LibParserCellInfo &libCell : lib.libCells) {
		Rsyn::LibraryCell rsynLibraryCell = clsDesign.findLibraryCellByName(libCell.name);
		Rsyn::LibraryCellTag rsynLibraryCellTag = clsDesign.getTag(rsynLibraryCell);

		if (libCell.isSequential) {
			rsynLibraryCellTag.setLogicType(Rsyn::LOGIC_TYPE_TAG_SEQUENTIAL);
		} else {
			rsynLibraryCellTag.setLogicType(Rsyn::LOGIC_TYPE_TAG_COMBINATIONAL);
		} // end else
		...
		if (libCell.timingArcs.size() == 1) {
			rsynLibraryCellTag.setBufferType(libCell.timingArcs[0].timingSense == "positive_unate"?
				Rsyn::BUFFER_TYPE_TAG_NON_INVERTING : Rsyn::BUFFER_TYPE_TAG_INVERTING);
		} else {
			rsynLibraryCellTag.setBufferType(Rsyn::BUFFER_TYPE_TAG_NONE);
		} // end if
		...
		for (const ISPD13::LibParserTimingInfo &libArc : libCell.timingArcs) {
			...
			TimingLibraryArc &timingLibraryArc = getTimingLibraryArc(
					rsynLibraryCell.getLibraryArcByPinNames(libArc.fromPin, libArc.toPin));

			timingLibraryArc.sense = getTimingSenseFromString(libArc.timingSense);
			
			setTimingLibraryArcDelayModel(timingLibraryArc, mode, RISE, libArc.riseDelay);
			setTimingLibraryArcDelayModel(timingLibraryArc, mode, FALL, libArc.fallDelay);

			setTimingLibraryArcSlewModel(timingLibraryArc, mode, RISE, libArc.riseTransition);
			setTimingLibraryArcSlewModel(timingLibraryArc, mode, FALL, libArc.fallTransition);
		} // end for
		...
		for (const ISPD13::LibParserPinInfo &libPin : libCell.pins) {
			...
			setTimingLibraryPinCapacitance(timingLibraryPin, (Number) libPin.capacitance);
			
			if (rsynLibraryPin.getDirection() == Rsyn::OUT) {
				setTimingLibraryPinMaxCapacitance(timingLibraryPin, (Number) libPin.maxCapacitance);
			} // end if 
			
			if (rsynLibraryPin.getDirection() == Rsyn::IN) {
				if(libPin.maxTransition == -std::numeric_limits<Number>::infinity())
					setTimingLibraryPinMaxTransition(timingLibraryPin, (Number) lib.default_max_transition);
				else
					setTimingLibraryPinMaxTransition(timingLibraryPin, (Number) libPin.maxTransition);
			} // end if 
			...
			if (libPin.isTimingEndpoint) {
				...
				if (mode == EARLY) {
					setTimingLibraryPinHoldConstraint(timingLibraryPin, timingLibraryPinRelated, RISE, libPin.riseHold);
					setTimingLibraryPinHoldConstraint(timingLibraryPin, timingLibraryPinRelated, FALL, libPin.fallHold);
				} else {
					setTimingLibraryPinSetupConstraint(timingLibraryPin, timingLibraryPinRelated, RISE, libPin.riseSetup);
					setTimingLibraryPinSetupConstraint(timingLibraryPin, timingLibraryPinRelated, FALL, libPin.fallSetup);
				} // end else
			} // end if
		} // end for
		
		TimingLibraryCell &timingLibraryCell = getTimingLibraryCell(rsynLibraryCell);
...
bool Scenario::isUnusualTimingArc(const ISPD13::LibParserTimingInfo &libArc) const {
...
void Scenario::setTimingLibraryArcDelayModel(
...
void Scenario::setTimingLibraryArcSlewModel(
	TimingLibraryArc &larc,
	const TimingMode mode,
	const TimingTransition transition,
	const ISPD13::LibParserLUT &lut) {
...
void Scenario::setTimingLibraryPinCapacitance(
...
void Scenario::setTimingLibraryPinMaxCapacitance(
	TimingLibraryPin &lpin,
	const Number maxCap) {
...
void Scenario::setTimingLibraryPinMaxTransition(
	TimingLibraryPin &lpin,
...
void Scenario::init_Constraints(const ISPD13::SDCInfo &sdc) {
	...
	if (!clsClockNet) {
		throw Rsyn::Exception("Clock net " + sdc.clk_port + " not found.");
	} // end if

	clsClockPort = clsDesign.findPortByName(sdc.clk_port);
	if (!clsClockPort) {
		throw Rsyn::Exception("Clock port " + sdc.clk_port + " not found.");
	} // end if	
	...
	for (const ISPD13::InputDelay &data : sdc.input_delays) {
		...
		if (port) {
			...
			std::cout << "[WARNING] Port "
				<< data.port_name << " referenced in SDC file not found [Input Delay].\n";
		} // end else
	} // end for
	...
	for (const ISPD13::OutputDelay &data : sdc.output_delays) {
		...
		if (port) {
			...
			std::cout << "[WARNING] Port "
				<< data.port_name << " referenced in SDC file not found [Output Delay].\n";
		} // end else
	} // end for	
	...
	for (const ISPD13::OutputLoad &data : sdc.output_loads) {
		...
		if (port) {
			...
			std::cout << "[WARNING] Port "
				<< data.port_name << " referenced in SDC file not found [Output Load].\n";
		} // end else
	} // end for	
	...
	for (const ISPD13::InputDriver &data : sdc.input_drivers) {
		...
		if (port) {
			Rsyn::LibraryCell driver = clsDesign.findLibraryCellByName(data.driver);

			InputDriver inputDriver;
			inputDriver.driver = driver;
			inputDriver.inputSlew.set((Number) data.rise, (Number) data.fall);
			...
			if (!inputDriver.libraryArc) {
				std::cout << "[BUG] No timing arc found for the input driver.\n";
			} // end if

			setInputDriver(port, inputDriver);
		} else {
			std::cout << "[WARNING] Port "
				<< data.port_name << " referenced in SDC file not found [Input Driver].\n";
		} // end else
	} // end for
...
void Scenario::setInputTransition(Rsyn::Instance port, const EdgeArray<Number> value) {
	clsInputTransitions[port] = value;
} // end method

// -----------------------------------------------------------------------------

void Scenario::setOutputRequiredTime(Rsyn::Instance port, const TimingMode &mode, const EdgeArray<Number> value) {
...
Number Scenario::getInputDelay(Rsyn::Instance port, const Number defaultValue) const {
...
EdgeArray<Number> Scenario::getInputTransition(Rsyn::Instance port, const EdgeArray<Number> defaultValue) const {
...
EdgeArray<Number> Scenario::getOutputRequiredTime(Rsyn::Instance port, const TimingMode &mode, const EdgeArray<Number> defaultValue) const {
...
Number Scenario::getOutputLoad(Rsyn::Instance port, const Number defaultValue) const {
...
const Scenario::InputDriver *Scenario::getInputDriver(Rsyn::Instance cell) const { 
...
void
Scenario::onPostNetCreate(Rsyn::Net net) {
...
void
Scenario::onPostPinConnect(Rsyn::Pin pin) {
	// NOTE: Untested...
...
void
Scenario::onPrePinDisconnect(Rsyn::Pin pin) {
	Rsyn::Net net = pin.getNet();
	Rsyn::NetTag tag = clsDesign.getTag(net);
	if (net.getNumPins() == 1) {
		...
		// Okay, if a clock subtree gets disconnected from the clock tree, let's
...
void Scenario::propagateNetTypeTag(Rsyn::Net seed, Rsyn::NetTypeTag tag) {
	...
	if (seedNetTag.getType() == tag)
		return;

	// Propagate the tag till we find registers or nets that already have the
		...
		for (Rsyn::Pin sink : currentNet.allPins(Rsyn::SINK)) {
			...
			if (!instance.isSequential()) {
				for (Rsyn::Pin pin : instance.allPins(Rsyn::OUT)) {
					...
					if (nextNet) {
						Rsyn::NetTag nextNetTag = clsDesign.getTag(nextNet);
						if (nextNetTag.getType() != tag) {
