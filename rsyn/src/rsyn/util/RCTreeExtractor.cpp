...
std::string RCTreeExtractor::getPinNameInSPEFFormat(Rsyn::Pin pin) {
    ...
    return (instance.getType() == Rsyn::CELL)?(instName+":"+pinName):pinName;
    ...
    if (useDefaultCapacitance) {
        ...
        if (index != -1) {
            ...
            if (dscp.getNode(index).totalCap == 0.0f) {
                Number cap = (Number)Rsyn::Units::convertToInternalUnits(Rsyn::MEASURE_CAPACITANCE, 1.0, unitPrefixForCapacitance);
...
void RCTreeExtractor::extractRCTreeFromSPEF(SPEFNetModel netModel, Rsyn::Net net, Rsyn::RCTree &tree) {
    ...
    const ISPD13::SpefNet &info = spefInfo->getNet(spefInfo->search(net.getName()));
    
//Lumped Capacitance net model
    
    if (netModel == LUMPED_CAPACITANCE_NET_MODEL) {
        double parsedCapacitance = info.netLumpedCap;
        Number capacitance = (Number)Rsyn::Units::convertToInternalUnits(Rsyn::MEASURE_CAPACITANCE, parsedCapacitance, unitPrefixForCapacitance);
...
//RC Tree net model
    ...
    { // builds the topology
        ...
        for (const ISPD13::SpefResistance &resistance : info.resistances) {
            ...
            Number res = (Number)Rsyn::Units::convertToInternalUnits(Rsyn::MEASURE_RESISTANCE, parsedResistance, unitPrefixForResistance);
            dscp.addResistor(fromNode, toNode, res);
        } // end for 
        
        for (const ISPD13::SpefCapacitance &capacitance : info.capacitances) {
            ...
            Number cap = (Number)Rsyn::Units::convertToInternalUnits(Rsyn::MEASURE_CAPACITANCE, parsedCapacitance, unitPrefixForCapacitance);
        ...
        for (const auto &node : clsNodeMap) {
            ...
            if (clsPinMap[node.first]) {
                ...
                RCTreeNodeTag &tag = dscp.getNodeTag(node.second);
                tag.setPin(pin);
            } // end if 
	...
	int root = clsNodeMap[getPinNameInSPEFFormat(driver)];
        ...
        if (!tree.build(dscp, root, true))
            std::cout << "Net: " << net.getName() << ": loop detected.\n";
