...
#include <Rsyn/PhysicalDesignObserver>
#include <Rsyn/DesignObserver>
...
class LayoutGraphicsScene : public GraphicsScene, public DesignObserver, public PhysicalDesignObserver {
public:
	...
	LayoutGraphicsLayer *getGraphicsLayer(const GraphicsLayerEnum &graphicsLayer) const;
	LayoutGraphicsLayer *getRoutingGraphicsLayer(Rsyn::PhysicalLayer &layer) const;
	...
	//! @brief
	void redrawRegionCoveredByRoutingGuides(Rsyn::Net net, const int expandBy);
	...
	//! @brief Selects an instance an returns a the enclosing rectangle of the
	//! instance in scene coordinates. The enclosing rectangle is useful, for
	...
	//! @brief Selects a net an returns a the enclosing rectangle of the pin in
	//! scene coordinates. The enclosing rectangle is useful, for instance, to
	...
	//! @brief Selects a pin an returns a the enclosing rectangle of the pin in
	//! scene coordinates. The enclosing rectangle is useful, for instance, to
	//! zoom in the selected object.
	...
	virtual void
	onPostNetRoutingChange(Rsyn::PhysicalNet physicalNet) override;
...
template<typename T>
void
LayoutGraphicsScene::registerOverlay() {
	...
	registeredOverlayVec.push_back([]() -> LayoutGraphicsLayer *{
		return new T();
	});
} // end method
