...
void
GraphicsItemGridLookup::defineGrid(const QRectF &sceneRect, const int binWidth, const int binHeight) {
...
void
GraphicsItemGridLookup::addItem(GraphicsItem *item) {
	const QRectF boundingRect = item->getBoundingRect();
	forEachBin(boundingRect, [&] (int col, int row) {
		clsGrid(col, row).insert(item);
	});
...
void
GraphicsItemGridLookup::removeItem(GraphicsItem *item) {
	const QRectF boundingRect = item->getBoundingRect();
	forEachBin(boundingRect, [&] (int col, int row) {
		clsGrid(col, row).erase(item);
	});
	clsItems.erase(item);
} // end method
...
GraphicsItem *
GraphicsItemGridLookup::getItemAt(const QPointF &pos) const {
...
GraphicsItemGridLookup::getItemsAt(const QPointF &pos, const int maxNumItems) const {
...
std::list<GraphicsItem *>
GraphicsItemGridLookup::getItemsAt(const QRectF &region, const int maxNumItems) const {
	std::list<GraphicsItem *> result;
	std::unordered_set<GraphicsItem *> visited;
	
	forEachBin(region, [this, &result, &visited, &region](int col, int row) {
		for (GraphicsItem * item : clsGrid(col, row)) {
			if (!visited.count(item)) {
				if (region.intersects(item->getBoundingRect())) {
					visited.insert(item);
...
int
GraphicsItemGridLookup::mapToCol(const qreal x) const {
	const int col = (int) (clsBinWidth > 0? (x -  clsSceneRect.x()) / clsBinWidth : 0);
...
int
GraphicsItemGridLookup::mapToRow(const qreal y) const {
	const int row = (int) (clsBinHeight > 0? (y -  clsSceneRect.y()) / clsBinHeight : 0);
...
int
GraphicsItemGridLookup::getMinCol(const QRectF &region) const {
...
int
GraphicsItemGridLookup::getMinRow(const QRectF &region) const {
...
int
GraphicsItemGridLookup::getMaxCol(const QRectF &region) const {
...
int
GraphicsItemGridLookup::getMaxRow(const QRectF &region) const {
...
void
GraphicsItemGridLookup::forEachBin(const QRectF &region, std::function<void(int, int)> f) const {
