...
inline Rsyn::Direction bookshelfPinDirectionFromString(
    const std::string &direction) {
        if (direction == "I") return Rsyn::IN;
        if (direction == "O") return Rsyn::OUT;
        if (direction == "B")
                return Rsyn::OUT;  // Due to Rsyn restrictions, I am considering
                                   // the Bidirectional is only output.
        return Rsyn::UNKNOWN_DIRECTION;
}  // end function

inline std::string bookshelfPinDirectionToString(
