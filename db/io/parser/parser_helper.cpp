        ...
        while (is && tokens.empty()) {
                ...
                for (unsigned i = 0; i < line.size(); ++i) {
                        ...
                        if (std::isspace(currChar) || isSpecialChar) {
                                if (!token.empty()) {
                                        // Add the current token to the list of
                                        // tokens
...
bool VerilogParser::read_cell_inst(
    ...
    vector<std::pair<string, string> >& pinNetPairs) {
        ...
        assert(tokens.size() >= 4);  // We should have cellType, instName, and
                                     // at least one pin-net pair
        ...
        for (unsigned i = 2; i < tokens.size() - 1; i += 2) {
                ...
                string pinName = tokens[i].substr(
                    1);  // skip the first character of tokens[i]
...
bool VerilogParser::read_cell_inst_tau15(
    string& cellType, string& cellInstName,
    vector<std::pair<string, string> >& pinNetPairs) {
        cellType = "";
        cellInstName = "";
        ...
        assert(tauTokens.size() >= 4);  // We should have cellType, instName,
                                        // and at least one pin-net pair
        ...
        for (unsigned i = 2; i < tauTokens.size() - 1; i += 2) {
                ...
                string pinName = tauTokens[i].substr(
                    1);  // skip the first character of tokens[i]

                pinNetPairs.push_back(
                    std::make_pair(pinName, tauTokens[i + 1]));
        }
...
bool SdcParser::read_input_delay(string& portName, double& delay) {
        ...
        if (valid && tokens[0] == "set_input_delay") {
                ...
                portName = tokens[3];
                if (tokens.size() > 6) portName += "[" + tokens[4] + "]";
...
bool SdcParser::read_output_load(string& outPortName, double& load) {
        ...
        if (valid && tokens[0] == "set_load") {
                ...
                if (tokens.size() > 5) outPortName += "[" + tokens[5] + "]";
...
bool SpefParser::read_connections(vector<SpefConnection>& connections) {
        ...
        vector<string> tokens;
        bool valid =
            read_line_as_tokens(is, tokens, true /*include special chars*/);

        // Skip the lines that are not "*CONN"
        while (
            valid &&
            !(tokens.size() == 2 && tokens[0] == "*" && tokens[1] == "CONN")) {
                // The following if condition checks for nets without any
                // connections
        ...
        while (valid) {
                ...
                assert(tokens[tokenIndex].size() ==
                       1);  // should be a single character
...
void SpefParser::read_resistances(vector<SpefResistance>& resistances) {
        ...
        while (valid) {
                ...
                // Note that each nodeName can be either a single token or 3
                // tokens
                ...
                if (tokens.size() == 4 && tokens[0] == "*" &&
                    tokens[1] == "D_NET") {
                        ...
                        if (tokens[0] == "*" && tokens[1] == "D_NET")
                                cout << "[BUG] @ SpefParser::read_net_data: "
                                        "possibly wrong-named net starting "
                                        "with '"
                                     << tokens[2] << "'\n";
                }  // end else
...
// If the line read corresponds to a port, then name1 will be set to the port
// name, and
...
bool TimingParser::read_timing_tau15(string& str1, string& str2, double& val1,
...
bool OperationsParser::read_ops_tau15(ISPD13::CommandTypeEnum& cmdType,
                                      ISPD13::QueryTypeEnum& queryType,
                                      string& pin, string& net, string& file,
                                      string& gate, string& cell, bool& early,
                                      bool& rise, int& paths) {
        ...
        if (str1 == "report_at") {
                ...
                while (tokenIndex < tokenSize) {
                        ...
                        if (tmp1 == "-pin") {
                                // read pin char after ':' when it's internal
                                // pin
                                if (tokenIndex == tokenSize - 1 ||
                                    tokenIndex == tokenSize - 3) {
...
void LibParser::_begin_read_timing_info(string toPin, LibParserPinInfo& pin,
        ...
        while (!finishedReading) {
                ...
                if (tokens[0] == "cell_fall") {
                        ...
                        if (setup) _begin_read_lut(pin.fallSetup);
        ...
        while (!finishedReading) {
                ...
                if (tokens[0] == "direction") {
                        ...
                        cell.timingArcs.push_back(
                            LibParserTimingInfo());  // add an empty TimingInfo
                                                     // object
                        _begin_read_timing_info(
                            pinName, pin,
                            cell.timingArcs.back());  // pass the empty object
...
void LibParser::_begin_read_cell_info(string cellName,
        ...
        while (!finishedReading) {
                ...
                if (tokens[0] == "cell_leakage_power") {
                        ...
                        cell.pins.push_back(
                            LibParserPinInfo());  // add empty PinInfo object
                        _begin_read_pin_info(tokens[1], cell,
                                             cell.pins.back());  // pass the new
                                                                 // PinInfo
                                                                 // object to be
                                                                 // filled
...
bool LibParser::read_default_max_transition(double& maxTransition) {
        ...
        while (valid) {
                if (tokens.size() == 2 &&
                    tokens[0] == "default_max_transition") {
...
ostream& operator<<(ostream& os, LibParserCellInfo& cell) {
        ...
        cout << "Cell has " << cell.timingArcs.size()
             << " timing arcs: " << endl;
...
void test_sdc_parser(string filename) {
        ...
        do {
                ...
                if (valid) {
                        cout << "Input port " << portName
                             << " is assumed to be connected to the "
                             << driverPin << " pin of lib cell " << driverSize
                             << endl;
        ...
        while (true) {
                ...
                if (name2 != "") {
                        ...
                        std::cout << name1 << "/" << name2 << " " << riseSlack
                                  << " " << fallSlack << " " << riseTransition
                                  << " " << fallTransition << " " << riseArrival
                                  << " " << fallArrival << endl;

                } else {
                        // timing of a port
                        // name1: port name
                        std::cout << name1 << " " << riseSlack << " "
                                  << fallSlack << " " << riseTransition << " "
                                  << fallTransition << " " << riseArrival << " "
                                  << fallArrival << endl;
                }
...
void test_ceff_parser(string filename) {
        ...
        while (true) {
                ...
                if (name2 != "") {
                        ...
                        // timing of a port
                        // name1: port name
                        std::cout << name1 << " " << riseCeff << " " << fallCeff
                                  << endl;
                }
        }
}
...
void test_lib_parser(string filename) {
        ...
        assert(valid);
        cout << "The default max transition defined is " << maxTransition
             << endl;
