...
bool GuideParser::readLayerGuide(GuideNetDscp& dscp) {
        while (true) {
                std::vector<std::string> tokens;
                readLine(tokens);
                if (isStartLayer(tokens.front())) continue;
                if (isEndLayer(tokens.front())) return true;
                if (tokens.size() < 5) {
                        std::cout
                            << "WARNING: skipping parsing a layer guide of net "
                            << dscp.clsNetName
                            << ". The guide definition has less then four "
                               "points or it do not has defined the layer "
                               "name.\n";
                        continue;
                }  // end if
