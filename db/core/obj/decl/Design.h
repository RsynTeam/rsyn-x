...
class Design : public Proxy<DesignData> {
        ...
        friend class AttributeImplementation;
       ...
       public:
        ...
        //! @brief Copy operator.
        //! @note  Not sure if this is actually necessary. I don't remember why
        //! it
        //!        was created.
       ...
       public:
        ...
        LibraryCell createLibraryCell(const CellDescriptor &dscp,
                                      const bool ignoreDuplicated = false);
       ...
       private:
        ...
        Cell createCell(const Module parent, const LibraryCell lcell,
                        const std::string &name);

        //! @brief Instantiates a new port inside a module.
        Port createPort(Module parent, const Direction direction,
                        const std::string &name);

        //! @brief Creates a new module.
        Module createModule(const LibraryModule lmoudle,
                            const std::string &name);
       ...
       public:
        ...
        ////////////////////////////////////////////////////////////////////////////
       private:
        //! @brief Updates incrementally the topological ordering given a change
        //! in
       ...
       public:
        ...
        void unregisterObserver(DesignObserver *observer);

        //! @brief Notifies about a change in an instance placement. Usually
        //! this
        //! does not need to be done manually, but notification can be turned
        //! off
        ...
        ////////////////////////////////////////////////////////////////////////////
       public:
        //! @brief Finds an instance by name. If the instance is not found,
        //! returns
        //!        null.
        ...
        //! @brief Finds a library cell by name. If the library cell is not
        //! found,
        //!        returns null.
        ...
        Pin findPinByName(const std::string &cellName,
                          const std::string &pinName) const;

        //! @brief Finds a pin by name. If the pin is not found, returns null.
        //! @note  A pin name is in the form <instance>:<pin name>.
        Pin findPinByName(const std::string &name,
       ...
       public:
        ...
        AttributeInitializer createAttribute();

        //! @brief Creates an object attribute with a default value.
        template <typename DefaultValueType>
        AttributeInitializerWithDefaultValue<DefaultValueType> createAttribute(
       ...
       public:
        ...
        Range<ListCollection<LibraryCellData, LibraryCell>> allLibraryCells(
            const bool showDeprecatedMessage = true);
};  // end class
...
class CollectionOfLibraryPinsFilteredByDirection {
       ...
       public:
        CollectionOfLibraryPinsFilteredByDirection(
            std::vector<LibraryPin> &pins, Direction direction)
...
class CollectionOfLibraryArcsToLibraryPin {
       ...
       public:
        CollectionOfLibraryArcsToLibraryPin(const std::vector<LibraryArc> &arcs,
                                            LibraryPin to)
            : arcs(arcs), to(to) {
                index = 0;
        }  // end constructor
...
class CollectionOfLibraryArcsFromLibraryPin {
       ...
       public:
        CollectionOfLibraryArcsFromLibraryPin(
            const std::vector<LibraryArc> &arcs, LibraryPin from)
            : arcs(arcs), from(from) {
                index = 0;
        }  // end constructor
...
class CollectionOfGenericPredecessorPins {
       ...
       public:
        CollectionOfGenericPredecessorPins(PinType pin,
                                           const bool crossBoundary) {
                switch (pin.getDirection()) {
                        case IN: {
                                NetType net = pin.getNet();
                                if (net) {
                                        pins.reserve(net.getNumDrivers());
                                        for (PinType predecessor :
                                             net.allPins(DRIVER)) {
                        ...
                        default:
                                std::cout << "[ERROR] "
                                             "1e1673fccd9b42d225b502a273cb3a20"
                                             "\n";
                                std::exit(1);
                }  // end switch
...
class CollectionOfGenericSuccessorPins {
       ...
       public:
        CollectionOfGenericSuccessorPins(PinType pin,
                                         const bool crossBoundary) {
                switch (pin.getDirection()) {
                        case IN: {
                                if (pin.isPort()) {
                        ...
                        default:
                                std::cout << "[ERROR] "
                                             "44167c065ebd3a23a553fdd96301831e"
                                             "\n";
                                std::exit(1);
                }  // end switch
