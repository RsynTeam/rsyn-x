...
class Pin : public Proxy<PinData> {
       ...
       public:
        ...
        //! @brief Returns the name of the net which this pin connects to if
        //! any.
        ...
        //! @brief Returns true if this pin belongs to a port instance of a
        //! given
        //!        direction.
        bool isPort(const Direction direction) const;

        //! @brief Returns true if this pin belongs to a port that connects to
        //! an
        ...
        //! @brief Returns true if this pin is an output pin and false
        //! otherwise.
        bool isOutput() const;
        ...
        //! @brief Returns the number of arcs that have this pin as the "to"
        //! pin.
        int getNumIncomingArcs() const;

        //! @brief Returns the number of arcs that have this pin as the "from"
        //! pin.
        ...
        Range<CollectionOfPredecessorPins> allPredecessorPins(
            const bool crossBoundaries = false) const;

        //! @brief Returns an iterable collection of all pins that are directly
        //!        connected to this pin via outgoing arcs (i.e. all "to" pins
        //!        of the arcs that have this pin as the "from" pin).
        Range<CollectionOfSuccessorPins> allSucessorPins(
            const bool crossBoundaries = false) const;

        //! @brief Returns an iterable collection of all arcs that have this pin
        //! as
        //!        the "to" pin.
        const std::vector<Arc> &allIncomingArcs() const;

        //! @brief Returns an iterable collection of all arcs that have this pin
        //! as
        //!        the "from" pin.
        const std::vector<Arc> &allOutgoingArcs() const;

        //! @brief Returns an iterable collection of all arcs given a traverse
        //!        direction.
        const std::vector<Arc> &allArcs(const TraverseType direction) const;

        //! @brief Connects this pin to a net. If the pin is currently
        //! connected,
        ...
        //! @brief Returns true if this pin belongs to a non-standard-cell and
        //! false
        //!        otherwise.
        ...
        //!        false otherwise.
        //! @note  This should be true even if the pin is indirectly connected
        //! to
