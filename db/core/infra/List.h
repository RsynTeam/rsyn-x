...
struct ElementPointer {
       private:
        // For some reason the compiler allows ElementPointer to be silently
        // casted
       ...
       public:
        ...
        friend bool operator<(const ElementPointer<T> &left,
                              const ElementPointer<T> &right) {
                // [IMPORTANT] We don't use the pointer (e) directly to avoid
                // non-determinism behavior. Note that the pointer address may
                // change
                // from execution to execution and hence the mapping function
                // may return
...
class List {
       public:
        ...
        typedef std::list<CreateElementCallback>::iterator
            CreateElementCallbackHandler;
        typedef std::list<RemoveElementCallback>::iterator
            RemoveElementCallbackHandler;
        typedef std::list<DestructorCallback>::iterator
            DestructorCallbackHandler;
       ...
       private:
        ...
        // to
        // elements that have not been removed remain valid after the resize and
        // refer
        ...
        // pointers and references remain valid, referring to the same elements
        // they
                ...
                if (recycle != -1) {
                        ...
                        *e = T();
                        ...
                        if (currentChunkFreeSpace == 0) {
                                // Move to the next chunk or creates a new chunk
                                // to store the new
                                // element.
                        ...
                        e = &(c.elements[DEFAULT_CHUNK_SIZE -
                                         currentChunkFreeSpace]);
                        lastInsertedElementId =
                            (currentChunk + 1) * DEFAULT_CHUNK_SIZE -
                            currentChunkFreeSpace;
       ...
       public:
        List()
            : numElements(0),
              ...
              currentChunk(-1),
              currentChunkFreeSpace(0){};  // end constructor
        ...
        int largestId() const {
                return (currentChunk + 1) * DEFAULT_CHUNK_SIZE -
                       currentChunkFreeSpace;
        }
        ...
        int capacity() const {
                ...
                for (auto &c : chunks) capacity += DEFAULT_CHUNK_SIZE;
        ...
        void reserve(const int n) {
                const int numChunks =
                    (int)std::ceil(n / double(DEFAULT_CHUNK_SIZE));
        ...
        Element<T> *get(const int index) {
                return &chunks[index / DEFAULT_CHUNK_SIZE]
                            .elements[index % DEFAULT_CHUNK_SIZE];
        }  // end method
        ...
        DestructorCallbackHandler addDestructorEventCallback(
            DestructorCallback callback) {
                callbackOnDestructor.push_back(callback);
                return --callbackOnDestructor.end();
        }  // end method

        CreateElementCallbackHandler addCreateCallback(
            CreateElementCallback callback) {
                callbackOnCreate.push_back(callback);
                return --callbackOnCreate.end();
        }  // end method

        RemoveElementCallbackHandler addRemoveCallback(
            CreateElementCallback callback) {
        ...
        class Iterator {
               ...
               public:
                Iterator()
                    : l(NULL), e(NULL), currChunk(0), currElementInChunk(0){};
                Iterator(List *l) : l(l), currChunk(0), currElementInChunk(0) {
                        if (!l->isEmpty()) {
                                e = l->get(0);
                                if (e->deleted) {
                                        // Skip deleted elements at the
                ...
                void operator++() { e = nextElement(); }  // end method
        ...
        Iterator end() {
                Iterator end;
                    ...
                    &(chunks[end.currChunk].elements[end.currElementInChunk]);
        ...
        void printState() {
                ...
                std::cout << "> Number of elements: " << numElements
                          << std::endl;
                std::cout << "> Current chunk free elements: "
                          << currentChunkFreeSpace << std::endl;
