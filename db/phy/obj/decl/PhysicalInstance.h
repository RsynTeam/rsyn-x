...
class PhysicalInstance : public Proxy<PhysicalInstanceData> {
       ...
       protected:
        //! @brief Constructs a Rsyn::PhysicalInstance object with a pointer to
        //! Rsyn::PhysicalInstanceData.
        PhysicalInstance(PhysicalInstanceData *data) : Proxy(data) {}

       public:
        //! @brief Constructs a Rsyn::PhysicalInstance object with a null
        //! pointer to Rsyn::PhysicalInstanceData.
        PhysicalInstance() {}
        //! @brief Constructs a Rsyn::PhysicalInstance object with a null
        //! pointer to Rsyn::PhysicalInstanceData.
        ...
        //! @brief Returns the central position of the PhysicalInstance
        //! boundaries for the given dimension.
        ...
        DBU getY() const;
        //! @brief Returns the Instance height. It is the length of
        //! PhysicalInstance boundaries is Y dimension.
        DBU getHeight() const;
        //! @brief Returns the Instance width. It is the length of
        //! PhysicalInstance boundaries is X dimension.
        DBU getWidth() const;
        //! @brief Returns the length of PhysicalInstance boundaries for given
        //! dimension
        DBU getSize(const Dimension dimension) const;
        //! @brief Returns the length of PhysicalInstance boundaries.
        //! In X is the length for abscissa (width) while in Y is the length for
        //! ordinate (height).
        DBUxy getSize() const;
        //! @brief Returns the left-lower PhysicalInstance point that is defined
        //! as its position.
        DBUxy getPosition() const;
        //! @brief Returns the left-lower PhysicalInstance point for the given
        //! dimension
        //! that is defined as its position.
        DBU getPosition(const Dimension dim) const;
        //! @brief Returns the PhysicalInstance point for the given boundary
        //! (LOWER or UPPER).
        DBUxy getCoordinate(const Boundary bound) const;
        //! @brief Returns the PhysicalInstance point for the given boundary
        //! (LOWER or UPPER) and dimension (X or Y).
        ...
        friend bool operator<(const PhysicalInstance &left,
                              const PhysicalInstance &right) {
