...
class PhysicalViaData : public PhysicalObject {
       public:
        ...
        DBU clsEnclosure[NUM_VIA_LEVELS][2];  // bottomXEnclosure,
                                              // bottomYEnclosure,
                                              // topXEnclosure, topYEnclosure
        DBU clsOrigin[2] = {0, 0};            // xOrigin, yOrigin
        DBU clsOffset[NUM_VIA_LEVELS][2];     // bottomXOffset, bottomYOffset,
                                              // topXOffset, topYOffset
        int clsNumRows = 0;
        int clsNumCols = 0;
        float clsCutResistance = 0.0;
        Rsyn::PhysicalLayerData *clsLayers[NUM_VIA_LAYERS];  // bottom, cut, top
        std::vector<Rsyn::PhysicalViaGeometry>
            clsViaGeometries[NUM_VIA_LAYERS];  // bottom, cut, top
        std::string clsName;
