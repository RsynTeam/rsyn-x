...
class PhysicalRoutingWire {
       public:
        ...
        void setSourceExtension(const DBU extension) {
                clsSourceExtension = extension;
        }
        void setTargetExtension(const DBU extension) {
                clsTargetExtension = extension;
        }
       ...
       private:
        DBUxy getExtendedPosition(const DBUxy p0, const DBUxy p1,
...
class PhysicalRouting {
       public:
        ...
        //! @brief Adds a wire.
        void addWire(const PhysicalRoutingWire &wire);

        //! @brief Adds a wire.
        void addWire(Rsyn::PhysicalLayer layer, const DBUxy source,
                     const DBUxy target,
        ...
        //! @brief Adds a wire.
        void addWire(Rsyn::PhysicalLayer layer,
        ...
        //! @brief Adds a via.
        void addVia(const PhysicalRoutingVia &via);

        //! @brief Adds a via.
        void addVia(Rsyn::PhysicalVia via, const DBUxy position);

        //! @brief Adds a patch rectangle.
        void addRect(const PhysicalRoutingRect &rect);

        //! @brief Adds a patch rectangle.
        void addRect(Rsyn::PhysicalLayer layer, const Bounds &rect);

        //! @brief Checks if this physical routing is valid by visiting all
        //! routing
        //! elements (e.g. wires and vias) and checking if they are valid.
        bool isValid() const;

        //! @brief Checks if this physical routing is empty (i.e. no wires,
        //! vias,
        //! etc.)
        bool isEmpty() const;

        //! @brief Iterates over all wires.
        const std::vector<PhysicalRoutingWire> &allWires() const {
                return clsWires;
        }

        //! @brief Iterates over all vias.
        const std::vector<PhysicalRoutingVia> &allVias() const {
                return clsVias;
        }

        //! @brief Iterates over all patch rectangles.
        const std::vector<PhysicalRoutingRect> &allRects() const {
                return clsRects;
        }
