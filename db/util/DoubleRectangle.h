...
class DoubleRectangle {
       ...
       public:
        ...
        DoubleRectangle(const double xmin, const double ymin, const double xmax,
                        const double ymax) {
        ...
        Bounds scaleAndConvertToDbu(
            const double scale, const RoundingStrategy roundingLower,
        ...
        Bounds convertToDbu(const RoundingStrategy roundingLower,
                            const RoundingStrategy roundingUpper) const {
                return Bounds(clsPoints[0].convertToDbu(roundingLower),
                              clsPoints[1].convertToDbu(roundingUpper));
        }  // end method
        ...
        void updatePoints(const double xMin, const double yMin,
        ...
        const double2 &operator[](const int boundary) const {
                return clsPoints[boundary];
        }

        double computeLength(const int dimension) const {
                return (*this)[UPPER][dimension] - (*this)[LOWER][dimension];
        }
        double2 computeLength() const {
                return (*this)[UPPER] - (*this)[LOWER];
        }

        double computeDiagonal() const {
                return std::sqrt(std::pow(computeLength(X), 2.0) +
                                 std::pow(computeLength(Y), 2.0));
        }

        double computeCenter(const int dimension) const {
                return ((*this)[UPPER][dimension] + (*this)[LOWER][dimension]) /
                       2.0;
        }
        double2 computeCenter() const {
                return ((*this)[UPPER] + (*this)[LOWER]) / 2.0;
        }

        double computeArea() const {
                return computeLength(0) * computeLength(1);
        }
        double computeSemiperimeter() const {
                return computeLength(0) + computeLength(1);
        }
        ...
        double randomInnerPoint(const int dimension) const {
                ...
                return (*this)[LOWER][dimension] +
                       random * computeLength(dimension);
        }  // end method

        double overlapArea(const DoubleRectangle &rect) const {
                const double dx = max((*this)[LOWER][X], rect[LOWER][X]) -
                                  min((*this)[UPPER][X], rect[UPPER][X]);
                const double dy = max((*this)[LOWER][Y], rect[LOWER][Y]) -
                                  min((*this)[UPPER][Y], rect[UPPER][Y]);

                return (dx < 0 && dy < 0) ? dx * dy : 0;
        }  // end method

        double getCoordinate(const Boundary bound, const Dimension dim) const {
                return (*this)[bound][dim];
        }
        double2 getCoordinate(const Boundary bound) const {
                return (*this)[bound];
        }
        ...
        // rectangle. If they don't overlap returns a degenerated rectangle
        // (zero
        ...
        DoubleRectangle overlapRectangle(const DoubleRectangle &rect) const {
                ...
                if (dx < 0 && dy < 0) {
                        ...
                        // Returns a zero-area rectangle at half the way of the
                        // two
                        // rectangles.
        ...
        bool between(const double pos, const Dimension DIMENSION) const {
                return (pos >= (*this)[LOWER][DIMENSION] &&
                        pos <= (*this)[UPPER][DIMENSION]);
        }  // end method

        // Check if a point is inside this rectangle.

        bool inside(const double x, const double y) const {
                return (x >= (*this)[LOWER][X] && x <= (*this)[UPPER][X]) &&
        ...
        void moveCenterTo(const double x, const double y) {
                moveCenterTo(double2(x, y));
        }
        void translate(const double x, const double y) {
                translate(double2(x, y));
        }
