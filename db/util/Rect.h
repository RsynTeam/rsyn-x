...
class Rect {
       public:
        ...
        Rect(const DBU x, const DBU y, const DBU w, const DBU h)
            : clsLower(x, y), clsUpper(x + w, y + h) {}
        Rect(const Bounds &bounds)
            : clsLower(bounds.getLower()), clsUpper(bounds.getUpper()) {}
        ...
        bool overlaps(const Rect &rect) const {
                const DBU dx =
                    std::max(getLower().getX(), rect.getLower().getX()) -
                    std::min(getUpper().getX(), rect.getUpper().getX());
                const DBU dy =
                    std::max(getLower().getY(), rect.getLower().getY()) -
