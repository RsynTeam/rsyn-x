...
#define MAKE_SELF_OPERATOR(OP)                       \
        inline void operator OP(const DBUxy right) { \
                x OP right.x;                        \
                y OP right.y;                        \
        }
...
#define MAKE_FUNCTION_1(FUNC) \
        inline DBUxy FUNC(const DBUxy v) { return DBUxy(FUNC(v.x), FUNC(v.y)); }
...
struct DBUxy {
        ...
        explicit DBUxy(const DBU scalar)
            : x(scalar),
              y(scalar) {
        ...
        const DBU &operator[](const int dimension) const {
                return xy[dimension];
        }
        ...
        void scale(const FloatingPointDBU xScaling,
                   const FloatingPointDBU yScaling) {
                x = (DBU)(x * xScaling);
                y = (DBU)(y * yScaling);
        }  // end method
        ...
        DBU aggregated() const { return x + y; }  // end method
...
DBUxy operator*(const T scalar, const DBUxy v) {
        ...
        // Cast must be performed after the multiplication to avoid wrong
        // results
...
template <typename T>
inline T roundedUpIntegralDivision(const T numerator, const T denominator) {
