...
enum RoundingStrategy { ROUND_DOWN, ROUND_UP, ROUND_NEAREST };  // end enum
...
class FloatingPoint {
       public:
        ...
        template <typename T>
        static bool notApproximatelyEqual(const T a, const T b,
                                          const T precision = 1e-6f) {
        ...
        static bool definitelyGreaterEqualThan(const T a, const T b,
                                               const T precision = 1e-6f) {
                return (a - b) >= ((std::abs(a) < std::abs(b) ? std::abs(b)
                                                              : std::abs(a)) *
                                   precision);
        ...
        static bool isUninit(const float value) {
                return isUninit<float>(value);
        }
        static bool isUninit(const double value) {
                return isUninit<double>(value);
        }

        static bool isInfinity(const float value) {
                return isInfinity<float>(value);
        }
        static bool isInfinity(const double value) {
                return isInfinity<double>(value);
        }
        ...
        static long round(const double value,
                          const RoundingStrategy roudingStrategy) {
       ...
       private:
        ...
        inline static R round(const T value,
                              const RoundingStrategy roudingStrategy) {
                switch (roudingStrategy) {
                        case ROUND_DOWN:
                                return (R)std::floor(value);
                        case ROUND_UP:
                                return (R)std::ceil(value);
                        case ROUND_NEAREST:
                                return (R)std::round(value);
                        default:
                                assert(false);
                                return 0;
                }  // end switch
