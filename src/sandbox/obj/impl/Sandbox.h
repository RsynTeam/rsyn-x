...
inline void Sandbox::create(Rsyn::Module module, const std::string &name) {
...
inline void Sandbox::create(Rsyn::Cell seed) {
        ...
        for (Rsyn::Pin pin : seed.allPins()) {
                ...
                if (!net || getRelated(net)) continue;

                createNet(net);
                for (Rsyn::Pin netPin : net.allPins()) {
                        Rsyn::Instance instance = netPin.getInstance();
                        if (getRelated(instance)) continue;

                        createInstance(instance);
                }  // end for
        }          // end for

        // Connect pins in the sandbox according to the design. Pins with no
        // related
        ...
        for (Rsyn::SandboxPin pin : allPins()) {
                ...
                if (!net) {
                        // Dangling pin. Create a virtual port and attach this
                        // pin to it.
                        Rsyn::SandboxPort port =
                            createPort(pin.getDirection(), pin.getFullName());
                        port->attachedPin = pin.data;

                        // Create a net to connect the virtual port and the
                        // attached pin.
...
inline const Design Sandbox::getDesign() const {
        return getModule() ? getModule().getDesign() : nullptr;
}  // end method

// -----------------------------------------------------------------------------

inline Design Sandbox::getDesign() {
        return getModule() ? getModule().getDesign() : nullptr;
}  // end method

// -----------------------------------------------------------------------------

inline const Module Sandbox::getModule() const {
        return data->module;
}  // end method

// -----------------------------------------------------------------------------

inline Module Sandbox::getModule() { return data->module; }  // end method

// -----------------------------------------------------------------------------

inline const std::string &Sandbox::getName() const {
        return data->name;
}  // end method

// -----------------------------------------------------------------------------

inline std::string Sandbox::generateUniqueInstanceName(
...
inline std::string Sandbox::generateUniqueNetName(const std::string &prefix) {
        std::ostringstream oss;
...
inline int Sandbox::generateNextSign() {
        return ++data->sign; /*must be pre-increment*/
}  // end method

// -----------------------------------------------------------------------------

inline int Sandbox::getSign() const { return data->sign; }  // end method

// -----------------------------------------------------------------------------

inline SandboxInstance Sandbox::findInstanceByName(
...
inline SandboxCell Sandbox::findCellByName(const std::string &name) const {
        Rsyn::SandboxInstance instance = findInstanceByName(name);
        if (instance) {
                return instance.getType() == Rsyn::CELL ? instance.asCell()
                                                        : nullptr;
        } else {
                return nullptr;
        }  // end else
...
inline SandboxPort Sandbox::findPortByName(const std::string &name) const {
        Rsyn::SandboxInstance instance = findInstanceByName(name);
        if (instance) {
                return instance.getType() == Rsyn::PORT ? instance.asPort()
                                                        : nullptr;
        } else {
                return nullptr;
        }  // end else
...
inline SandboxNet Sandbox::findNetByName(const std::string &name) const {
...
inline SandboxPin Sandbox::findPinByName(const std::string &cellName,
...
inline SandboxPin Sandbox::findPinByName(
    const std::string &name, const std::string::value_type separator) const {
...
inline Instance Sandbox::getRelated(const SandboxInstance instance) const {
        return instance->related;
}  // end method

// -----------------------------------------------------------------------------

inline SandboxInstance Sandbox::getRelated(const Instance instance) const {
...
inline Net Sandbox::getRelated(const SandboxNet net) const {
        return net->related;
}  // end method

// -----------------------------------------------------------------------------

inline SandboxNet Sandbox::getRelated(const Net net) const {
...
inline SandboxInstance Sandbox::createInstance(const Rsyn::Instance instance) {
        if (instance.getParent() != getModule()) {
                std::cout << "ERROR: Only instances from the same module can "
                             "be created in a sandbox.\n";
                std::exit(1);
        }  // end if
        ...
        switch (instance.getType()) {
                case Rsyn::CELL: {
                        ...
                        sandboxInstance =
                            createCell(cell.getLibraryCell(), cell.getName());
                        break;
                }  // end case
                ...
                case Rsyn::PORT: {
                        ...
                        sandboxInstance =
                            createPort(port.getDirection(), port.getName());
                        break;
                }  // end case

                case Rsyn::MODULE: {
                        std::cout << "ERROR: Modules cannot be created inside "
                                     "a sandbox.\n";
                        std::exit(1);
                        break;
                }  // end case
...
inline SandboxNet Sandbox::createNet(const Rsyn::Net net) {
        if (net.getParent() != getModule()) {
                std::cout << "ERROR: Only nets from the same module can be "
                             "created in a sandbox.\n";
                std::exit(1);
        }  // end if
...
inline SandboxCell Sandbox::createCell(const LibraryCell libraryCell,
                                       const std::string &name) {
        const LibraryCellData *lcell = libraryCell.data;

        const std::string &cellName =
            name == "" ? generateUniqueInstanceName("__cell") : name;

        const int numPins = lcell->pins.size();
        const int numArcs = lcell->arcs.size();

        // Creates a new cell in the data structure.
        SandboxInstanceData *instance =
            &(data->instances.create()->value);  // TODO: awful
        ...
        for (int i = 0; i < numPins; i++) {
                SandboxPinData *pin =
                    &(data->pins.create()->value);  // TODO: awful
                ...
                switch (pin->direction) {
                        ...
                        case Rsyn::BIDIRECTIONAL:
                                throw Exception(
                                    "Rsyn does not support bidirectional pins "
                                    "yet.\n");
                        default:
        ...
        for (int i = 0; i < numArcs; i++) {
                SandboxArcData *arc =
                    &(data->arcs.create()->value);  // TODO: awful
...
inline SandboxPort Sandbox::createPort(const Direction direction,
                                       const std::string &name) {
...
inline void Sandbox::connectPin(SandboxPin pin, SandboxNet net) {
        if (pin->net) {
                disconnectPin(pin);
        }  // end if
        ...
        switch (pin.getDirection()) {
                ...
                case Rsyn::SINK: {
                        for (Rsyn::SandboxPin driver :
                             net.allPins(Rsyn::DRIVER)) {
                                SandboxArc arc = &(
                                    data->arcs.create()->value);  // TODO: awful
                                arc->id = data->arcs.lastId();
                                arc->type = NET_ARC;
...
inline void Sandbox::disconnectPin(SandboxPin pin) {
        if (pin->net) {
                ...
                for (int i = 0; i < numPins; i++) {
                        if (net->pins[i] == pin) {
                                // Swap the element to be delete with the last
                                // one and then shrink
                                // the vector. Note that is affect the pin
                                // ordering inside the net.
...
inline void Sandbox::remap(SandboxCell cell, LibraryCell newLibraryCell) {
        ...
        for (LibraryArc oldLibraryArc : oldLibraryCell.allLibraryArcs()) {
                LibraryArc newLibraryArc =
                    newLibraryCell.getLibraryArcByPinNames(
                        oldLibraryArc.getFromName(), oldLibraryArc.getToName());
        ...
        for (SandboxPin pin : cell.allPins()) {
                Rsyn::LibraryPin newLibraryPin =
                    mapLibraryPins[oldLibraryCell.getLibraryPinByIndex(
                        pin->index)];
        ...
        for (SandboxArc arc : cell.allArcs()) {
                Rsyn::LibraryArc newLibraryArc =
                    mapLibraryArcs[arc.getLibraryArc()];
...
inline void Sandbox::remap(SandboxCell cell,
                           const std::string &newLibraryCellName) {
        LibraryCell newLibraryCell =
            getDesign().findLibraryCellByName(newLibraryCellName);
        if (!newLibraryCell) {
...
inline int Sandbox::getNumInstances() const {
        return data->instances.size();
}  // end method

// -----------------------------------------------------------------------------

inline int Sandbox::getNumNets() const {
        return data->nets.size();
}  // end method

// -----------------------------------------------------------------------------

inline int Sandbox::getNumPins() const {
...
inline Index Sandbox::getId(SandboxInstance instance) const {
        return instance->id;
}
...
inline SandboxAttributeInitializer Sandbox::createAttribute() {
...
inline SandboxAttributeInitializerWithDefaultValue<DefaultValueType>
Sandbox::createAttribute(const DefaultValueType &defaultValue) {
        return SandboxAttributeInitializerWithDefaultValue<DefaultValueType>(
            *this, defaultValue);
}  // end method
...
inline void Sandbox::updateTopologicalIndex(SandboxPin pin) {
        // Some checks...
        static_assert(std::is_integral<TopologicalIndex>::value,
                      "Integer required.");
        static_assert(std::is_signed<TopologicalIndex>::value,
        ...
        static_assert(
            TOPOLOGICAL_SORTING_SMALL_GAP <= TOPOLOGICAL_SORTING_LARGE_GAP,
            "small gap > large gap");
        ...
        if (!hasLower && !hasUpper) {
                ...
                if (lower < upper && (upper - lower >= 2)) {
                        // The easy case...
                        // Since we use integral types, we must guarantee that
                        // there's room
                        // for the new pin between the lower and upper bounds,
                        // that's why
                        // we check if (upper - lower >= 2).
                        pin->order = (lower + upper) / 2;
                } else {
                        // We need to propagate the indexes...
                        const TopologicalIndex smallGap =
                            TOPOLOGICAL_SORTING_SMALL_GAP;
                        const TopologicalIndex largeGap =
                            TOPOLOGICAL_SORTING_LARGE_GAP;
                        ...
                        std::queue<
                            std::tuple<Rsyn::SandboxPin, TopologicalIndex>>
                            open;
                        for (Rsyn::SandboxPin successor :
                             pin.allSucessorPins(true)) {
                                if (successor->order < right) {
                                        open.push(std::make_tuple(successor,
                                                                  pin->order));
                                }  // end if
                        }          // end for

                        while (!open.empty()) {
                                Rsyn::SandboxPin current =
                                    std::get<0>(open.front());
                                const TopologicalIndex generatorOrder =
                                    std::get<1>(open.front());
                                open.pop();
                                ...
                                if (current->sign != sign) {
                                        ...
                                        order = (TopologicalIndex)std::floor(
                                            (float((current->order - upper) *
                                                   w1) /
                                             float(w0)) +
                                            left1);
                                        if (order <= generatorOrder) {
                                                order =
                                                    generatorOrder + smallGap;
                                        }  // end if
                                } else {
...
inline Range<SandboxListCollection<SandboxInstanceData, SandboxInstance>>
Sandbox::allInstances() {
        return SandboxListCollection<SandboxInstanceData, SandboxInstance>(
            data->instances);
}  // end method

// -----------------------------------------------------------------------------

inline Range<SandboxReferenceListCollection<SandboxPort>> Sandbox::allPorts() {
        return SandboxReferenceListCollection<SandboxPort>(data->ports);
}  // end method

// -----------------------------------------------------------------------------

inline std::set<SandboxPort> &Sandbox::allPorts(
...
inline Range<SandboxListCollection<SandboxNetData, SandboxNet>>
Sandbox::allNets() {
...
inline Range<SandboxListCollection<SandboxPinData, SandboxPin>>
Sandbox::allPins() {
...
inline Range<SandboxListCollection<SandboxArcData, SandboxArc>>
Sandbox::allArcs() {
...
inline std::vector<TupleElement<1, TopologicalIndex, SandboxNet>>
...
inline std::vector<TupleElement<1, TopologicalIndex, SandboxInstance>>
Sandbox::allInstancesInTopologicalOrder() {
        ...
        for (Rsyn::SandboxInstance instance : allInstances()) {
                sortedInstances[i++] =
                    std::make_tuple(instance.getTopologicalIndex(), instance);
        }  // end for
...
inline std::vector<Rsyn::SandboxNet>
Sandbox::getFanoutConeNetsInBreadthFirstOrder(Rsyn::SandboxPin seed) {
        ...
        while (!open.empty()) {
                ...
                if (currentNet->sign == sign) continue;
                ...
                for (Rsyn::SandboxPin sink : currentNet.allPins(SINK)) {
                        for (Rsyn::SandboxArc arc : sink.allOutgoingArcs()) {
                                ...
                                if (net && net->sign != sign) open.push(net);
                        }  // end for
                }          // end for
        }                  // end while
