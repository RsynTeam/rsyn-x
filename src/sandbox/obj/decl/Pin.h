...
class SandboxPin : public Proxy<SandboxPinData> {
       ...
       public:
        ...
        Range<CollectionOfPredecessorSandboxPins> allPredecessorPins(
            const bool crossBoundaries = false) const;

        Range<CollectionOfSuccessorSandboxPins> allSucessorPins(
            const bool crossBoundaries = false) const;

        const std::vector<SandboxArc> &allIncomingArcs()
            const;  // allArcsToPin(Pin pin) const;

        const std::vector<SandboxArc> &allOutgoingArcs()
            const;  // allArcsFromPin(Rsyn::Pin pin) const;
