...
class LibraryPinNotFoundException : public Exception {
       public:
        LibraryPinNotFoundException(const std::string &libCellName,
                                    const std::string &libPinName)
            : Exception("Library pin '" + libPinName +
                        "' not found in library cell '" + libCellName + "'.") {}
};  // end class
...
class PinNotFoundException : public Exception {
       public:
        PinNotFoundException(const std::string &libCellName,
                             const std::string &cellName,
                             const std::string &pinName)
            : Exception("Pin '" + pinName + "' not found in cell '" + cellName +
                        "'(" + libCellName + ").") {}
};  // end class
...
class IncompatibleLibraryCellForRemapping : public Exception {
       public:
        IncompatibleLibraryCellForRemapping(const std::string &oldLibraryCell,
                                            const std::string &newLibraryCell)
            : Exception("Library cell '" + newLibraryCell +
                        "' is not compatible to library cell '" +
                        oldLibraryCell + "'.") {}
};  // end class
