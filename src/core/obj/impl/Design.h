...
inline Design::Design(const std::string &name) {
        create(name);
}  // end constructor

// -----------------------------------------------------------------------------

inline void Design::create(const std::string &name) {
...
inline Module Design::getTopModule() { return data->topModule; }  // end method
...
inline void Design::updateName(std::string name) {
        data->name = name;
}  // end method

// -----------------------------------------------------------------------------

inline std::string Design::generateUniqueInstanceName(
...
inline std::string Design::generateUniqueNetName(const std::string &prefix) {
        std::ostringstream oss;
...
inline int Design::generateNextSign() {
        return ++data->sign; /*must be pre-increment*/
}  // end method

// -----------------------------------------------------------------------------

inline int Design::getSign() const { return data->sign; }  // end method

// -----------------------------------------------------------------------------

inline Instance Design::findInstanceByName(const std::string &name) const {
...
inline Cell Design::findCellByName(const std::string &name) const {
...
inline Port Design::findPortByName(const std::string &name) const {
        Rsyn::Instance instance = findInstanceByName(name);
        if (instance) {
...
inline Module Design::findModuleByName(const std::string &name) const {
        Rsyn::Instance instance = findInstanceByName(name);
        if (instance) {
                return instance.getType() == Rsyn::MODULE ? instance.asModule()
                                                          : nullptr;
        } else {
                return nullptr;
        }  // end else
...
inline LibraryCell Design::findLibraryCellByName(
...
inline Net Design::findNetByName(const std::string &name) const {
...
inline Pin Design::findPinByName(const std::string &cellName,
...
inline Pin Design::findPinByName(
...
inline LibraryCell Design::createLibraryCell(const CellDescriptor &dscp,
                                             const bool ignoreDuplicated) {
        ...
        LibraryCellData *lcell =
            &(data->libraryCells.create()->value);  // TODO: awful
        ...
        for (auto element : mapping) {
                ...
                const std::tuple<std::string, Direction, Use> &t =
                    dscp.pins[index];

                LibraryPinData *lpin =
                    &(data->libraryPins.create()->value);  // TODO: awful
        ...
        for (int i = 0; i < numArcs; i++) {
                LibraryArcData *larc =
                    &(data->libraryArcs.create()->value);  // TODO: awful
                larc->id = data->libraryArcs.lastId();
                larc->design = *this;
                larc->lcell = libraryCell;
                larc->index = i;
                larc->from =
                    libraryCell.getLibraryPinByName(std::get<0>(dscp.arcs[i]));
                larc->to =
                    libraryCell.getLibraryPinByName(std::get<1>(dscp.arcs[i]));
...
inline Cell Design::createCell(const Module parent,
                               const LibraryCell libraryCell,
...
inline Port Design::createPort(Module parent, const Direction direction,
                               const std::string &name) {
...
inline Module Design::createModule(const LibraryModule lmoudle,
                                   const std::string &name) {
        // Creates a new cell in the data structure.
        InstanceData *instance =
            &(data->instances.create()->value);  // TODO: awful
...
inline Net Design::createNet(const Module parent, const std::string &name) {
...
inline void Design::connectPin(Pin pin, Net net) {
        if (pin->net) {
                disconnectPin(pin);
        }  // end if
...
inline void Design::disconnectPin(Pin pin) {
        // Notify observers.
...
inline void Design::remap(Cell cell, LibraryCell newLibraryCell) {
...
inline void Design::remap(Cell cell, const std::string &newLibraryCellName) {
...
inline int Design::getNumInstances() const {
        return data->instances.size();
}  // end method

// -----------------------------------------------------------------------------

inline Range<ReferenceListCollection<Cell>> Design::getAllCells() const {
        for (Instance inst : data->topModule.allInstances()) {
                if (inst.getType() != CELL) continue;
                data->cells.add(inst.asCell());
        }
        Range<ReferenceListCollection<Cell>> a =
            ReferenceListCollection<Cell>(data->cells);
        for (Cell c : a) {
                std::cout << c.getName() << "\n";
                break;
        }
...
inline int Design::getNumInstances(const InstanceType type) const {
        return data->instanceCount[type];
}  // end method

// -----------------------------------------------------------------------------

inline int Design::getNumNets() const {
        return data->nets.size();
}  // end method

// -----------------------------------------------------------------------------

inline int Design::getNumPins() const {
...
inline void Design::updateTopologicalIndex(Pin pin) {
        // Some checks...
        ...
        if (!hasLower && !hasUpper) {
                ...
                if (lower < upper && (upper - lower >= 2)) {
                        ...
                        std::queue<std::tuple<Rsyn::Pin, TopologicalIndex>>
                            open;
                        for (Rsyn::Pin successor : pin.allSucessorPins(true)) {
...
template <class T>
inline void Design::registerObserver(T *observer) {
        ...
        if (typeid(&DesignObserver::onDesignDestruction) !=
            typeid(&T::onDesignDestruction)) {
                data->observers[EVENT_DESTRUCTION].push_back(observer);
        }  // end if

        if (typeid(&DesignObserver::onPostInstanceCreate) !=
            typeid(&T::onPostInstanceCreate)) {
                data->observers[EVENT_POST_INSTANCE_CREATE].push_back(observer);
        }  // end if

        if (typeid(&DesignObserver::onPreInstanceRemove) !=
            typeid(&T::onPreInstanceRemove)) {
                data->observers[EVENT_PRE_INSTANCE_REMOVE].push_back(observer);
        }  // end if

        if (typeid(&DesignObserver::onPostNetCreate) !=
            typeid(&T::onPostNetCreate)) {
                data->observers[EVENT_POST_NET_CREATE].push_back(observer);
        }  // end if

        if (typeid(&DesignObserver::onPreNetRemove) !=
            typeid(&T::onPreNetRemove)) {
                data->observers[EVENT_PRE_NET_REMOVE].push_back(observer);
        }  // end if

        if (typeid(&DesignObserver::onPostCellRemap) !=
            typeid(&T::onPostCellRemap)) {
                data->observers[EVENT_POST_CELL_REMAP].push_back(observer);
        }  // end if

        if (typeid(&DesignObserver::onPostPinConnect) !=
            typeid(&T::onPostPinConnect)) {
                data->observers[EVENT_POST_PIN_CONNECT].push_back(observer);
        }  // end if

        if (typeid(&DesignObserver::onPrePinDisconnect) !=
            typeid(&T::onPrePinDisconnect)) {
                data->observers[EVENT_PRE_PIN_DISCONNECT].push_back(observer);
        }  // end if

        if (typeid(&DesignObserver::onPostInstanceMove) !=
            typeid(&T::onPostInstanceMove)) {
...
inline void Design::unregisterObserver(DesignObserver *observer) {
...
inline void Design::notifyInstancePlaced(Rsyn::Instance instance,
                                         Rsyn::DesignObserver *ignoreObserver) {
        for (DesignObserver *observer :
             data->observers[EVENT_POST_INSTANCE_MOVE]) {
...
inline AttributeInitializer Design::createAttribute() {
...
inline AttributeInitializerWithDefaultValue<DefaultValueType>
Design::createAttribute(const DefaultValueType &defaultValue) {
        return AttributeInitializerWithDefaultValue<DefaultValueType>(
            *this, defaultValue);
}  // end method
...
inline NetTag Design::getTag(Rsyn::Net net) {
        return NetTag(&net->tag);
}  // end method

// -----------------------------------------------------------------------------

inline InstanceTag Design::getTag(Rsyn::Instance instance) {
        return InstanceTag(&instance->tag);
}  // end method

// -----------------------------------------------------------------------------

inline LibraryCellTag Design::getTag(Rsyn::LibraryCell libraryCell) {
...
Design::allLibraryCells(const bool showDeprecatedMessage) {
        if (showDeprecatedMessage) {
                std::cout << "WARNING: Rsyn::Desing::allLibraryCells() is "
                             "deprecated. "
