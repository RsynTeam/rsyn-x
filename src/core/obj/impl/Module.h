...
inline Design Module::getDesign() {
        return data->moduleData->design;
}  // end method

// -----------------------------------------------------------------------------

inline const Design Module::getDesign() const {
        return data->moduleData->design;
}  // end method

// -----------------------------------------------------------------------------

inline int Module::generateNextSign() const {
        return ++data->moduleData->sign; /*must be pre-increment*/
}  // end method

// -----------------------------------------------------------------------------

inline int Module::getSign() const {
        return data->moduleData->sign;
}  // end method

// -----------------------------------------------------------------------------

inline Cell Module::createCell(const std::string &libraryCellName,
...
inline Cell Module::createCell(const LibraryCell lcell,
                               const std::string &name) {
        Design design = getDesign();
        return design.createCell(*this, lcell, name);
}  // end method
...
inline Port Module::createPort(const Direction &direction,
                               const std::string &name) {
        Design design = getDesign();
...
inline Net Module::createNet(const std::string &name) {
        Design design = getDesign();
...
inline Port Module::getPortByIndex(const int index) {
        return data->moduleData->ports.get(index)->value;  // TODO: awful
}  // end method

// -----------------------------------------------------------------------------

inline void Module::getInstancesPerLogicalDepth(
    std::vector<std::vector<Rsyn::Instance>> &levels) {
        ...
        Rsyn::Attribute<Rsyn::Instance, int> depth =
            getDesign().createAttribute(-1);
        for (Rsyn::Instance instance : allInstancesInTopologicalOrder()) {
                // Set the depth as the largest depth of the driver cells (if
                // any) plus
                // one.
                int lower = -1;
                ...
                switch (instance.getType()) {
                        case Rsyn::CELL: {
                                // We check the from pin of arcs instead of just
                                // checking the
                                // input pins to handle the D pin of registers.
                                // We want that
                                // the depth of a register is based on the depth
                                // of its CK pin.
                                ...
                                // If no arcs, as a fall back, get the depth
                                // from the input
                                // pins.
                                if (!hasArcs) {
                                        for (Rsyn::Pin pin :
...
inline void Module::getNetsPerLogicalDepth(
...
inline int Module::getNumPorts(const Direction direction) const {
        return data->moduleData->portsByDirection[direction].size();
}  // end method

// -----------------------------------------------------------------------------

inline Range<ReferenceListCollection<Instance>> Module::allInstances() const {
        return ReferenceListCollection<Instance>(data->moduleData->instances);
}  // end method

// -----------------------------------------------------------------------------

inline Range<ReferenceListCollection<Port>> Module::allPorts() const {
        return ReferenceListCollection<Port>(data->moduleData->ports);
}  // end method

// -----------------------------------------------------------------------------

inline std::set<Port> &Module::allPorts(const Rsyn::Direction direction) const {
        return data->moduleData->portsByDirection[direction];
}  // end method

// -----------------------------------------------------------------------------

inline Range<ReferenceListCollection<Net>> Module::allNets() const {
        return ReferenceListCollection<Net>(data->moduleData->nets);
}  // end method

// -----------------------------------------------------------------------------

inline Range<CollectionOfPinsFilteredByDirection> Module::allInterfacePins(
...
inline Range<CollectionOfPins> Module::allInterfacePins() const {
        return Instance::allPins();
}  // end method

// -----------------------------------------------------------------------------

inline Range<CollectionOfArcs> Module::allInterfaceArcs() const {
        return Instance::allArcs();
}  // end method

// -----------------------------------------------------------------------------

inline std::vector<TupleElement<1, TopologicalIndex, Pin>>
Module::allPinsInTopologicalOrder() const {
        ...
        for (Rsyn::Instance instance : allInstances()) {
                for (Rsyn::Pin pin : instance.allPins())
                        sortedPins.push_back(
                            std::make_tuple(pin.getTopologicalIndex(), pin));
        }  // end for
...
inline std::vector<TupleElement<1, TopologicalIndex, Net>>
...
inline std::vector<TupleElement<1, TopologicalIndex, Instance>>
...
inline std::vector<Rsyn::Net> Module::getFanoutConeNetsInBreadthFirstOrder(
    Rsyn::Pin seed) const {
...
inline std::vector<Rsyn::Net> Module::getFaninConeNetsInBreadthFirstOrder(
    Rsyn::Pin seed) const {
        std::vector<Rsyn::Net> result;
        std::queue<Rsyn::Net> open;
