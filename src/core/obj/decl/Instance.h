...
class Instance : public Proxy<InstanceData> {
        ...
        // Temporary while we merge some physical data into the netlist
        // (2018/01/17).
       ...
       public:
        ...
        //! @brief Returns the parent module in which this instance is
        //! instantiated.
        Module getParent();
        ...
        //! @brief Returns a pin by its name. If no pin matches, null is
        //! returned.
        Pin getPinByName(const std::string &name) const;

        //! @brief Returns any input pin. Useful when handling
        //! buffers/inverters,
        //!        which have only one input pin.
        Pin getAnyInputPin() const;

        //! @brief Returns any output pin. Useful when handling
        //! buffers/inverters,
        //!        which have only one output pin.
        Pin getAnyOutputPin() const;

        //! @brief Returns the arc from the "from" pin to the "to" pin. Returns
        //! null
        //!        if the pins do not belong to this instance or if no such arc
        //!        is
        //!        found.
        ...
        //! @brief Returns the arc from the "from" pin to the "to" pin. Returns
        //! null
        //!        if either pin is not found or if no such arc exist between
        //!        the
        //!        pins.
        Arc getArcByPinNames(const std::string &from, const std::string &to);

        //! @brief Returns any arc. Useful when handling buffers/inverters,
        //! which
        //!        have only one arc.
        Arc getAnyArc();
        ...
        //! @note  Since the topological index of an instance may be ambiguous
        //! (e.g.
        //!        for register), some choice needed to be made here. Currently,
        //!        the topological index of an instance is the maximum
        //!        topological
        //!        index of the "from" pins of its arcs. If no arcs, the
        //!        topological
        //!        index is set to the maximum topological index of all pins.
        //!        In this way, the topological index of registers is given by
        //!        their
        ...
        //! given
        //!        direction.
        //! @note  The name of this method may be confusing for modules where
        //! one
        //!        may expect the collection of all pins inside the module.
        //!        However
        //!        to be consistent, only the interface pins are returned that
        //!        is
        //!        the outer pins of the module's ports.
        Range<CollectionOfPinsFilteredByDirection> allPins(
            const Direction direction, bool filterPG = true) const;

        //! @brief Returns an iterable collection of all interface pins.
        //! @see allPins(const Direction direction)
        Range<CollectionOfPins> allPins(bool filterPG = true) const;

        //! @brief Returns an iterable collection of all arcs.
        Range<CollectionOfArcs> allArcs() const;

        //! @brief Returns true if this instance represents a sequential cell
        //! and
        //!        false otherwise.
        //! @note  This is a user-defined flag. If the flag was not set, an
        //! exception
        //!        is raised.
        ...
        //! @brief Returns true if this instance represents a buffer cell of a
        //! given
        ...
        //! exception
        //!        is raised.
        bool isClockBuffer(const BufferType type = ANY_BUFFER_TYPE,
                           const bool local = false) const;

        //! @brief Returns true if this instance represents a local clock buffer
        //! of
        ...
        //! @brief Returns true if this instance can be moved and false
        //! otherwise.
        //! @see   isFixed()
        //! @note  This is a user-defined flag. If the flag was not set, an
        //! exception
        ...
        //! @brief Returns true if this instance represents a non-standard-cell
        //! and
        //!        false otherwise.
        //! @note  This is a user-defined flag. If the flag was not set, an
        //! exception
        ...
        //! @note  X is the length for abscissa (width) while in Y is the length
        //! for
        //!        ordinate (height).
        DBUxy getSize() const;
        ...
        //! transformation accounts for translation and the orientation of the
        //! cell.
        //! @param origin If set to true, the transform is created w.r.t (0, 0)
        //! and
        //! not the current cell position.
        PhysicalTransform getTransform(const bool origin = false) const;

        //! @brief Returns the bound box Bounds that defines the limits of
        //! PhysicalInstance.
