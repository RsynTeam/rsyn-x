...
class CellDescriptor {
       ...
       public:
        ...
        const std::string &getName() const { return name; }  // end method
        ...
        void checkConsistency() const {
                ...
                for (int i = 0; i < numPins; i++) {
                        if ((std::get<1>(pins[i]) != Rsyn::IN) &&
                            (std::get<1>(pins[i]) != Rsyn::OUT)) {
                                throw Exception(
                                    "Only in and out direction are supported "
                                    "right now.");
                        }  // end if
                        ...
                        mapPinToDirection[std::get<0>(pins[i])] =
                            std::get<1>(pins[i]);
                }  // end for
