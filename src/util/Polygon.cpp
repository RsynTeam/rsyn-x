...
void Polygon::translate(const DBU dx, const DBU dy) {
...
Polygon Polygon::translated(const DBU dx, const DBU dy) const {
        Polygon poly = *this;
...
Polygon Polygon::createFromLineString(const std::vector<DBUxy> &points,
                                      const DBU width) {
        Polygon rsynPolygon;
        ...
        for (const polygon &poly : result) {
                for (const point &p : poly.outer()) {
                        rsynPolygon.addPoint(
                            DBUxy(static_cast<DBU>(std::round(p.x())),
                                  static_cast<DBU>(std::round(p.y()))));
                }  // end for

                // Defensive programming: this operation should not return more
                // than one
                // polygon.
                break;
        }  // end for
...
bool findPoint(float2 v0, float2 v1, float2 v2, const float thickness,
        ...
        float m1 =
            (v0.x - v1.x) == 0 ? (v0.y - v1.y) : (v0.y - v1.y) / (v0.x - v1.x);
        float m2 =
            (v1.x - v2.x) == 0 ? (v1.y - v2.y) : (v1.y - v2.y) / (v1.x - v2.x);

        float tg = (m1 - m2) / (1 + m1 * m2);
        float distancia = std::sqrt(std::pow((p1.x - p2.x), 2.0f) +
                                    std::pow((p1.y - p2.y), 2.0f));
...
void tracePathOutline(const std::vector<DBUxy> &pathPoints,
                      const float halfThickness,
                      std::vector<float2> &outlinePoints) {
        // Historical Note: This function was adapted from wxSightGL developed
        // by
        ...
        v1 = float2(pathPoints[0]) +
             (float2(pathPoints[1]) - float2(pathPoints[0]))
                     .perpendicular()
                     .normalized() *
                 halfThickness;
        ...
        for (size_t i = 1; i < pathPoints.size() - 1; i++) {
                control = findPoint(
                    float2(pathPoints[i - 1]), float2(pathPoints[i]),
                    float2(pathPoints[i + 1]), halfThickness, outlinePoints);
        }  // end for

        // Control point.
        if (control) {
                v1 = float2(pathPoints.back()) +
                     float2(float2(pathPoints.back()) -
                            float2(pathPoints[pathPoints.size() - 2]))
                             .perpendicular()
                             .normalized() *
                         halfThickness;
                outlinePoints.push_back(v1);
        ...
        // Last point.
        v1 = float2(pathPoints.back()) +
             float2(float2(pathPoints[pathPoints.size() - 2]) -
                    float2(pathPoints.back()))
                     .perpendicular()
                     .normalized() *
                 halfThickness;
        outlinePoints.push_back(v1);
        ...
        for (int i = pathPoints.size() - 2; i >= 1; i--) {
                findPoint(float2(pathPoints[i + 1]), float2(pathPoints[i]),
                          float2(pathPoints[i - 1]), halfThickness,
                          outlinePoints);
        }  // end if

        v1 = float2(pathPoints[0]) +
             float2(float2(pathPoints[0]) - float2(pathPoints[1]))
                     .perpendicular()
                     .normalized() *
                 halfThickness;
        outlinePoints.push_back(v1);
        v1 = float2(pathPoints[0]) +
             float2(float2(pathPoints[1]) - float2(pathPoints[0]))
                     .perpendicular()
                     .normalized() *
                 halfThickness;
        outlinePoints.push_back(v1);
...
Polygon Polygon::createFromLineString(const std::vector<DBUxy> &points,
        ...
        for (const float2 &p : outline) {
                rsynPolygon.addPoint(DBUxy(static_cast<DBU>(std::round(p.x)),
                                           static_cast<DBU>(std::round(p.y))));
        }  // end for
