...
enum Measure {
        ...
        MEASURE_TIME,         // ps     Note: Time(ps) = Resistance(kOhm) *
                              // Capacitance(fF)
...
class Units {
       public:
        ...
        //! @brief Returns the default unit string given a measure. For
        //! instance,
        //!        return "fC" for capacitance and "mW" for power.
        static std::string getDefaultInternalUnitString(const Measure measure) {
                return getUnitPrefixSymbol(getInternalUnitPrefix(measure)) +
                       getMeasureSymbol(measure);
        }  // end method

        static double convertUnits(const double sourceValue,
                                   const UnitPrefix sourceUnitPrefix,
                                   const UnitPrefix targetUnitPrefix) {
                return sourceValue *
                       std::pow(10, sourceUnitPrefix - targetUnitPrefix);
        }  // end method

        static double convertToInternalUnits(
            const Measure measure, const double sourceValue,
            const UnitPrefix sourceUnitPrefix) {
                const UnitPrefix targetUnitPrefix =
                    getInternalUnitPrefix(measure);
                return convertUnits(sourceValue, sourceUnitPrefix,
                                    targetUnitPrefix);
        }  // end method
