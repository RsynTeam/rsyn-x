...
class Polygon {
       public:
        ...
        static Polygon createFromLineString(const std::vector<DBUxy> &points,
                                            const DBU width);

        //! @brief Returns true if this a non-empty polygon.
        bool isValid() const { return getNumPoints() > 0; }  // end method
        ...
        //! @brief Returns the shortest distance from a rectangle to this
        //! polygon.
        ...
        FloatingPointDBU distance(const Polygon &poly) const {
                correct();
                return boost::geometry::distance(poly.clsBoostPolygon,
                                                 clsBoostPolygon);
        }  // end method
        ...
        bool overlaps(const Rect &rect) const {
                ...
                // directly so we first convert the rectangle to an equivalent
                // polygon.
                return boost::geometry::overlaps(
                    rect.toPolygon().clsBoostPolygon, clsBoostPolygon);
        }  // end method

        //! @brief Returns true if a polygon to overlaps this polygon.
        bool overlaps(const Polygon &poly) const {
                correct();
                return boost::geometry::overlaps(poly.clsBoostPolygon,
                                                 clsBoostPolygon);
        }  // end method
        ...
        //! inserted as the points are corrected to be in the order
        //! boost::geometry
        //! assumes they to be.
       ...
       private:
        ...
        typedef boost::geometry::model::polygon<
            Rsyn::Point, false /*clock-wise*/, false /*closed*/>
            BoostPolygon;

        //! @brief Corrects the order of points in this polygon.
        //! @note This is necessary to comply with boost::geometry, which
        //! assumes
        //! the points of polygons are given in a specific order (e.g.
        //! clockwise).
        void correct() const {
                if (clsDirty) {
        ...
        //! @brief Indicates that this polygon may need correction as boost
        //! requires
