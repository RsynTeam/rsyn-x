...
class AsciiProgressBar {
       private:
        ...
        void draw() {
                const int percentage =
                    clsMaxValue > 0 ? (100 * clsCurrentValue) / clsMaxValue
                                    : 100;
       ...
       public:
        AsciiProgressBar() : N(DEFAULT_N) { reset(); }  // end constructor

        AsciiProgressBar(const int maxValue, const int numColumns = DEFAULT_N)
            : N(numColumns) {
                reset(maxValue);
        }  // end constructor
        ...
        void progress(const int increment = 1) {
                clsCurrentValue =
                    std::min(clsCurrentValue + increment, clsMaxValue);
