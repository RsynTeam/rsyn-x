...
class Bounds {
       ...
       public:
        ...
        void updatePoints(const DBU xMin, const DBU yMin, const DBU xMax,
                          const DBU yMax) {
                (*this)[LOWER][X] = xMin;
                (*this)[LOWER][Y] = yMin;
                (*this)[UPPER][X] = xMax;
        ...
        const DBUxy &operator[](const int boundary) const {
                return clsPoints[boundary];
        }

        DBU computeLength(const int dimension) const {
                return (*this)[UPPER][dimension] - (*this)[LOWER][dimension];
        }
        DBUxy computeLength() const { return (*this)[UPPER] - (*this)[LOWER]; }

        FloatingPointDBU computeDiagonal() const {
                return std::sqrt(std::pow(computeLength(X), 2) +
                                 std::pow(computeLength(Y), 2));
        }

        DBU computeCenter(const int dimension) const {
                return ((*this)[UPPER][dimension] + (*this)[LOWER][dimension]) /
                       2;
        }
        DBUxy computeCenter() const {
                return ((*this)[UPPER] + (*this)[LOWER]) / 2;
        }

        DBU computeArea() const { return computeLength(0) * computeLength(1); }
        DBU computeSemiperimeter() const {
        ...
        DBU randomInnerPoint(const int dimension) const {
                ...
                return (DBU)((*this)[LOWER][dimension] +
                             random * computeLength(dimension));
        }  // end method

        DBU overlapArea(const Bounds &rect) const {
                const DBU dx = std::max((*this)[LOWER][X], rect[LOWER][X]) -
                               std::min((*this)[UPPER][X], rect[UPPER][X]);
                const DBU dy = std::max((*this)[LOWER][Y], rect[LOWER][Y]) -
                               std::min((*this)[UPPER][Y], rect[UPPER][Y]);
        ...
        // Returns the dimension length overlap of this and other rectangle.
        // Otherwise, returns zero
        DBU overlapDimensionLength(const Bounds &rect,
                                   const Dimension dim) const {
                const DBU delta =
                    std::max((*this)[LOWER][dim], rect[LOWER][dim]) -
                    std::min((*this)[UPPER][dim], rect[UPPER][dim]);
        ...
        void moveCenterTo(const DBU x, const DBU y) {
                moveCenterTo(DBUxy(x, y));
        }
        ...
        void scaleCentralized(const FloatingPointDBU xFactor,
                              const FloatingPointDBU yFactor) {
        ...
        void setLength(const Dimension DIMENSION, const DBU length) {
                clsPoints[UPPER][DIMENSION] =
                    clsPoints[LOWER][DIMENSION] + length;
        }  // end method
