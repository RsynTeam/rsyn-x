...
#define MAKE_SELF_OPERATOR(OP)                         \
        inline void operator OP(const double2 right) { \
                x OP right.x;                          \
                y OP right.y;                          \
        }

#define MAKE_SELF_OPERATOR_SCALAR(OP)                 \
        inline void operator OP(const double right) { \
                x OP right;                           \
                y OP right;                           \
        }
...
struct double2 {
        ...
        explicit double2(const double scalar)
            : x(scalar),
              y(scalar) {
        }  // explicit to avoid accidentally assigning to scalar

        DBUxy scaleAndConvertToDbu(
            const double scale,
            const RoundingStrategy roundingStrategy = ROUND_DOWN) const {
                return DBUxy(
        ...
        DBUxy convertToDbu(
            const RoundingStrategy roundingStrategy = ROUND_DOWN) const {
                return DBUxy((DBU)FloatingPoint::round(x, roundingStrategy),
        ...
        const double &operator[](const int dimension) const {
                return xy[dimension];
        }

        // Methods.
        double norm() const { return sqrt(x * x + y * y); }  // end method
        ...
        double aggregated() const { return x + y; }  // end method
        ...
        inline bool approximatelyEqual(const double2 other,
...
inline bool double2::approximatelyEqual(const double2 other,
...
class double2_split_vector {
       public:
        struct double2_ref {
                ...
                double &operator[](const int dimension) {
                        return (dimension == X)
                                   ? x
                                   : ((dimension == Y) ? y : *((double *)NULL));
                }
                const double &operator[](const int dimension) const {
                        return (dimension == X)
                                   ? x
                ...
                // Returns a const double2 to avoid accidentally trying to
                // assign to a
                // temporary.
        ...
        const double2 operator[](const int index) const {
                return double2(xy[0][index], xy[1][index]);
        }
        double2_ref operator[](const int index) {
                return double2_ref(xy[0][index], xy[1][index]);
        }
        ...
        void resize(size_t size, double2 val) {
                xy[0].resize(size, val.x);
                xy[1].resize(size, val.y);
        }

        void assign(size_t n, double2 val) {
                xy[0].assign(n, val.x);
                xy[1].assign(n, val.y);
        }
