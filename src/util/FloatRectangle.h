...
class FloatRectangle {
       ...
       public:
        ...
        FloatRectangle(const float xmin, const float ymin, const float xmax,
                       const float ymax) {
        ...
        Bounds scaleAndConvertToDbu(
            const float scale, const RoundingStrategy roundingLower,
        ...
        void updatePoints(const float xMin, const float yMin, const float xMax,
                          const float yMax) {
                (*this)[LOWER][X] = xMin;
                (*this)[LOWER][Y] = yMin;
                (*this)[UPPER][X] = xMax;
        ...
        const float2 &operator[](const int boundary) const {
                return clsPoints[boundary];
        }

        float computeLength(const int dimension) const {
                return (*this)[UPPER][dimension] - (*this)[LOWER][dimension];
        }
        float2 computeLength() const { return (*this)[UPPER] - (*this)[LOWER]; }

        float computeDiagonal() const {
                return std::sqrt(std::pow(computeLength(X), 2.0f) +
                                 std::pow(computeLength(Y), 2.0f));
        }

        float computeCenter(const int dimension) const {
                return ((*this)[UPPER][dimension] + (*this)[LOWER][dimension]) /
                       2.0f;
        }
        float2 computeCenter() const {
                return ((*this)[UPPER] + (*this)[LOWER]) / 2.0f;
        }

        float computeArea() const {
                return computeLength(0) * computeLength(1);
        }
        float computeSemiperimeter() const {
                return computeLength(0) + computeLength(1);
        }
        ...
        float overlapArea(const FloatRectangle &rect) const {
                const float dx = max((*this)[LOWER][X], rect[LOWER][X]) -
                                 min((*this)[UPPER][X], rect[UPPER][X]);
                const float dy = max((*this)[LOWER][Y], rect[LOWER][Y]) -
                                 min((*this)[UPPER][Y], rect[UPPER][Y]);

                return (dx < 0 && dy < 0) ? dx * dy : 0;
        }  // end method

        float getCoordinate(const Boundary bound, const Dimension dim) const {
                return (*this)[bound][dim];
        }
        float2 getCoordinate(const Boundary bound) const {
                return (*this)[bound];
        }
        ...
        void moveCenterTo(const float x, const float y) {
                moveCenterTo(float2(x, y));
        }
        void translate(const float x, const float y) {
                translate(float2(x, y));
        }
