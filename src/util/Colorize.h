...
class Colorize {
       public:
        static void colorTemperature(const double weight, int &r, int &g,
                                     int &b) {
                ...
                //		static const int R[numColors] = {   0,   0,   0,
                // 0,   0,   0,  66, 132, 189, 255, 255, 255, 255, 255, 189,
                // 132};
                //		static const int G[numColors] = {   0,   0,  66,
                // 132, 189, 255, 255, 255, 255, 255, 189, 132,  66,   0,   0,
                // 0};
                //		static const int B[numColors] = { 189, 255, 255,
                // 255, 255, 255, 189, 132,  66,   0,   0,   0,   0,   0,   0,
                // 0};
                ...
                if (std::isnan(weight)) {
                        ...
                        r = (int)std::floor(R[i0] * (1 - alpha) +
                                            R[i1] * (alpha) + 0.5);
                        g = (int)std::floor(G[i0] * (1 - alpha) +
                                            G[i1] * (alpha) + 0.5);
                        b = (int)std::floor(B[i0] * (1 - alpha) +
                                            B[i1] * (alpha) + 0.5);
                }  // end else
        }          // end method

        static void colorTemperatureWarmCold(const double weight, int &r,
                                             int &g, int &b) {
                static const std::array<int, 33> R{
                    59,  68,  77,  87,  98,  108, 119, 130, 141, 152, 163,
                    174, 184, 194, 204, 213, 221, 229, 236, 241, 245, 247,
                    247, 247, 244, 241, 236, 229, 222, 213, 203, 192, 180};
                static const std::array<int, 33> G{
                    76,  90,  104, 117, 130, 142, 154, 165, 176, 185, 194,
                    201, 208, 213, 217, 219, 221, 216, 211, 204, 196, 187,
                    177, 166, 154, 141, 127, 112, 96,  80,  62,  40,  4};
                static const std::array<int, 33> B{
                    192, 204, 215, 225, 234, 241, 247, 251, 254, 255, 255,
                    253, 249, 244, 238, 230, 221, 209, 197, 185, 173, 160,
