...
class Array2D {
       ...
       public:
        ...
        T &operator()(const int col, const int row) {
                return clsElements[computeLinearIndex(col, row)];
        }
        const T &operator()(const int col, const int row) const {
                return clsElements[computeLinearIndex(col, row)];
        }

        T &operator()(const int linearIndex) {
                return clsElements[linearIndex];
        }
        const T &operator()(const int linearIndex) const {
                return clsElements[linearIndex];
        }
        ...
        int getCol(const int linearIndex) const {
                return linearIndex % clsNumCols;
        }
        int getRow(const int linearIndex) const {
                return linearIndex / clsNumCols;
        }

        int clampCol(const int col) const {
                return std::max(0, std::min(col, clsNumCols - 1));
        }
        int clampRow(const int row) const {
                return std::max(0, std::min(row, clsNumRows - 1));
        }

        bool isValidIndex(const int col, const int row) const {
                return (col >= 0 && col < clsNumCols) &&
                       (row >= 0 && row < clsNumRows);
        }  // end method
        ...
        void gnuplot3DMap(const std::string &filename) const {
                ...
                plotfile
                    << "set style data lines				"
                       ...
                       "	"
                       "						\n"
                       "set terminal epscairo enhanced color size 75in,50in "
                       "font \"Arial,100\"	\n"
                       "set output \""
                    << filename
                    << ".eps						"
...
class Array3D {
       public:
        void initialize(const int length0, const int length1,
                        const int length2) {
        ...
        int getNumElements() const { return _data.size(); }  // end method
        ...
        bool isValidIndex(const int i, const int j, const int k) const {
                return i >= 0 && i < _length[0] && j >= 0 && j < _length[1] &&
                       k >= 0 && k < _length[2];
        }  // end method
       ...
       private:
        void initialize(const int length0, const int length1, const int length2,
                        const bool setDefaultValue, const T &defaultValue) {
                _length[0] = length0;
                _length[1] = length1;
                _length[2] = length2;
