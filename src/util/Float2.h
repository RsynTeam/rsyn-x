...
#define MAKE_SELF_OPERATOR(OP)                        \
        inline void operator OP(const float2 right) { \
                x OP right.x;                         \
                y OP right.y;                         \
        }

#define MAKE_SELF_OPERATOR_SCALAR(OP)                \
        inline void operator OP(const float right) { \
                x OP right;                          \
                y OP right;                          \
        }
...
struct float2 {
        ...
        explicit float2(const float scalar)
            : x(scalar),
              y(scalar) {
        ...
        const float &operator[](const int dimension) const {
                return xy[dimension];
        }

        // Methods.
        float norm() const { return std::sqrt(x * x + y * y); }  // end method
        ...
        // clockwise
        float2 perpendicular() const { return float2(y, -x); }  // end method
        ...
        float aggregated() const { return x + y; }  // end method
        ...
        inline bool approximatelyEqual(const float2 other,
                                       const float precision = 1e-6f) const;
...
inline bool float2::approximatelyEqual(const float2 other,
