        ...
        if (timingLibraryCell.floatingPins.size() > 0 &&
            !rysnLibraryCell.isSequential()) {
                for (Rsyn::LibraryPin lpin : timingLibraryCell.floatingPins) {
                        Rsyn::SandboxPin pin =
                            rsynCell.getPinByLibraryPin(lpin);
...
void SandboxTimer::init(Rsyn::Session rsynSession, Rsyn::Sandbox rsynSandbox) {
        ...
        for (Rsyn::SandboxPin pin : sandbox.allPins()) {
                ...
                if (relatedPin) {
                        ...
                        const TimingPin &timingPinDesign =
                            clsTimer->getTimingPin(relatedPin);
        ...
        for (Rsyn::SandboxArc arc : sandbox.allArcs()) {
                ...
                if (!relatedArc) continue;

                TimingArc &timingArcSandbox = getTimingArc(arc);
                const TimingArc &timingArcDesign =
                    clsTimer->getTimingArc(relatedArc);
        ...
        for (Rsyn::SandboxPort port : sandbox.allPorts()) {
                if (port.isVirtual()) {
                        Rsyn::Pin relatedPin =
                            port.getAttachedPin().getRelated();

                        if (relatedPin) {
                                TimingPin &timingPinSandbox =
                                    getTimingPin(port.getInnerPin());
                                const TimingPin &timingPinDesign =
                                    clsTimer->getTimingPin(relatedPin);
                                timingPinSandbox.state = timingPinDesign.state;
                                ...
                                switch (port.getDirection()) {
                                        case Rsyn::IN: {
                                                setInputDelay(port, EARLY,
                                                              timingPinSandbox
                                                                  .state[EARLY]
                                                                  .a);
                                                setInputDelay(
                                                    port, LATE,
                                                    timingPinSandbox.state[LATE]
                                                        .a);
                                                break;
                                        }  // end case
                                        case Rsyn::OUT: {
                                                setOutputRequiredTime(
                                                    port, EARLY,
                                                    timingPinSandbox
                                                        .state[EARLY]
                                                        .q);
                        ...
                        // If the port is not virtual, the timing values were
                        // already
                        // copied when we copied the pin data, but we still need
                        // to copy the
                        // constraints.
                        ...
                        if (relatedPort) {
                                switch (port.getDirection()) {
                                        case Rsyn::IN: {
                                                const Number a =
                                                    clsScenario->getInputDelay(
                                                        relatedPort, 0);
                                                setInputDelay(
                                                    port, EARLY,
                                                    EdgeArray<Number>(a, a));
                                                setInputDelay(
                                                    port, LATE,
                                                    EdgeArray<Number>(a, a));
                                                break;
                                        ...
                                        case Rsyn::OUT: {
                                                const Number c =
                                                    clsScenario->getOutputLoad(
                                                        relatedPort, 0);
                                                setOutputLoad(
                                                    port, EARLY,
                                                    EdgeArray<Number>(c, c));
                                                setOutputLoad(
                                                    port, LATE,
                                                    EdgeArray<Number>(c, c));
                                                setOutputRequiredTime(
...
void SandboxTimer::timingBuildTimingArcs_SetupBacktrackEdge(
    TimingArc &arc, const TimingSense sense) {
...
void SandboxTimer::updateTiming_Arc_NonUnate(const TimingMode mode,
        ...
        if (ENABLE_IITIMER_COMPATIBILITY_MODE) {
                ...
                if (timingPinFrom.clocked /*is sequential*/) {
                        ...
                        for (const std::tuple<TimingTransition,
                                ...
                                timingModel->calculateCellArcTiming(
                                    arc, mode, oedge, islew[iedge], load[oedge],
                                    delay[oedge][iedge], oslew[oedge][iedge]);
                        }  // end for
...
void SandboxTimer::updateTiming_Arc(const TimingMode mode,
                                    const EdgeArray<Number> islew,
                                    const EdgeArray<Number> load,
                                    const bool skip, Rsyn::SandboxArc arc,
...
void SandboxTimer::updateTiming_Net_InitDriver(Rsyn::SandboxPin driver,
        ...
        if (driver.isPort()) {
                ...
                timingPin.state[mode].a =
                    getInputDelay(port, mode, EdgeArray<Number>(0, 0));
                timingPin.state[mode].slew =
                    getInputTransition(port, mode, EdgeArray<Number>(0, 0));
                ...
                if (inputDriver) {
                        switch (inputDriverDelayMode) {
                                case INPUT_DRIVER_DELAY_MODE_UI_TIMER: {
                                        TimingArcState state;
                                        updateTiming_Arc(
                                            mode, inputDriver->inputSlew, load,
                                            false, nullptr,
                                            inputDriver->libraryArc, state);
                                ...
                                case INPUT_DRIVER_DELAY_MODE_PRIME_TIME: {
                                        // Note. The delay at an input port
                                        // driven by an input driver is
                                        // set to the user defined input delay
                                        // plus the delta between the
                                        // delay of the input driver driven the
                                        // actual net load and the
                                        // delay of the input driver driven no
                                        // load. The idea is to remove
                                        // the parasitic delay from the arrival
                                        // time.
                                        TimingArcState state0;
                                        TimingArcState state1;
                                        updateTiming_Arc(
                                            mode, inputDriver->inputSlew,
                                            EdgeArray<Number>(0, 0), false,
                                            nullptr, inputDriver->libraryArc,
                                            state0);
                                        updateTiming_Arc(
                                            mode, inputDriver->inputSlew, load,
                                            false, nullptr,
                                            inputDriver->libraryArc, state1);
                                        timingPin.state[mode].a +=
                                            state1.delay - state0.delay;
                                        timingPin.state[mode].slew =
                                            state1.oslew;
                                        break;
                                }  // end case
...
void SandboxTimer::updateTiming_Net_TimingMode(const TimingMode mode,
                                               Rsyn::SandboxNet net,
                                               const EdgeArray<Number> load,
                                               Rsyn::SandboxArc arc) {
...
void SandboxTimer::updateTiming_UpdateTimingTests_SetupHold_DataPin(
    Rsyn::SandboxPin pin) {
...
void SandboxTimer::updateTiming_UpdateTimingTests() {
        ...
        for (Rsyn::SandboxPin pin : allEndpoints()) {
                ...
                if (pin.isPort()) {
                        ...
                        timingPin.state[EARLY].q = getOutputRequiredTime(
                            port, EARLY, EdgeArray<Number>(0, 0));
                        timingPin.state[LATE].q = getOutputRequiredTime(
...
void SandboxTimer::updateTiming_PropagateRequiredTimes_Net(
    Rsyn::SandboxNet net) {
...
void SandboxTimer::updateTiming_PropagateRequiredTimesIncremental(
    const std::set<Rsyn::SandboxNet> &nets) {
        ...
        while (!queue.empty()) {
                ...
                Rsyn::SandboxPin driver =
                    net.getDriver();  // [ASSUMPTION] Net has a single driver.
...
// Debug
                ...
                if (driver) {
                        for (Rsyn::SandboxArc arc : driver.allIncomingArcs()) {
                                ...
                                if (previousNet) {
                                        const TimingNet &previousTimingNet =
                                            getTimingNet(previousNet);
...
void SandboxTimer::updateTiming_PropagateArrivalTimesIncremental(
    std::set<Rsyn::SandboxNet> &endpoints) {
        ...
        while (!queue.empty()) {
                ...
                for (Rsyn::SandboxPin from : net.allPins(Rsyn::SINK)) {
                        ...
                        if (wasDirty) {
                                for (Rsyn::SandboxArc arc :
                                     from.allOutgoingArcs()) {
                                        Rsyn::SandboxNet nextNet =
                                            arc.getToNet();
                                        if (nextNet) {
        ...
        if (!opt_show_stwl)
                for (Rsyn::SandboxInstance instance : sandbox.allInstances()) {
                        for (Rsyn::SandboxPin pin : instance.allPins()) {
                                ...
                                for (Rsyn::SandboxPin predecessor :
                                ...
                                Rsyn::SandboxNet net = pin.getNet();
                                // Rsyn::SandboxCell cell = pin.getCell();
                                // unused variable
                                // TimingPin &timingPin = getTimingPin(pin);
                                // unused variable
...
void SandboxTimer::printPinDebug(ostream &out, Rsyn::SandboxPin pin,
                                 const bool early, const bool late) {
...
void SandboxTimer::writeTimingFile(ostream &out) {
        ...
        for (Rsyn::SandboxPort cell : sandbox.allPorts(Rsyn::OUT)) {
                const EdgeArray<Number> earlyLoadCap =
                    getOutputLoad(cell, EARLY, EdgeArray<Number>(0, 0));
                const EdgeArray<Number> lateLoadCap =
                    getOutputLoad(cell, EARLY, EdgeArray<Number>(0, 0));
                out << "load " << cell.getName() << " " << earlyLoadCap[FALL]
                    << " " << earlyLoadCap[RISE] << " " << lateLoadCap[FALL]
                    << " " << lateLoadCap[RISE] << "\n";
        }
}  // end method
...
void SandboxTimer::queryTopCriticalPaths_Queue_AddCriticalEndpoint(
    CriticalPathQueue &queue, Rsyn::SandboxPin endpoint, const TimingMode mode,
...
void SandboxTimer::queryTopCriticalPaths_Queue_AddAllCriticalEndpoints(
...
bool SandboxTimer::queryTopCriticalPaths_Internal(
...
bool SandboxTimer::queryTopCriticalPaths(
    const TimingMode mode, const int maxNumPaths,
...
int SandboxTimer::queryTopCriticalPathsPassingThruPin(
    ...
    const Number slackThreshold) {
        ...
        for (Rsyn::SandboxNet net :
             sandbox.getFaninConeNetsInBreadthFirstOrder(referencePin)) {
                getTimingNet(net).sign = sign;
        }  // end for

        // Mark nets in the fan-out of the reference pin.
        for (Rsyn::SandboxNet net :
             sandbox.getFanoutConeNetsInBreadthFirstOrder(referencePin)) {
                getTimingNet(net).sign = sign;
        }  // end for
...
void SandboxTimer::reportPath(const std::vector<PathHop> &path,
                              std::ostream &out) {
...
void SandboxTimer::reportPaths(const std::vector<std::vector<PathHop>> &paths,
                               std::ostream &out) {
...
void SandboxTimer::reportCriticalPath(const TimingMode mode,
                                      std::ostream &out) {
...
void SandboxTimer::reportEndpoints(const TimingMode mode, const int top,
                                   const Number slackThreshold,
                                   std::ostream &out) {
...
void SandboxTimer::getPathDelayDueToCellsAndWires(
    const std::vector<PathHop> &path, Number &delayDueToCell,
...
std::string SandboxTimer::getPathHash(const std::vector<PathHop> &path,
...
bool SandboxTimer::queryTopCriticalEndpoints(
    const TimingMode mode, const int maxNumEndpoints,
    std::vector<Rsyn::SandboxPin> &endpoints, const Number slackThreshold) {
        const bool debug = false;
...
void SandboxTimer::setInputDriver(Rsyn::SandboxInstance port,
                                  InputDriver driver) {
...
void SandboxTimer::setInputDelay(Rsyn::SandboxInstance port,
                                 const TimingMode &mode,
...
void SandboxTimer::setInputTransition(Rsyn::SandboxInstance port,
                                      const TimingMode &mode,
                                      const EdgeArray<Number> &value) {
...
void SandboxTimer::setOutputRequiredTime(Rsyn::SandboxInstance port,
                                         const TimingMode &mode,
                                         const EdgeArray<Number> &value) {
...
void SandboxTimer::setOutputLoad(Rsyn::SandboxInstance port,
                                 const TimingMode &mode,
                                 const EdgeArray<Number> &value) {
...
void SandboxTimer::setOutputDelay(Rsyn::SandboxInstance port,
                                  const TimingMode &mode,
                                  const EdgeArray<Number> &value) {
...
EdgeArray<Number> SandboxTimer::getInputDelay(
...
EdgeArray<Number> SandboxTimer::getInputTransition(
    Rsyn::SandboxInstance port, const TimingMode &mode,
    const EdgeArray<Number> &defaultValue) const {
        auto it = clsInputTransitions[mode].find(port);
        return it == clsInputTransitions[mode].end() ? defaultValue
                                                     : it->second;
}  // end method

// -----------------------------------------------------------------------------

EdgeArray<Number> SandboxTimer::getOutputRequiredTime(
    Rsyn::SandboxInstance port, const TimingMode &mode,
    const EdgeArray<Number> &defaultValue) const {
...
EdgeArray<Number> SandboxTimer::getOutputLoad(
    Rsyn::SandboxInstance port, const TimingMode &mode,
    const EdgeArray<Number> &defaultValue) const {
...
const SandboxTimer::InputDriver *SandboxTimer::getInputDriver(
    Rsyn::SandboxPort port) const {
