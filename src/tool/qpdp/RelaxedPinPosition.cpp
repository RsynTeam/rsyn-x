...
void RelaxedPinPosition::getNetBoundingBoxUsingRelaxedPinPositions(Rsyn::Net net, 
		DBU &xmin,
		DBU &ymin,
		DBU &xmax,
		DBU &ymax
	...
	for (Rsyn::Pin pin : net.allPins()) {
		...
		const DBUxy position = clsPhysicalDesign.getRelaxedPinPosition(pin);
...
void RelaxedPinPosition::getNetBoundaryPinsUsingRelaxedPinPositions(Rsyn::Net net, 
		Rsyn::Pin &left, 
		Rsyn::Pin &right, 
		Rsyn::Pin &bottom, 
		Rsyn::Pin &top
...
void RelaxedPinPosition::getNetBoundaryPinsUsingRelaxedPinPositions(Rsyn::Net net, 
	...
	Rsyn::Pin bottom; 
	Rsyn::Pin top;	
	getNetBoundaryPinsUsingRelaxedPinPositions(net, left, right, bottom, top);
...
void RelaxedPinPosition::getNetPinsSortedByRelaxedPinPositions(Rsyn::Net net, 
	...
	int index = 0;
	for (Rsyn::Pin pin : net.allPins()) {
		pins[index++] = std::make_tuple(clsPhysicalDesign.getRelaxedPinPosition(pin)[dimension], pin);
	} // end 
