...
private:
	...
	typedef std::function<double (
			Rsyn::Pin pin0, Rsyn::Pin pin1, const double edgeWeight)> Weightening;
	...
	void neutralizeSystemUsingAnchors(
	...
	void neutralizeSystemUsingForces(
			const SquareCompressedRowMatrix &A,
			const std::vector<double> &b,
			const std::vector<double> &position,
			std::vector<double> &digIncrement,
	...
	void markAsMovableAllNonFixedCells(const bool considerCriticalCellsAsFixed, std::set<Rsyn::Cell> &movable);
	void markAsMovableAllNonFixedCriticalCells(std::set<Rsyn::Cell> &movable);
	void markAsMovableAllNonFixedNonCriticalCellsDrivenByCriticalNets(std::set<Rsyn::Cell> &movable);
	void markAsMovableAllNonFixedCellsConnectedToCriticalNets(std::set<Rsyn::Cell> &movable);
	void markAsMovableAllNonFixedCriticalCellsAndTheirSinks(std::set<Rsyn::Cell> &movable);
	void markAsMovableAllNonFixedCriticalCellsAndTheirNeighbors(std::set<Rsyn::Cell> &movable);
	void markAsMovableAllNonFixedCellsInTopCriticalPaths(const int numPaths, std::set<Rsyn::Cell> &movable);
	void markAsMovableAllNonFixedCellsInKthTopCriticalPath(const int k, std::set<Rsyn::Cell> &movable);
	void markAsMovableAllNonFixedCellsInTopCriticalPathsFromTopCriticalEndpoints(const int numEndpoints, const int numPathsPerEndpoings, std::set<Rsyn::Cell> &movable);
	void markAsMovableAllNonFixedCellsInTopCriticalPathsAndTheirNeighbors(const int numPaths, std::set<Rsyn::Cell> &movable);
