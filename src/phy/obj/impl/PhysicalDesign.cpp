...
void PhysicalDesign::loadLibrary(const LefDscp &library) {
        ...
        if (library.clsLefUnitsDscp.clsHasDatabase) {
                if (getDatabaseUnits(LIBRARY_DBU) == 0) {
                        data->clsDBUs[LIBRARY_DBU] =
                            library.clsLefUnitsDscp.clsDatabase;
                } else {
                        if (getDatabaseUnits(LIBRARY_DBU) !=
                            library.clsLefUnitsDscp.clsDatabase) {
                                std::cout
                                    << "WARNING: Stored LEF database units "
                                    << getDatabaseUnits(LIBRARY_DBU)
                                    << " is not equal to LEF database units "
                                       "defined in Library "
                                    << library.clsLefUnitsDscp.clsDatabase
                                    << ".\n";
                                std::cout << "WARNING: Lef library elements "
...
void PhysicalDesign::loadDesign(const DefDscp &design) {
        if (data->clsLoadDesign) {
                std::cout
                    << "WARNING: Design was already loaded. Skipping ... \n";
                return;
        }  // end if
        ...
        if (getDatabaseUnits(LIBRARY_DBU) % getDatabaseUnits(DESIGN_DBU) != 0) {
                std::cout << "ERROR: Invalid DEF database units "
                          << getDatabaseUnits(DESIGN_DBU)
                          << " (LEF database units: "
                          << getDatabaseUnits(LIBRARY_DBU) << ").\n";
                std::cout << "DEF design units should be lower or equal to LEF "
                             "design units and must have a integer multiple. "
                             "Physical design was not initialized!\n";
                return;
        }  // end if

        // This operation always results in an integer number factor.
        // LEF/DEF specifications prohibit division that results in a real
        // number factor.
        data->clsDBUs[MULT_FACTOR_DBU] =
            getDatabaseUnits(LIBRARY_DBU) / getDatabaseUnits(DESIGN_DBU);

        // Adding design defined vias
        std::size_t numVias =
            data->clsPhysicalVias.size() + design.clsVias.size();
        ...
        for (const DefComponentDscp &component : design.clsComps) {
                ...
                Rsyn::Cell cell =
                    data->clsDesign.findCellByName(component.clsName);
                if (!cell) {
                        throw Exception("Cell " + component.clsName +
                                        " not found.\n");
                }  // end if
        ...
        for (const DefPortDscp &pin_port : design.clsPorts) {
                // Adding Library cell to Physical Layer
                Rsyn::Port cell =
                    data->clsDesign.findPortByName(pin_port.clsName);
                if (!cell) {
                        throw Exception("Port " + pin_port.clsName +
                                        " not found.\n");
                }  // end if
        ...
        data->clsPhysicalRows.reserve(
            design.clsRows.size());  // reserving space for rows
        ...
        for (const DefNetDscp &net : design.clsNets) addPhysicalNet(net);
        ...
        data->clsNumElements[PHYSICAL_PORT] =
            data->clsDesign.getNumInstances(Rsyn::PORT);

        std::cout << "Loaded Design\n";
        std::cout << "\tName: " << data->clsDesign.getName() << "\n";
        std::cout << "\t#Moveable Insts: "
                  << getNumElements(PhysicalType::PHYSICAL_MOVABLE) << "\n";
        std::cout << "\t#Fixed Insts: "
                  << getNumElements(PhysicalType::PHYSICAL_FIXED) << "\n";
}  // end method

// -----------------------------------------------------------------------------

void PhysicalDesign::initPhysicalDesign(Rsyn::Design dsg,
        ...
        if (!params.is_null()) {
                data->clsEnablePhysicalPins = params.value(
                    "clsEnablePhysicalPins", data->clsEnablePhysicalPins);
                data->clsEnableMergeRectangles = params.value(
                    "clsEnableMergeRectangles", data->clsEnableMergeRectangles);
                data->clsEnableNetPinBoundaries =
                    params.value("clsEnableNetPinBoundaries",
                                 data->clsEnableNetPinBoundaries);
                data->clsMode = getPhysicalDesignModeType(
                    params.value("clsPhysicalDesignMode", "ALL"));
        }  // end if
        ...
        PhysicalInstanceData &phTopModule =
            data->clsPhysicalInstances[dsg.getTopModule()];
...
void PhysicalDesign::updateAllNetBounds(const bool skipClockNet) {
        ...
        for (Rsyn::Net net : data->clsModule.allNets()) {
                ...
                if (skipClockNet && (data->clsClkNet == net)) continue;
...
void PhysicalDesign::updateNetBound(Rsyn::Net net) {
        ...
        for (Rsyn::Pin pin : net.allPins()) {
                ...
                if (pos[X] >= bound[UPPER][X]) {
                        bound[UPPER][X] = pos[X];
                        if (updatePinBound) phNet.clsBoundPins[UPPER][X] = pin;
                }  // end if

                // lower x corner pos
                if (pos[X] <= bound[LOWER][X]) {
                        bound[LOWER][X] = pos[X];
                        if (updatePinBound) phNet.clsBoundPins[LOWER][X] = pin;
                }  // end if

                // upper y corner pos
                if (pos[Y] >= bound[UPPER][Y]) {
                        bound[UPPER][Y] = pos[Y];
                        if (updatePinBound) phNet.clsBoundPins[UPPER][Y] = pin;
                }  // end if

                // lower y corner pos
                if (pos[Y] <= bound[LOWER][Y]) {
                        bound[LOWER][Y] = pos[Y];
                        if (updatePinBound) phNet.clsBoundPins[LOWER][Y] = pin;
                }                                // end if
        }                                        // end for
...
void PhysicalDesign::addPhysicalSite(const LefSiteDscp &site) {
        std::unordered_map<std::string, int>::iterator it =
            data->clsMapPhysicalSites.find(site.clsName);
        if (it != data->clsMapPhysicalSites.end()) {
                std::cout << "WARNING: Site " << site.clsName
                          << " was already defined. Skipping ...\n";
                return;
        }  // end if
        ...
        data->clsMapPhysicalSites[site.clsName] =
            data->clsPhysicalSites.size() - 1;
...
Rsyn::PhysicalLayerData *PhysicalDesign::addPhysicalLayer(
    const LefLayerDscp &layer, Rsyn::PhysicalLayerData *lower) {
        std::unordered_map<std::string, std::size_t>::iterator it =
            data->clsMapPhysicalLayers.find(layer.clsName);
        if (it != data->clsMapPhysicalLayers.end()) {
                std::cout << "WARNING: Layer " << layer.clsName
                          << " was already defined. Skipping ...\n";
                // TODO -> return the pointer to the phLayerData of already
        ...
        phLayer->clsDirection =
            Rsyn::getPhysicalLayerDirection(layer.clsDirection);
        phLayer->clsType = Rsyn::getPhysicalLayerType(layer.clsType);
        phLayer->clsPitch[X] =
            static_cast<DBU>(std::round(layer.clsPitch[X] * libDBU));
        phLayer->clsPitch[Y] =
            static_cast<DBU>(std::round(layer.clsPitch[Y] * libDBU));
        phLayer->clsSpacing.reserve(layer.clsSpacingRules.size());
        for (const LefSpacingRuleDscp &spcRule : layer.clsSpacingRules) {
                phLayer->clsSpacing.push_back(
                    PhysicalSpacingRule(new PhysicalSpacingRuleData()));
                PhysicalSpacingRule &spc = phLayer->clsSpacing.back();
                spc.data->clsEOL =
                    static_cast<DBU>(std::round(spcRule.clsEOL * libDBU));
                spc.data->clsSpacing =
                    static_cast<DBU>(std::round(spcRule.clsSpacing * libDBU));
                spc.data->clsEOLWithin =
                    static_cast<DBU>(std::round(spcRule.clsEOLWithin * libDBU));
        }  // end for
        phLayer->clsMinWidth =
            static_cast<DBU>(std::round(layer.clsMinWidth * libDBU));
        phLayer->clsArea =
            static_cast<DBU>(std::round(layer.clsArea * libDBU * libDBU));
        phLayer->clsWidth =
            static_cast<DBU>(std::round(layer.clsWidth * libDBU));
...
void PhysicalDesign::addPhysicalVia(const LefViaDscp &via) {
        std::unordered_map<std::string, std::size_t>::iterator it =
            data->clsMapPhysicalVias.find(via.clsName);
        if (it != data->clsMapPhysicalVias.end()) {
                std::cout << "WARNING: Via " << via.clsName
                          << " was already defined. Skipping ...\n";
                return;
        }  // end if
        ...
        if (via.clsHasViaRule) {
                Rsyn::PhysicalViaRuleBase phViaRuleBase =
                    getPhysicalViaRuleBaseByName(via.clsViaRuleName);
                phVia->clsViaRuleData = phViaRuleBase.data;
                phVia->clsCutSize[X] =
                    convertMicronToLibraryDatabaseUnits(via.clsXCutSize);
                phVia->clsCutSize[Y] =
                    convertMicronToLibraryDatabaseUnits(via.clsYCutSize);
                phVia->clsSpacing[X] =
                    convertMicronToLibraryDatabaseUnits(via.clsXCutSpacing);
                phVia->clsSpacing[Y] =
                    convertMicronToLibraryDatabaseUnits(via.clsYCutSpacing);
                phVia->clsEnclosure[BOTTOM_VIA_LEVEL][X] =
                    convertMicronToLibraryDatabaseUnits(
                        via.clsXBottomEnclosure);
                phVia->clsEnclosure[TOP_VIA_LEVEL][Y] =
                    convertMicronToLibraryDatabaseUnits(
                        via.clsYBottomEnclosure);
                phVia->clsEnclosure[BOTTOM_VIA_LEVEL][X] =
                    convertMicronToLibraryDatabaseUnits(via.clsXTopEnclosure);
                phVia->clsEnclosure[TOP_VIA_LEVEL][Y] =
                    convertMicronToLibraryDatabaseUnits(via.clsYTopEnclosure);

                Rsyn::PhysicalLayer bottom =
                    getPhysicalLayerByName(via.clsBottomLayer);
                Rsyn::PhysicalLayer cut =
                    getPhysicalLayerByName(via.clsCutLayer);
                Rsyn::PhysicalLayer top =
                    getPhysicalLayerByName(via.clsTopLayer);
                ...
                if (via.clsHasOrigin) {
                        ...
                        phVia->clsOrigin[X] =
                            convertMicronToLibraryDatabaseUnits(via.clsXOrigin);
                        phVia->clsOrigin[Y] =
                            convertMicronToLibraryDatabaseUnits(via.clsYOrigin);
                }  // end if
                if (via.clsHasOffset) {
                        phVia->clsHasOffset = true;
                        phVia->clsOffset[BOTTOM_VIA_LEVEL][X] =
                            convertMicronToLibraryDatabaseUnits(
                                via.clsXBottomOffset);
                        phVia->clsOffset[TOP_VIA_LEVEL][Y] =
                            convertMicronToLibraryDatabaseUnits(
                                via.clsYBottomOffset);
                        phVia->clsOffset[BOTTOM_VIA_LEVEL][X] =
                            convertMicronToLibraryDatabaseUnits(
                                via.clsXTopOffset);
                        phVia->clsOffset[TOP_VIA_LEVEL][Y] =
                ...
                for (const std::pair<std::string,
                                     std::deque<LefViaGeometryDscp>> &geoPair :
                     via.clsGeometries) {
                        const std::string &layerName = geoPair.first;
                        Rsyn::PhysicalLayer layer =
                            getPhysicalLayerByName(layerName);
                        assert(layer);
                        layers.push_back(
                            std::make_tuple(layer.getIndex(), layer.data));
                }  // end for
                ...
                for (int i = 0; i < NUM_VIA_LAYERS; i++) {
                        ...
                        std::vector<Rsyn::PhysicalViaGeometry> &viaGeos =
                            phVia->clsViaGeometries[i];
                        auto iterator =
                            via.clsGeometries.find(layerData->clsName);
                        const std::deque<LefViaGeometryDscp> &geoDscps =
                            iterator->second;
                        viaGeos.reserve(geoDscps.size());
                        for (const LefViaGeometryDscp &geoDscp : geoDscps) {
                                viaGeos.push_back(Rsyn::PhysicalViaGeometry(
                                    new ViaGeometryData()));
                                Rsyn::PhysicalViaGeometry viaGeo =
                                    viaGeos.back();
                                viaGeo->id = viaGeos.size() - 1;
                                const DoubleRectangle &doubleRect =
                                    geoDscp.clsBounds;
...
void PhysicalDesign::addPhysicalViaRule(const LefViaRuleDscp &via) {
        std::unordered_map<std::string, std::size_t>::iterator it =
            data->clsMapPhysicalViaRuleBases.find(via.clsName);
        if (it != data->clsMapPhysicalViaRuleBases.end()) {
                std::cout << "WARNING: Via Rule " << via.clsName
                          << " was already defined. Skipping ...\n";
                return;
        }  // end if

        // Adding new lib site
        data->clsMapPhysicalViaRuleBases[via.clsName] =
            data->clsPhysicalViaRuleBases.size();
        data->clsPhysicalViaRuleBases.push_back(
            PhysicalViaRuleBase(new ViaRuleData()));
        PhysicalViaRuleBase phViaRuleBase =
            data->clsPhysicalViaRuleBases.back();
        phViaRuleBase->init();
        ...
        for (int i = 0; i < via.clsLayers.size(); i++) {
                ...
                Rsyn::PhysicalLayer viaLayer =
                    getPhysicalLayerByName(viaLayerDscp.clsName);
        ...
        // Routing layers
        const LefViaRuleLayerDscp &viaLayerDscp0 =
            via.clsLayers[*layerIds.begin()];
        const LefViaRuleLayerDscp &viaLayerDscp1 =
            via.clsLayers[*layerIds.rbegin()];
        Rsyn::PhysicalLayer layer0 =
            getPhysicalLayerByName(viaLayerDscp0.clsName);
        Rsyn::PhysicalLayer layer1 =
            getPhysicalLayerByName(viaLayerDscp1.clsName);
        ...
        if (viaLayerDscp0.clsHasWidth) {
                ...
                phViaRuleBase->clsWidth[layerLevel0][ViaRange::VIA_RANGE_MIN] =
                    convertMicronToLibraryDatabaseUnits(
                        viaLayerDscp0.clsMinWidth);
                phViaRuleBase->clsWidth[layerLevel0][ViaRange::VIA_RANGE_MAX] =
                    convertMicronToLibraryDatabaseUnits(
                        viaLayerDscp0.clsMaxWidth);
        }  // end if
        if (viaLayerDscp1.clsHasWidth) {
                phViaRuleBase->clsHasWidth[layerLevel1] = true;
                phViaRuleBase->clsWidth[layerLevel1][ViaRange::VIA_RANGE_MIN] =
                    convertMicronToLibraryDatabaseUnits(
                        viaLayerDscp1.clsMinWidth);
                phViaRuleBase->clsWidth[layerLevel1][ViaRange::VIA_RANGE_MAX] =
                    convertMicronToLibraryDatabaseUnits(
                        viaLayerDscp1.clsMaxWidth);
        }  // end if

        if (!via.clsIsGenerate) {
                phViaRuleBase->clsIsGenerate = false;
                phViaRuleBase->clsRelativeIndex =
                    data->clsPhysicalViaRules.size();
                data->clsPhysicalViaRules.push_back(
                    Rsyn::PhysicalViaRule(phViaRuleBase.data));
                phViaRuleBase->clsLayers[layerLevel0] = layer0.data;
                phViaRuleBase->clsLayers[layerLevel1] = layer1.data;
                phViaRuleBase->clsLayerDirection[layerLevel0] =
                    viaLayerDscp0.clsIsHorizontal
                        ? PhysicalLayerDirection::HORIZONTAL
                        : PhysicalLayerDirection::VERTICAL;
                phViaRuleBase->clsLayerDirection[layerLevel1] =
                    viaLayerDscp1.clsIsHorizontal
                        ? PhysicalLayerDirection::HORIZONTAL
                        : PhysicalLayerDirection::VERTICAL;
                ...
                if (!via.clsVias.empty()) {
                        ...
                        for (const std::string &viaName : via.clsVias) {
                                Rsyn::PhysicalVia phVia =
                                    getPhysicalViaByName(viaName);
                                if (phVia == nullptr) {
                                        std::cout << "Via " << viaName
                                                  << " is not defined in "
                                                     "library. Skipping ...\n";
                                        continue;
                                }  // end if
                ...
                phViaRuleBase->clsRelativeIndex =
                    data->clsPhysicalViaRuleGenerates.size();
                data->clsPhysicalViaRuleGenerates.push_back(
                    Rsyn::PhysicalViaRuleGenerate(phViaRuleBase.data));

                // cut layer
                const LefViaRuleLayerDscp &cutLayerDscp =
                    via.clsLayers[cutLayerId];
                Rsyn::PhysicalLayer cutLayer =
                    getPhysicalLayerByName(cutLayerDscp.clsName);
                ...
                bds[LOWER][X] =
                    convertMicronToLibraryDatabaseUnits(doubleRect[LOWER][X]);
                bds[LOWER][Y] =
                    convertMicronToLibraryDatabaseUnits(doubleRect[LOWER][Y]);
                bds[UPPER][X] =
                    convertMicronToLibraryDatabaseUnits(doubleRect[UPPER][X]);
                bds[UPPER][Y] =
                    convertMicronToLibraryDatabaseUnits(doubleRect[UPPER][Y]);
                phViaRuleBase->clsCutSpacing[X] =
                    convertMicronToLibraryDatabaseUnits(
                        cutLayerDscp.clsXSpacing);
                phViaRuleBase->clsCutSpacing[Y] =
                    convertMicronToLibraryDatabaseUnits(
                        cutLayerDscp.clsYSpacing);
                if (cutLayerDscp.clsHasResistance) {
                        phViaRuleBase->clsHasCutResistance = true;
                        phViaRuleBase->clsCutResistance =
                            cutLayerDscp.clsCutResistance;
                }  // end if

                phViaRuleBase->clsLayers[layerType0] = layer0.data;
                phViaRuleBase->clsLayers[layerType1] = layer1.data;
                phViaRuleBase->clsEnclosure1[layerLevel0] =
                    convertMicronToLibraryDatabaseUnits(
                        viaLayerDscp0.clsEnclosure1);
                phViaRuleBase->clsEnclosure1[layerLevel1] =
                    convertMicronToLibraryDatabaseUnits(
                        viaLayerDscp1.clsEnclosure1);
                phViaRuleBase->clsEnclosure2[layerLevel0] =
                    convertMicronToLibraryDatabaseUnits(
                        viaLayerDscp0.clsEnclosure2);
                phViaRuleBase->clsEnclosure2[layerLevel1] =
                    convertMicronToLibraryDatabaseUnits(
                        viaLayerDscp1.clsEnclosure2);
        }  // end if-else
}  // end method

// -----------------------------------------------------------------------------

Rsyn::LibraryCell PhysicalDesign::addPhysicalLibraryCell(
    const LefMacroDscp &macro) {
        Rsyn::LibraryCell lCell =
            data->clsDesign.findLibraryCellByName(macro.clsMacroName);
        if (!lCell) {
                throw Exception("Library Cell " + macro.clsMacroName +
                                " not found.\n");
        }  // end if
        Rsyn::PhysicalLibraryCellData &phlCell =
            data->clsPhysicalLibraryCells[lCell];
        ...
        phlCell.clsMacroClass =
            Rsyn::getPhysicalMacroClass(macro.clsMacroClass);
        ...
        for (const LefObsDscp &libObs : macro.clsObs) {
                ...
                for (DoubleRectangle doubleRect : libObs.clsBounds) {
                        // Jucemar Monteiro - 2017/05/20 - Avoiding cast round
                        // errors.
                        doubleRect.scaleCoordinates(
                            static_cast<double>(getDatabaseUnits(LIBRARY_DBU)));
                        scaledBounds.push_back(Bounds());
                        Bounds &bds = scaledBounds.back();
                        bds[LOWER][X] =
                            static_cast<DBU>(std::round(doubleRect[LOWER][X]));
                        bds[LOWER][Y] =
                            static_cast<DBU>(std::round(doubleRect[LOWER][Y]));
                        bds[UPPER][X] =
                            static_cast<DBU>(std::round(doubleRect[UPPER][X]));
                        bds[UPPER][Y] =
                            static_cast<DBU>(std::round(doubleRect[UPPER][Y]));
                }  // end for

                phlCell.clsObs.push_back(phObs);
                //		if (macro.clsMacroName == "bufx2" ||
                //macro.clsMacroName == "BUFX2") {
                //			std::cout << "----#Obstacles: " <<
                //macro.clsObs.size() << "\n";
                //			std::cout << "" <<
                //macro.clsObs[0].clsBounds.size() << "\n";
                //		}
                if (data->clsEnableMergeRectangles &&
                    libObs.clsBounds.size() > 1) {
                ...
                // Mateus - 2018/03/11 - Defensive programming to avoid crashes
                // in ICCAD15 benchmarks.
                if (!phObs->clsLayer) continue;

                phObs->id = phlCell.clsObs.size() - 1;
                // Hard code. After implementing mapping structure in Rsyn,
                // remove this line.
                ...
                if (topPhObs != nullptr) {
                        if (topPhObs.getLayer().getIndex() <
                            phObs->clsLayer.getIndex())
        ...
        // benchmarks of ICCAD contest define some macro boundaries as metal 1
        // blockages
        if (phlCell.clsLayerBoundIndex >= 0) {
                PhysicalObstacle phObs =
                    phlCell.clsObs[phlCell.clsLayerBoundIndex];

                if (phObs.getNumObs() >
                    1) {  // if boundaries are defined by a rectangle. Then
                          // boundaries number is one because it is rectangle.
                        DBUxy lower, upper;
                        ...
                        for (const Bounds &bds : phObs.allBounds()) {
                                ...
                                if (lowPt[X] != lower[X]) {
                                        phlCell.clsPolygonBounds.addPoint(
                                            lower[X], lowPt[Y]);
                                        lower = lowPt;
                                        phlCell.clsPolygonBounds.addPoint(
                                            lower[X], lower[Y]);
                                ...
                                if (uppPt[X] != upper[X]) {
                                        upperPoints.push_front(
                                            DBUxy(upper[X], lowPt[Y]));
                                        upper = uppPt;
                                        upper[Y] = lowPt[Y];
                        ...
                        phlCell.clsPolygonBounds.addPoint(
                            phObs.allBounds().back()[LOWER][X],
                            phObs.allBounds().back()[UPPER][Y]);

                        for (DBUxy point : upperPoints) {
                                phlCell.clsPolygonBounds.addPoint(point[X],
                                                                  point[Y]);
                        }  // end for
                }          // end if
        }                  // end if
...
void PhysicalDesign::addPhysicalLibraryPin(Rsyn::LibraryCell libCell,
                                           const LefPinDscp &lefPin) {
        Rsyn::LibraryPin rsynLibraryPin =
            libCell.getLibraryPinByName(lefPin.clsPinName);
        if (!rsynLibraryPin) {
                throw Exception("Pin '" + lefPin.clsPinName +
                                "' not found in library cell '" +
                                libCell.getName() + "'.\n");
        }  // end if
        Rsyn::PhysicalLibraryPinData &phyPin =
            data->clsPhysicalLibraryPins[rsynLibraryPin];
        phyPin.clsLibraryCell = getPhysicalLibraryCell(libCell);

        phyPin.clsDirection =
            Rsyn::getPhysicalPinDirection(lefPin.clsPinDirection);
        ...
        for (const LefPortDscp &lefPort : lefPin.clsPorts) {
                phyPin.clsPhysicalPinGeometries.push_back(
                    PhysicalPinGeometry(new PhysicalPinGeometryData()));
                PhysicalPinGeometry phPinPort =
                    phyPin.clsPhysicalPinGeometries.back();
                phPinPort->id = phyPin.clsPhysicalPinGeometries.size() - 1;

                phyPin.clsLayerBound[LOWER] =
                    DBUxy(std::numeric_limits<DBU>::max(),
                          std::numeric_limits<DBU>::max());
                phyPin.clsLayerBound[UPPER] =
                    DBUxy(std::numeric_limits<DBU>::min(),
                          std::numeric_limits<DBU>::min());

                phPinPort->clsPinLayers.reserve(
                    lefPort.clsLefPortGeoDscp.size());
                for (const LefPortGeometryDscp &lefPortGeo :
                     lefPort.clsLefPortGeoDscp) {
                        phPinPort->clsPinLayers.push_back(
                            PhysicalPinLayer(new PhysicalPinLayerData()));
                        PhysicalPinLayer phPinLayer =
                            phPinPort->clsPinLayers.back();
                        phPinLayer->id = phPinPort->clsPinLayers.size() - 1;
                        phPinLayer->clsLibLayer =
                            getPhysicalLayerByName(lefPortGeo.clsMetalName);

                        // ICCAD 2015 contest pin bounds definition
                        if (!lefPortGeo.clsBounds.empty()) {
                                phPinLayer->clsBounds.reserve(
                                    lefPortGeo.clsBounds.size());
                                for (DoubleRectangle doubleRect :
                                     lefPortGeo.clsBounds) {
                                        doubleRect.scaleCoordinates(
                                            static_cast<double>(
                                                getDatabaseUnits(LIBRARY_DBU)));
                                        phPinLayer->clsBounds.push_back(
                                            Bounds());
                                        Bounds &bds =
                                            phPinLayer->clsBounds.back();
                                        bds[LOWER][X] = static_cast<DBU>(
                                            std::round(doubleRect[LOWER][X]));
                                        bds[LOWER][Y] = static_cast<DBU>(
                                            std::round(doubleRect[LOWER][Y]));
                                        bds[UPPER][X] = static_cast<DBU>(
                                            std::round(doubleRect[UPPER][X]));
                                        bds[UPPER][Y] = static_cast<DBU>(
                                            std::round(doubleRect[UPPER][Y]));

                                        phyPin.clsLayerBound[LOWER][X] =
                                            std::min(
                                                phyPin.clsLayerBound[LOWER][X],
                                                bds[LOWER][X]);
                                        phyPin.clsLayerBound[LOWER][Y] =
                                            std::min(
                                                phyPin.clsLayerBound[LOWER][Y],
                                                bds[LOWER][Y]);
                                        phyPin.clsLayerBound[UPPER][X] =
                                            std::max(
                                                phyPin.clsLayerBound[UPPER][X],
                                                bds[UPPER][X]);
                                        phyPin.clsLayerBound[UPPER][Y] =
                                            std::max(
                                                phyPin.clsLayerBound[UPPER][Y],
                                                bds[UPPER][Y]);
                                }  // end for
                        ...
                        if (!lefPortGeo.clsLefPolygonDscp.empty()) {
                                ...
                                phyPin.clsLayerBound[UPPER] =
                                    DBUxy(std::numeric_limits<DBU>::min(),
                                          std::numeric_limits<DBU>::min());
                                phPinLayer->clsPolygons.reserve(
                                    lefPortGeo.clsLefPolygonDscp.size());
                                for (const LefPolygonDscp &poly :
                                     lefPortGeo.clsLefPolygonDscp) {
                                        phPinLayer->clsPolygons.resize(
                                            phPinLayer->clsPolygons.size() + 1);
                                        Polygon &polygon =
                                            phPinLayer->clsPolygons.back();

                                        for (double2 point :
                                             poly.clsPolygonPoints) {
                                                point.scale(static_cast<double>(
                                                    getDatabaseUnits(
                                                        LIBRARY_DBU)));
                                                DBUxy pt(
                                                    static_cast<DBU>(
                ...
                if (phPinPort.getNumPinLayers() > 1) {
                        std::sort(phPinPort->clsPinLayers.begin(),
                                  phPinPort->clsPinLayers.end(),
                                  [](PhysicalPinLayer phPinLayer0,
                                     PhysicalPinLayer phPinLayer1) {
                                          Rsyn::PhysicalLayer phLayer0 =
                                              phPinLayer0.getLayer();
                                          Rsyn::PhysicalLayer phLayer1 =
                                              phPinLayer1.getLayer();
                                          return phLayer0.getIndex() <
                                                 phLayer1.getIndex();
                                  });  // end sort
                }                      // end if
        }                              // end for
...
void PhysicalDesign::addPhysicalCell(Rsyn::Instance cell,
                                     const DefComponentDscp &component) {
        PhysicalLibraryCellData &phLibCell =
            data->clsPhysicalLibraryCells[cell.asCell()
                                              .getLibraryCell()];  // TODO:
                                                                   // assuming
                                                                   // instance
                                                                   // is a cell
        ...
        tag.setMacroBlock(phLibCell.clsMacroClass == Rsyn::MACRO_BLOCK ||
                          phLibCell.clsMacroClass == Rsyn::MACRO_RING);
        if (component.clsIsFixed) {
                if (phLibCell.clsLayerBoundIndex > -1) {
                        PhysicalObstacle obs =
                            phLibCell.clsObs[phLibCell.clsLayerBoundIndex];
        ...
        if (physicalCell.clsBlock) data->clsNumElements[PHYSICAL_BLOCK]++;
        if (cell.isMovable()) data->clsNumElements[PHYSICAL_MOVABLE]++;
        physicalCell.clsHasLayerBounds = phLibCell.clsLayerBoundIndex > -1;
        physicalCell.clsInstance->clsOrientation =
            getPhysicalOrientation(component.clsOrientation);
        ...
        if (physicalCell.clsBlock) data->clsTotalAreas[PHYSICAL_BLOCK] += area;
        if (cell.isFixed()) {
                if (cell.isPort())
...
void PhysicalDesign::addPhysicalPort(Rsyn::Instance cell,
                                     const DefPortDscp &port) {
        ...
        physicalGate.clsInstance->clsOrientation =
            getPhysicalOrientation(port.clsOrientation);
...
void PhysicalDesign::addPhysicalRow(const DefRowDscp &defRow) {
        ...
        if (!phSite)
                throw Exception("Site " + defRow.clsSite +
                                " was not find for row " + defRow.clsName);

        // Creates a new cell in the data structure.
        PhysicalRowData *phRowData =
            &(data->clsPhysicalRows.create()->value);  // TODO: awful
        phRowData->id = data->clsPhysicalRows.lastId();
        phRowData->clsRowName = defRow.clsName;
        phRowData->clsPhysicalSite = phSite;
        phRowData->clsSiteOrientation =
            Rsyn::getPhysicalOrientation(defRow.clsOrientation);
        ...
        phRowData->clsBounds[UPPER][X] =
            defRow.clsOrigin[X] + phRowData->getWidth();
        phRowData->clsBounds[UPPER][Y] =
            defRow.clsOrigin[Y] + phRowData->getHeight();
        Rsyn::PhysicalModule phModule = getPhysicalModule(data->clsModule);
        Bounds &bounds = phModule->clsInstance->clsBounds;
        bounds[LOWER] = min(bounds[LOWER], phRowData->clsBounds[LOWER]);
        bounds[UPPER] = max(bounds[UPPER], phRowData->clsBounds[UPPER]);
        data->clsTotalAreas[PHYSICAL_PLACEABLE] +=
            phRowData->clsBounds.computeArea();
}  // end method

// -----------------------------------------------------------------------------

void PhysicalDesign::addPhysicalRegion(const DefRegionDscp &defRegion) {
        data->clsPhysicalRegions.push_back(
            PhysicalRegion(new PhysicalRegionData()));
...
void PhysicalDesign::addPhysicalGroup(const DefGroupDscp &defGroup) {
        data->clsPhysicalGroups.push_back(
            PhysicalGroup(new PhysicalGroupData()));
...
void PhysicalDesign::addPhysicalNet(const DefNetDscp &netDscp) {
        ...
        for (const DefWireDscp &wireDscp : netDscp.clsWires) {
                for (const DefWireSegmentDscp &segmentDscp :
                     wireDscp.clsWireSegments) {
                        Rsyn::PhysicalLayer physicalLayer =
                            getPhysicalLayerByName(segmentDscp.clsLayerName);

                        int numPoints = segmentDscp.clsRoutingPoints.size();
                        Rsyn::PhysicalRoutingWire wire;
                        for (const DefRoutingPointDscp &point :
                             segmentDscp.clsRoutingPoints) {
                                if (point.clsHasRectangle) {
                                        ...
                                        DBUxy pos =
                                            segmentDscp.clsRoutingPoints.back()
                                                .clsPos;
                        ...
                        if (numPoints > 1) {
                                ...
                                const DefRoutingPointDscp &source =
                                    segmentDscp.clsRoutingPoints.front();
                                const DefRoutingPointDscp &target =
                                    segmentDscp.clsRoutingPoints.back();
                                if (source.clsHasExtension)
                                        wire.setSourceExtension(
                                            source.clsExtension);
                                if (target.clsHasExtension)
                                        wire.setTargetExtension(
                                            target.clsExtension);
...
void PhysicalDesign::addPhysicalSpecialNet(
    const DefSpecialNetDscp &specialNet) {
...
///-----------------------------------------------------------------------------
//
// void PhysicalDesign::addSpecialWireNet(const DefWireDscp & wire, PhysicalWire
// phWire, const bool isSpecialNet) {
//	phWire->clsWireSegments.reserve(wire.clsWireSegments.size());
//	for (const DefWireSegmentDscp & segmentDscp : wire.clsWireSegments) {
//		phWire->clsWireSegments.push_back(PhysicalWireSegment(new
//PhysicalWireSegmentData()));
//		PhysicalWireSegment phWireSegment =
//phWire->clsWireSegments.back();
...
///-----------------------------------------------------------------------------
//
// void PhysicalDesign::addWireSegment(const DefWireSegmentDscp & segmentDscp,
// PhysicalWireSegment phWireSegment, const bool isSpecialNet) {
//	phWireSegment->clsPhysicalLayer =
//getPhysicalLayerByName(segmentDscp.clsLayerName);
...
//
//	for (const DefRoutingPointDscp & routingPoint :
//segmentDscp.clsRoutingPoints) {
//		phWireSegment->clsRoutingPoints.push_back(PhysicalRoutingPoint(new
//PhysicalRoutingPointData()));
//		PhysicalRoutingPoint phPoint =
//phWireSegment->clsRoutingPoints.back();
//		phPoint->clsExtension = routingPoint.clsExtension;
//		phPoint->clsPos = routingPoint.clsPos;
//		phPoint->clsOrientation =
//getPhysicalOrientation(routingPoint.clsOrientation);
//		if (routingPoint.clsHasVia)
//			phPoint->clsVia =
//getPhysicalViaByName(routingPoint.clsViaName);
//
...
void PhysicalDesign::addPhysicalTracks(const DefTrackDscp &track) {
        data->clsPhysicalTracks.push_back(
            PhysicalTracks(new PhysicalTracksData()));
        Rsyn::PhysicalTracks phTrack = data->clsPhysicalTracks.back();
        phTrack->id = data->clsPhysicalTracks.size() - 1;
        phTrack->clsDirection =
            Rsyn::getPhysicalTrackDirectionDEF(track.clsDirection);
        ...
        for (const std::string &layerName : track.clsLayers) {
                ...
                if (phLayer) {
                        ...
                        std::vector<Rsyn::PhysicalTracks> &trackLayer =
                            data->clsMapLayerToTracks[phLayer];
...
void PhysicalDesign::initRoutingGrid() {
        for (Rsyn::PhysicalTracks track : allPhysicalTracks()) {
                for (Rsyn::PhysicalLayer phLayer : track.allLayers()) {
                        if (data->clsMapLayerToRoutingGrid.find(phLayer) ==
                            data->clsMapLayerToRoutingGrid.end()) {
                                data->clsPhysicalRoutingGrids.push_back(
                                    PhysicalRoutingGrid(
                                        new PhysicalRoutingGridData()));
                                data->clsMapLayerToRoutingGrid[phLayer] =
                                    data->clsPhysicalRoutingGrids.back();
                        }  // end if
                        Rsyn::PhysicalRoutingGrid routingGrid =
                            data->clsMapLayerToRoutingGrid[phLayer];
        ...
        // to use std::sort algorithm, the operator int() const in Proxy must be
        // public. Otherwise, there is a compiler error
        data->clsPhysicalRoutingGrids.clear();
        for (Rsyn::PhysicalLayer layer : allPhysicalLayers()) {
                if (!hasPhysicalRoutingGrid(layer)) continue;
                PhysicalRoutingGrid routing =
                    data->clsMapLayerToRoutingGrid[layer];
...
void PhysicalDesign::addPhysicalDesignVia(const DefViaDscp &via) {
        ...
        if (via.clsHasViaRule) {
                ...
                phVia->clsEnclosure[BOTTOM_VIA_LEVEL][X] =
                    via.clsXBottomEnclosure;
                phVia->clsEnclosure[BOTTOM_VIA_LEVEL][Y] =
                    via.clsYBottomEnclosure;
                ...
                std::vector<std::tuple<int, PhysicalLayerData *>> layers;
                for (const std::pair<std::string,
                                     std::deque<DefViaGeometryDscp>> &geoPair :
                     via.clsGeometries) {
                        const std::string &layerName = geoPair.first;
                        Rsyn::PhysicalLayer layer =
                            getPhysicalLayerByName(layerName);
                ...
                for (int i = 0; i < NUM_VIA_LAYERS; i++) {
                        ...
                        auto iterator =
                            via.clsGeometries.find(layerData->clsName);
                        const std::deque<DefViaGeometryDscp> &geoDscps =
                            iterator->second;
                        viaGeos.reserve(geoDscps.size());
...
void PhysicalDesign::addPhysicalSpacing(const LefSpacingDscp &spacing) {
        Element<PhysicalSpacingData> *element =
            data->clsPhysicalSpacing.create();
        ...
        phSpacing->clsDistance = static_cast<DBU>(
            std::round(spacing.clsDistance * getDatabaseUnits(LIBRARY_DBU)));
}  // end method

// -----------------------------------------------------------------------------

Rsyn::PhysicalLayer PhysicalDesign::getPhysicalLayerByIndex(const int index) {
        const int numLayers = data->clsPhysicalLayers.size();
        if (index < 0 || index >= numLayers) return PhysicalLayer(nullptr);
        Element<PhysicalLayerData> *phLayerDataElement =
            data->clsPhysicalLayers.get(index);
        return PhysicalLayer(&phLayerDataElement->value);
}  // end method

// -----------------------------------------------------------------------------

Rsyn::PhysicalLayer PhysicalDesign::getPhysicalLayerByIndex(
    const Rsyn::PhysicalLayerType layerType, const int index) {
        if (index < 0) return PhysicalLayer(nullptr);
        int numLayers;
        int layerId;
        Element<PhysicalLayerData> *phLayerDataElement;
        switch (layerType) {
                case PhysicalLayerType::ROUTING:
                        ...
                        if (index >= numLayers) return PhysicalLayer(nullptr);
                        layerId = data->clsPhysicalRoutingLayerIndeces[index];
                        phLayerDataElement =
                            data->clsPhysicalLayers.get(layerId);
                        return PhysicalLayer(&phLayerDataElement->value);
                        break;
                default:
...
void PhysicalDesign::updatePhysicalCell(Rsyn::Cell cell) {
        PhysicalInstanceData &phCell =
            data->clsPhysicalInstances[cell.asCell()];
        PhysicalLibraryCellData &phLibCell =
            data->clsPhysicalLibraryCells[cell.asCell()
                                              .getLibraryCell()];  // TODO:
                                                                   // assuming
        ...
        DBU area =
            (width * height) - phCell.clsInstance->clsBounds.computeArea();
        ...
        phCell.clsInstance->clsBounds.updatePoints(
            pos, DBUxy(pos[X] + width, pos[Y] + height));
}  // end method

// -----------------------------------------------------------------------------

void PhysicalDesign::removePhysicalCell(Rsyn::Cell cell) {
        PhysicalInstanceData &physicalCell =
            data->clsPhysicalInstances[cell.asCell()];
...
void PhysicalDesign::mergeBounds(const std::vector<Bounds> &source,
        ...
        for (const Bounds &bound : source) {
                ...
                lower[reverse] =
                    std::min(bound[LOWER][reverse], lower[reverse]);
                upper[reverse] =
                    std::max(bound[UPPER][reverse], upper[reverse]);
        }  // end for
...
void PhysicalDesign::initLayerViaManager() {
        for (PhysicalVia via : allPhysicalVias()) {
                ...
                std::vector<PhysicalVia> &bottomAll =
                    data->clsLayerViaManager.clsVias[bottom];
                std::vector<PhysicalVia> &bottomTop =
                    data->clsLayerViaManager.clsTopVias[bottom];
                bottomAll.push_back(via);
                bottomTop.push_back(via);

                std::vector<PhysicalVia> &topAll =
                    data->clsLayerViaManager.clsVias[top];
                std::vector<PhysicalVia> &topBottom =
                    data->clsLayerViaManager.clsBottomVias[top];
...
bool PhysicalDesign::checkEquivalentOrientations(
    Rsyn::PhysicalSymmetry symmetry, Rsyn::PhysicalOrientation orient1,
