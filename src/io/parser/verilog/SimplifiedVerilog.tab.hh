...
struct variant {
        ...
        /// Construct and fill.
        template <typename T>
        variant(const T& t) : yytypeid_(&typeid(T)) {
                YYASSERT(sizeof(T) <= S);
                new (yyas_<T>()) T(t);
        }
...
class SimplifiedVerilogParser {
       public:
        ...
        struct basic_symbol : Base {
                ...
                /// Constructor for valueless symbols, and symbols from each
                /// type.
        ...
        struct by_type {
                ...
                /// An int, not token_number_type, to be able to store
                /// empty_symbol.
                int type;
        };
        ...
        static inline symbol_type make_INTEGER(const int& v,
                                               const location_type& l);

        static inline symbol_type make_MODULE(const location_type& l);

        static inline symbol_type make_END_MODULE(const location_type& l);

        static inline symbol_type make_INPUT(const location_type& l);

        static inline symbol_type make_OUTPUT(const location_type& l);

        static inline symbol_type make_WIRE(const location_type& l);

        static inline symbol_type make_IDENTIFIER(const std::string& v,
                                                  const location_type& l);

        static inline symbol_type make_CHAR(const location_type& l);

        /// Build a parser object.
        SimplifiedVerilogParser(SimplifiedVerilogScanner& scanner_yyarg,
