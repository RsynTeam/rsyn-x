...
int lefMacroEndCB(lefrCallbackType_e c, const char* macroName,
                  lefiUserData ud) {
        return 0;
}  // end function
...
int lefPinCB(lefrCallbackType_e c, lefiPin* pin, lefiUserData ud) {
        ...
        typedef boost::polygon::polygon_traits<Polygon90>::point_type
            BoostPoint;
        ...
        if (lefPin.clsPinDirection.compare("INOUT") == 0) {
                ...
                if (numWarningsInoutPins < 10)
                        std::cout << "WARNING: Unsupported INOUT direction in "
                                     "data pin. "
                                  << lefPin.clsPinName
        ...
        for (int j = 0; j < pin->numPorts(); j++) {
                ...
                for (int i = 0; i < numGeo; i++) {
                        ...
                        std::vector<boost::polygon::rectangle_data<double> >
                            rects;
                        Polygon90 polygon90;
                        std::vector<BoostPoint> pts;
                        switch (geoType) {
                                case lefiGeomLayerE:
                                        lefPort.clsLefPortGeoDscp.push_back(
                                            LefPortGeometryDscp());
                                        geoDscp =
                                            &lefPort.clsLefPortGeoDscp.back();
                                        geoDscp->clsMetalName =
                                            geometry->lefiGeometries::getLayer(
                                                i);
                                        break;
                                case lefiGeomRectE:
                                        rect = geometry->getRect(i);
                                        geoDscp->clsBounds.resize(
                                            geoDscp->clsBounds.size() + 1);
                                        bound = &geoDscp->clsBounds.back();
                                        bound->updatePoints(rect->xl, rect->yl,
                                                            rect->xh, rect->yh);
                                        break;
                                case lefiGeomPolygonE:
                                        ...
                                        for (int k = 0; k < poly->numPoints;
                                             k++) {
                                                pts.push_back(
                                                    boost::polygon::construct<
                                                        BoostPoint>(
                                                        poly->x[k],
                                                        poly->y[k]));
                                        }  // end for

                                        boost::polygon::set_points(
                                            polygon90, pts.begin(), pts.end());
                                        boost::polygon::get_rectangles(
                                            rects, polygon90);
                                        for (int k = 0; k < rects.size(); k++) {
                                                ...
                                                ;
                                                geoDscp->clsBounds.resize(
                                                    geoDscp->clsBounds.size() +
                                                    1);
                                                bound =
                                                    &geoDscp->clsBounds.back();
                                                bound->updatePoints(xl, yl, xh,
                                                                    yh);
                                        }  // end for
                                ...
                                default:
                                        std::cout << "WARNING: function "
                                                  << __func__
                                                  << " does not supports pin "
                                                     "geometry type in the LEF "
                                                     "Parser Control.\n";
                                        break;
                        }  // end switch
                }          // end for
...
int lefObstructionCB(lefrCallbackType_e c, lefiObstruction* obs,
        ...
        for (int i = 0; i < numItems; i++) {
                ...
                } else if (geometry->lefiGeometries::itemType(i) ==
                           lefiGeomRectE) {
                        LefObsDscp& lefObs = lefMacro.clsObs.back();
                        rect = geometry->lefiGeometries::getRect(i);
                        DoubleRectangle libRect = DoubleRectangle(
...
int lefViaCb(lefrCallbackType_e typ, lefiVia* via, lefiUserData data) {
        ...
        if (viaDscp.clsHasViaRule) {
                ...
                for (int i = 0; i < via->numLayers(); ++i) {
                        ...
                        std::deque<LefViaGeometryDscp>& geoDscps =
                            viaDscp.clsGeometries[layerName];
