...
class SdcParser {
       ...
       public:
        ...
        SdcParser(string filename) : is(filename.c_str()) {
                if (!is) {
                        std::cout << "ERROR: SDC file '" << filename
                                  << "' could not be opened.\n";
                        std::exit(1);
                }  // end if
        }          // end constructor
...
class TimingParser {
       ...
       public:
        ...
        // Return value indicates if the last read was successful or not.
        // If the line read corresponds to a pin, then name1 and name2 will be
        // set to the cell
        // instance name and the pin name, respectively.
        // If the line read corresponds to a port, then name1 will be set to the
        // port name, and
        ...
        bool read_timing_tau15(string& str1, string& str2, double& val1,
...
class OperationsParser {
       ...
       public:
        ...
        bool read_ops_tau15(ISPD13::CommandTypeEnum& cmdType,
                            ISPD13::QueryTypeEnum& queryType, string& pin,
                            string& net, string& file, string& gate,
                            string& cell, bool& early, bool& rise, int& paths);
};
...
struct LibParserLUT {
        ...
        double getMaxLoad() const {
                return loadIndices[loadIndices.size() - 1];
        }
        double getMinLoad() const { return loadIndices[0]; }
        double getMaxTransition() const {
                return transitionIndices[transitionIndices.size() - 1];
        }
...
struct LibParserTimingInfo {
        string fromPin;
        string toPin;
        string timingSense;  // "non_unate" or "negative_unate" or
                             // "positive_unate".
        // Note that ISPD-13 library will have only negative-unate combinational
        // cells. The clock arcs
        // for sequentials will be non_unate (which can be ignored because of
        // the simplified sequential
...
struct LibParserPinInfo {
        ...
        LibParserPinInfo()
            : capacitance(0.0),
              ...
              maxTransition(-std::numeric_limits<double>::infinity()),
              isInput(true),
              isClock(false),
              isTimingEndpoint(false),
              risingEdge(false) {}
};
...
struct LibParserCellInfo {
        ...
        string
            footprint;  // only the cells with the same footprint are swappable
        ...
        bool isTieLow;  // Tie are cells that keeps output constantly in VDD or
                        // GND
        bool isTieHigh;
...
class LibParser {
        ...
        void _begin_read_timing_info(string pinName, LibParserPinInfo& pin,
                                     LibParserTimingInfo& cell);
