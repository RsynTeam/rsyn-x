...
struct Design {
        ...
        struct GCellGrid {
                ...
                int doStep;
                GCellGrid()
                    : master("null"), doStart(-1), doCount(-1), doStep(-1) {}
        };
...
struct DefStruct1 {
        ...
        struct DefRow {
                ...
                DefRow()
                    : rowType("core"),
                      ...
                      xStep(0),
                      yStep(0) {}
                // orient {0, 1, 2, 3, 4, 5, 6, 7} = {N, W, S, E, FN, FW, FS,
                // FE} source: defapi.pdf version 5.8
        };
        ...
        struct DefPortLayer {
                ...
                Bounds bounds;
                DefPortLayer()
                    : spacing(0), designRuleWidth(0), layerName("NULL") {}
        };
        ...
        struct DefPort {
                ...
                std::string
                    direction;    // {NULL, FEEDTHRU, INPUT, INOUT, OUTPUT}
                std::string use;  // {NULL, ANALOG, CLOCK, GROUND, POWER, RESET,
                                  // SCAN, SIGNAL, TIEOFF}
                std::string status;  // {NULL, COVER, FIXED, PLACED}
                int statusX;         // placement location; 0 ignore
                int statusY;         // placement location; 0 ignore
                int orient;  // orient {-1, 0, 1, 2, 3, 4, 5, 6, 7} = {IGNORE,
                             // N, W, S, E, FN, FW, FS, FE} source: defapi.pdf
                             // version 5.8
