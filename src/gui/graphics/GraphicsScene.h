...
public:
	...
	void redraw(QGraphicsScene::SceneLayers layers = QGraphicsScene::AllLayers);

	//! @brief Update a specific a region of the scene. The region, which is
	//! given in scene coordinates, is first mapped to view coordinates and then
	//! expanded by the number of pixels defined by expandBy. This is useful
	//! when using cosmetic outlines (i.e. outline whose width is invariant to
	//! the view scaling) to avoid artifacts around the borders of shapes.
	void redraw(const QRectF &rect, QGraphicsScene::SceneLayers layers, const int expandBy = 0);
	...
	//! @brief Returns the level-of-detail (lod) to be used to render the scene.
	//! The lod is defined as the value of of the typical length (default = 1)
	...
	//! @brief Sets the typical scene length. This is used as a scaling factor
	...
	void setHoverOutline(const QPainterPath &outline) {
		clsHover = true;
		redraw(clsHoverOutline.boundingRect(), QGraphicsScene::ForegroundLayer, clsHoverOutlineThickness);
	...
	void addHighlight(const QPainterPath &outline) {
		...
		redraw(outline.boundingRect(), QGraphicsScene::BackgroundLayer, clsHighligthOutlineThickness);
	} // end method
	...
	void addDrawing(const QPainterPath &outline) {
		...
		redraw(outline.boundingRect(), QGraphicsScene::BackgroundLayer, clsDrawingOutlineThickness);
	} // end method
	...
	//! hierarchical. For instance, "Parent/Child" may return false if either
	...
	//! @brief Sets the visibility of an object. The key can be hierarchical,
