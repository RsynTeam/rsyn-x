...
void
GraphicsScene::redraw(QGraphicsScene::SceneLayers layers) {
	...
	for (QGraphicsView *view : views()) {
		const QRect viewRect = QRect(0, 0,
				view->viewport()->size().width(),
				view->viewport()->size().height());
		const QRectF mappedViewRect = view->mapToScene(viewRect).boundingRect();
...
void
GraphicsScene::redraw(const QRectF &rect, QGraphicsScene::SceneLayers layers, const int expandBy) {
	for (QGraphicsView *view : views()) {
		const QRect mappedRect = view->mapFromScene(rect).boundingRect();
		const QRect adjustedMappedRect = mappedRect.adjusted(
				-expandBy, -expandBy, +expandBy, +expandBy);
		const QRectF adjustedRect = view->mapToScene(adjustedMappedRect).boundingRect();
...
float 
GraphicsScene::getLevelOfDetail(QPainter *painter) const {
	const QTransform &transform = painter->worldTransform();

	const QRectF typicalRectInViewportCoordinates =
			transform.mapRect(QRectF(0, 0, getTypicalLength(), getTypicalLength()));

	return std::min(
			typicalRectInViewportCoordinates.width(),
...
void
GraphicsScene::ensureVisible(const QRectF &rect) {
	for (QGraphicsView *view : views()) {
...
GraphicsItem *
GraphicsScene::getItemAt(const qreal x, const qreal y) const {
	const int numLayers = clsLayers.size();
...
GraphicsItem *
GraphicsScene::getItemAt(const QPointF &point) const {
...
bool
GraphicsScene::getVisibility(const std::string &key) const {
...
void
GraphicsScene::setVisibility(const std::string &key, const bool visible) {
