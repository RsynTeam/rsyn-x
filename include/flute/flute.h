...
// extern Tree flautist(int d, DTYPE x[], DTYPE y[], int acc, const
// uofm::vector<BBox> &obs, const uofm::vector<unsigned> &relevantObs, unsigned
// &legal);

// Other useful functions
extern FLUTE_DTYPE flutes_wl_LD(int d, FLUTE_DTYPE xs[], FLUTE_DTYPE ys[],
                                int s[]);
extern FLUTE_DTYPE flutes_wl_MD(int d, FLUTE_DTYPE xs[], FLUTE_DTYPE ys[],
                                int s[], int acc);
extern FLUTE_DTYPE flutes_wl_RDP(int d, FLUTE_DTYPE xs[], FLUTE_DTYPE ys[],
                                 int s[], int acc);
extern Tree flutes_LD(int d, FLUTE_DTYPE xs[], FLUTE_DTYPE ys[], int s[]);
extern Tree flutes_MD(int d, FLUTE_DTYPE xs[], FLUTE_DTYPE ys[], int s[],
                      int acc);
extern Tree flutes_RDP(int d, FLUTE_DTYPE xs[], FLUTE_DTYPE ys[], int s[],
                       int acc);
...
#define flutes_wl_LMD(d, xs, ys, s, acc)           \
        (d <= FLUTE_D ? flutes_wl_LD(d, xs, ys, s) \
                      : flutes_wl_MD(d, xs, ys, s, acc))
#define flutes_LMD(id, d, xs, ys, s, acc)       \
        (d <= FLUTE_D ? flutes_LD(d, xs, ys, s) \
                      : flutes_MD(id, d, xs, ys, s, acc))
...
inline double generation_cost(const int n) {
        static const double COST_UP_TO_20[] = {
            0.0000000000, 0.0000000000,  0.0465398638,  0.0641802706,
            0.0809460239, 0.0997421556,  0.1197798820,  0.1448438153,
            0.1763411899, 0.2461886417,  1.0000000000,  1.2675247404,
            2.3991262851, 3.4054468756,  4.9176023987,  6.3498472641,
            8.1995467193, 10.2411162153, 12.5221830672, 14.9206758856,
            17.5287992755};
