...
extern int defwRow(const char* rowName, const char* rowType, int x_orig,
                   int y_orig, int orient, int do_count, /* optional (0) */
 ...
 * NOTE: Use defwPinLayer to write out layer with SPACING or DESIGNRULEWIDTH */
extern int defwPinStr(
    const char* name,         /* pinName */
    const char* net,          /* netName */
    int special,              /* 0 - ignore, 1 - special */
    const char* direction,    // optional(NULL) - INPUT | OUTPUT |
                              // INOUT | FEEDTHRU
...
extern int defwPinAntennaPinPartialMetalArea(
    int value, const char* layerName); /* optional(NULL) */
 ...
 * any order. */
extern int defwSpecialNetPathStart(
    const char* typ);  // ROUTED | FIXED | COVER |
                       // SHIELD | NEW
...
extern int defwScanchainOrdered(
    const char* name1, const char* inst1, /* optional(NULL) - IN | OUT */
    const char* pin1,                     /* can't be null if inst1 is set */
    const char* inst2,                    /* optional(NULL) - IN | OUT */
    const char* pin2,                     /* can't be null if inst2 is set */
...
extern int defwConstraintOperandPath(const char* comp1,   /* PATH - comp|PIN */
                                     const char* fromPin, const char* comp2,
                                     const char* toPin);
...
extern int defwGroupSoft(const char* type1, /* MAXHALFPERIMETER | MAXX | MAXY */
                         double value1, const char* type2, double value2,
                         const char* type3, double value3);
