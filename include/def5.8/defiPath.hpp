...
class defiPath {
       public:
        ...
        void initTraverseBackwards()
            const;                     // Initialize the traverse in reverse.
        ...
        void getViaRect(int* deltaX1, int* deltaY1, int* deltaX2,
                        int* deltaY2) const;
        const char* getViaRotationStr()
            const;  // Return via rotation in string format
        void getViaData(int* numX, int* numY, int* stepX,
                        int* stepY) const;                   // 5.4.1
