...
EXTERN int defwInit(FILE* f, int vers1, int version2, const char* caseSensitive,
                    const char* dividerChar, const char* busBitChars,
                    const char* designName, const char* technology,
                    const char* array, const char* floorplan, double units);
...
EXTERN int defwIntPropDef(const char* objType, const char* propName,
                          double leftRange, double rightRange, int propValue);
 ...
 * The objType can be LIBRARY or VIA or MACRO or PIN. */
/* NONDEFAULTRULE | MACRO | PIN         */
EXTERN int defwRealPropDef(const char* objType, const char* propName,
 ...
 * The objType can be LIBRARY or VIA or MACRO or PIN. */
/* NONDEFAULTRULE | MACRO | PIN        */
EXTERN int defwStringPropDef(const char* objType, const char* propName,
                             double leftRange, double rightRange,
...
EXTERN int defwRow(const char* rowName, const char* rowType, int x_orig,
                   int y_orig, int orient, int do_count, int do_increment,
                   int xstep, int ystep);
...
EXTERN int defwRowStr(const char* rowName, const char* rowType, int x_orig,
                      int y_orig, const char* orient, int do_count,
                      int do_increment, int xstep, int ystep);

/* This routine must be called after the defwRow (if any).
 * This routine is optional.
 * Returns 0 if successful.
 * The operation of the do is explained in the documentation. */
EXTERN int defwTracks(const char* master, int doStart, int doCount, int doStep,
                      int numLayers, const char** layers, int mask,
                      int sameMask);
 ...
 * Returns 0 if successful.
 * The operation of the do is explained in the documentation. */
EXTERN int defwGcellGrid(const char* master, int doStart, int doCount,
                         int doStep);
...
EXTERN int defwCanPlace(const char* master, int xOrig, int yOrig, int orient,
                        int doCnt, int doInc, int xStep, int yStep);
...
EXTERN int defwCanPlaceStr(const char* master, int xOrig, int yOrig,
                           const char* orient, int doCnt, int doInc, int xStep,
                           int yStep);
 ...
 * Returns 0 if successful.
 * The routine can be called many times. */
EXTERN int defwCannotOccupy(const char* master, int xOrig, int yOrig,
                            int orient, int doCnt, int doInc, int xStep,
                            int yStep);
...
EXTERN int defwCannotOccupyStr(const char* master, int xOrig, int yOrig,
                               const char* orient, int doCnt, int doInc,
                               int xStep, int yStep);
...
/* This routine can be called multiple times. */
/* mask is 5.8 syntax */
EXTERN int defwViaRect(const char* layerName, int xl, int yl, int xh, int yh,
                       int mask);

/* This is a 5.6 syntax
 * This routine can be called multiple times. */
/* mask is 5.8 syntax */
EXTERN int defwViaPolygon(const char* layerName, int num_polys, double* xl,
                          double* yl, int mask);
...
EXTERN int defwViaViarule(const char* viaRuleName, double xCutSize,
                          double yCutSize, const char* botMetalLayer,
                          const char* cutLayer, const char* topMetalLayer,
 ...
 * This is a 5.6 syntax.
 * Returns 0 if successful. */
EXTERN int defwViaViaruleOffset(int xBotOffset, int yBotOffset, int xTopOffset,
                                int yTopOffset);
...
/*  PLACED | UNPLACED             */
/* xh, yh or region               */
EXTERN int defwComponent(const char* instance, const char* master,
                         int numNetName, const char** netNames, const char* eeq,
                         const char* genName, const char* genParemeters,
                         const char* source, int numForeign,
                         const char** foreigns, int* foreignX, int* foreignY,
                         int* foreignOrients, const char* status, int statusX,
                         int statusY, int statusOrient, double weight,
                         const char* region, int xl, int yl, int xh, int yh);
...
/* PLACED | UNPLACED              */
/* xh, yh or region               */
EXTERN int defwComponentStr(const char* instance, const char* master,
                            int numNetName, const char** netNames,
                            const char* eeq, const char* genName,
                            const char* genParemeters, const char* source,
                            int numForeign, const char** foreigns,
                            int* foreignX, int* foreignY,
                            const char** foreignOrients, const char* status,
                            int statusX, int statusY, const char* statusOrient,
                            double weight, const char* region, int xl, int yl,
                            int xh, int yh);
...
/* COVER                             */
EXTERN int defwPin(const char* name, const char* net, int special,
                   const char* direction, const char* use, const char* status,
                   int statusX, int statusY, int orient, const char* layer,
...
/* GROUND | CLOCK | TIEOFF | ANALOG  */
/* COVER                             */
EXTERN int defwPinStr(const char* name, const char* net, int special,
                      const char* direction, const char* use,
                      const char* status, int statusX, int statusY,
                      const char* orient, const char* layer, int xl, int yl,
                      int xh, int yh);
...
EXTERN int defwPinLayer(const char* layerName, int spacing, int designRuleWidth,
                        int xl, int yl, int xh, int yh, int mask);
...
EXTERN int defwPinPolygon(const char* layerName, int spacing,
                          int designRuleWidth, int num_polys, double* xl,
                          double* yl, int mask);
 ...
 * Returns 0 if successful.
 * This routine can be called multiple times within a pin. */
EXTERN int defwPinPortLayer(const char* layerName, int spacing,
                            int designRuleWidth, int xl, int yl, int xh, int yh,
                            int mask);
 ...
 * Returns 0 if successful.
 * The routine can be called multiple times within a pin. */
EXTERN int defwPinPortPolygon(const char* layerName, int spacing,
                              int designRuleWidth, int num_polys, double* xl,
                              double* yl, int mask);
 ...
 * The routine can be called many times.
 * NOTE: Use defwPinLayer to write out layer with SPACING or DESIGNRULEWIDTH */
EXTERN int defwPinPortLocation(const char* status, int statusX, int statusY,
                               const char* orient);
...
EXTERN int defwSpecialNetConnection(const char* inst, const char* pin,
                                    int synthesized);
...
EXTERN int defwSpecialNetSpacing(const char* layer, int spacing,
...
/* x and y location of the path */
EXTERN int defwSpecialNetPathPointWithWireExt(int numPts, double* pointx,
...
/* This is a 5.6 syntax
 * This routine can be called multiple times. */
EXTERN int defwSpecialNetPolygon(const char* layerName, int num_polys,
                                 double* xl, double* yl);

/* This is a 5.6 syntax
 * This routine can be called multiple times. */
EXTERN int defwSpecialNetRect(const char* layerName, int xl, int yl, int xh,
                              int yh);
...
/* x and y location of the path */
EXTERN int defwSpecialNetShieldPoint(int numPts, double* pointx,
                                     double* pointy);
EXTERN int defwSpecialNetShieldVia(const char* name);

/* A 5.4.1 syntax */
EXTERN int defwSpecialNetShieldViaData(int numX, int numY, int stepX,
                                       int stepY);
...
/* It can be called multiple times */
EXTERN int defwNetConnection(const char* inst, const char* pin,
                             int synthesized);
...
EXTERN int defwNetVpin(const char* vpinName, const char* layerName, int layerXl,
                       int layerYl, int layerXh, int layerYh,
                       const char* status, int statusX, int statusY,
                       int orient);
...
EXTERN int defwNetVpinStr(const char* vpinName, const char* layerName,
                          int layerXl, int layerYl, int layerXh, int layerYh,
                          const char* status, int statusX, int statusY,
                          const char* orient);
...
/* x and y location of the path */
EXTERN int defwNetNoshieldPoint(int numPts, const char** pointx,
                                const char** pointy);
...
EXTERN int defwNetPathLayer(const char* name, int isTaper,
                            const char* rulename);
...
EXTERN int defwNetPathPointWithExt(int numPts, double* pointx, double* pointy,
                                   double* optValue);
 ...
 * [PARALLEL numDrivers]
 * This api is obsolete in 5.4. */
EXTERN int defwIOTimingDrivecell(const char* name, const char* fromPin,
...
EXTERN int defwScanchainCommonscanpins(const char* inst1, const char* pin1,
 ...
 * This is a 5.4.1 syntax */
EXTERN int defwScanchainFloating(const char* name, const char* inst1,
                                 const char* pin1, const char* inst2,
                                 const char* pin2);
 ...
 * the option BITS. */
EXTERN int defwScanchainFloatingBits(const char* name, const char* inst1,
                                     const char* pin1, const char* inst2,
                                     const char* pin2, int bits);
...
EXTERN int defwScanchainOrdered(const char* name1, const char* inst1,
                                const char* pin1, const char* inst2,
                                const char* pin2, const char* name2,
 ...
 * the option BITS */
EXTERN int defwScanchainOrderedBits(const char* name1, const char* inst1,
                                    const char* pin1, const char* inst2,
                                    const char* pin2, int bits1,
                                    const char* name2, const char* inst3,
                                    const char* pin3, const char* inst4,
                                    const char* pin4, int bits2);
...
EXTERN int defwConstraintOperandPath(const char* comp1, const char* fromPin,
...
EXTERN int defwGroup(const char* groupName, int numExpr,
                     const char** groupExpr);

/* This routine is for + SOFT [MAXHALFPERIMETER value] [MAXX value]
 * [MAXY value] */
EXTERN int defwGroupSoft(const char* type1, double value1, const char* type2,
                         double value2, const char* type3, double value3);

/* This routine is for + REGION {pt pt | regionName} */
EXTERN int defwGroupRegion(int xl, int yl, int xh, int yh,
...
/* This routine is for - component
* This routine is called per entry within blockage placement, can't be more then
* one.
* This is a 5.4 syntax. */
 ...
 * This is a 5.6 syntax.
 * Returns 0 if successful. */
EXTERN int defwNonDefaultRuleLayer(const char* layerName, int width,
