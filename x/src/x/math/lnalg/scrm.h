...
class SparseMatrixDescriptor {
	...
	public:
		...
		// Otherwise returns false. If the matrix has a position that is infinity (inf) or has a 
...
class AbstractSquareCompressedRowMatrix {
	protected:
		...
		void Consolidate( const SparseMatrixDescriptor &dscp, const ConsolidationMode mode );
	...
	public:
		...
		int GetNumNonZeroEntries() const { return clsDig.size() + clsOff.size(); }
		int GetNummNonZeroOffDiagonalEntries() const { return clsOff.size(); }

		double GetDiagonalElement( const int index ) const { return clsDig[index]; }
		
		const vector<double> &GetDiagonalVector() const { return clsDig; }
		vector<double> &GetDiagonalVector() { return clsDig; }
		
		const vector<OffDiagonalElement> &GetOffDiagonalVector() const { return clsOff; }
		const vector<int> &GetRowPointerVector() const { return clsRow; }

		// Set the diagonal element value.
		void SetDiagonalElement( const int row, const double value ) { clsDig[row] = value; }
		void SetDiagonalVector( const vector<double> &diagonal ) { clsDig = diagonal; }

		// Add a value to an diagonal entry.
		void AddDiagonalElement( const int row, const double value ) { clsDig[row] += value; }
		...
		void SetElement(const int col, const int row, const double value) {
			if (col == row) {
				...
				throw SCRMException("Element must already be initialized in order to be updated.");
			} // end else
		} // end method
		...
		virtual void Initialize( const SparseMatrixDescriptor &dscp, const bool checkSymmetry = true ) = 0;
...
class SquareCompressedRowMatrix : public AbstractSquareCompressedRowMatrix {
	...
	friend void Mul( const SquareCompressedRowMatrix &A, const vector< double > &x, vector< double > &b );
	friend void Transpose( const SquareCompressedRowMatrix &A, SquareCompressedRowMatrix &B);
	friend void Forward( const SquareCompressedRowMatrix &A, const vector< double > &x, vector< double > &b);
	friend void Backward( const SquareCompressedRowMatrix &A, const vector< double > &x, vector< double > &b);

	friend void Mul( const SquareCompressedRowMatrix &A, const vector< double2 > &x, vector< double2 > &b );
	friend void Forward( const SquareCompressedRowMatrix &A, const vector< double2 > &x, vector< double2 > &b);
	friend void Backward( const SquareCompressedRowMatrix &A, const vector< double2 > &x, vector< double2 > &b);
	...
	public:
		...
		// Initialize.
		virtual void Initialize( const SparseMatrixDescriptor &dscp, const bool checkSymmetry = true );
}; // end class

// -----------------------------------------------------------------------------

class SymmetricCompressedRowMatrix : public AbstractSquareCompressedRowMatrix  {
	friend void Mul( const SymmetricCompressedRowMatrix &A, const vector< double > &d, const vector< double > &x, vector< double > &b );
	friend void Mul( const SymmetricCompressedRowMatrix &A, const vector< double > &x, vector< double > &b );

	friend void Mul( const SymmetricCompressedRowMatrix &A, const vector< double > &d, const vector< double2 > &x, vector< double2 > &b );
	friend void Mul( const SymmetricCompressedRowMatrix &A, const vector< double2 > &x, vector< double2 > &b );
...
public:
		...
		void SetElement(const int col, const int row, const double value) {
			if (row > col) {
				throw SCRMException("Symmetric matrices only store the elements in the upper part.");
			} else {
				AbstractSquareCompressedRowMatrix::SetElement(col, row, value);
			} // end else
		} // end method
		...
		void AddElement(const int col, const int row, const double value) {
			if (row > col) {
				...
				AbstractSquareCompressedRowMatrix::AddElement(col, row, value);
			} // end else
		} // end method		
		
		// Initialize.
		virtual void Initialize( const SparseMatrixDescriptor &dscp, const bool checkMatrix = true );
...
class SparseMatrixBlock {
	friend void Mul( const SparseMatrixBlock &A, const vector <double> &x, vector <double> &y );
	friend void Mul( const SparseMatrixBlock &A, const vector <double2> &x, vector <double2> &y );
    ...
    public:
        ...
        void mult_matriz_esparsa (const vector <double > &x, vector <double > &y) const; // a.(*x)= y
