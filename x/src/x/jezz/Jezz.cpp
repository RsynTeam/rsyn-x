...
const Jezz::ListLink Jezz::REVERSE_LIST_LINK[2] = {
	Jezz::TAIL,
	Jezz::HEAD
};
...
bool Jezz::jezz_LegalizeNode(JezzNode *jezzNode) {
	...
	// BUG FIX: jezz_InserNode requires also that the x position to be set to
	// the target x.
	// TODO: Make jezz_InsertNode use always the x position inside the Jezz node
	...
	double smallestMaxDisplacementCost = +std::numeric_limits<double>::max();
	...
	for (int row = initialRow; row >= 0; row--) {
		...
		if (!overflow) {
			if (cost < bestCost) {
				bestRow = row;
				bestCost = cost;
				smallestMaxDisplacementCost = currMaxDisplacementCost;
			} else if (clsJezzTieBreak && cost == bestCost) {
				if (currMaxDisplacementCost < smallestMaxDisplacementCost) {
					bestRow = row;
					smallestMaxDisplacementCost = currMaxDisplacementCost;
				} // end if
			} else {
...
bool Jezz::jezz_Legalize() {
	...
	for (int i = 0; i < numNodes; i++) {
		...
		if (!jezzNode->isDeleted() && !jezz_dp_IsLegalized(jezzNode))
			sortedCells.push_back(std::make_pair(jezzNode->refx, jezzNode));
	} // end for each
...
bool Jezz::jezz_Overlap(const int x, JezzNode *jezzNode) {
	// Note that we use "<" instead of "<=" in xmax is not the index of the last
...
Jezz::JezzNode *Jezz::jezz_FindOverlappingNodeCached(JezzRow *jezzRow, const int x) {
	...
	if (index != -1) {
		// Use the cached node as the starting point for the sequential search.
	...
	if (!jezz_ValidCachedNode(jezzRow, jezzNode)) {
		...
		while (!stop && (i0 >= 0 || i1 < numSlots)) {
			...
			if (i0 >= 0 && jezz_ValidCachedNode(jezzRow, jezzRow->slots[i0])) {
				...
				stop = true;
			} else if (i1 < numSlots && jezz_ValidCachedNode(jezzRow, jezzRow->slots[i1])) {
...
double Jezz::jezz_InsertNode(
	...
	if (!jezzReferenceNode) {
		...
		// This condition should not happen unless node is outside the core
		// bounds.
		if (true) {
			std::cout << "Jezz Debug:\n";
			std::cout << "Trying to place a cell out of the row bounds.\n";
			std::cout << "Note: This does not necessarily mean a bug inside Jezz neither means a bug elsewhere.\n";
			std::cout << "Note: Don't forget to check if the cell was not indeed outside the core bounds priori to legalization.\n";

			if (jezz_CheckRowConsistency(jezzRow)) {
				std::cout << "Info: Jezz row is consistent.\n";
			} else {
				std::cout << "Bug: Jezz row is inconsistent. This may explain the out of bounds issue...\n";
			} // end else

			std::cout << "row  (design coordinates): ";
			std::cout << "xmin=" << jezzRow->origin_x << " ";
			std::cout << "xmax=" << (jezzRow->origin_x + clsJezzNumSites * clsJezzSiteWidth) << "\n";
	...
	if (jezzReferenceNode->isWhitespace() && jezz_Enclosed(jezzReferenceNode, jezzNode)) {
		...
		if (!trial) {
			...
			if (ws0->width() == 0) jezz_RemoveZeroLengthWhitespace(ws0);
			if (ws1->width() == 0) jezz_RemoveZeroLengthWhitespace(ws1);
		} // end if
		...
		jezz_AdjustShiftingCostToSide(jezzRow, nodeToLeft, offsetRequired, HEAD, numSitesAvailableLeft, compoundOffsetToLeft);
		jezz_AdjustShiftingCostToSide(jezzRow, nodeToRight, offsetRequired, TAIL, numSitesAvailableRight, compoundOffsetToRight);

		//-------------------------------------
		//optimum shift
		const int maxOffsetLeft = offsetRequired - numSitesAvailableLeft;
		const int maxOffsetRight = offsetRequired - numSitesAvailableRight;
		...
		smallestMaxDisplacementCost = std::numeric_limits<double>::max();
		for (int i = maxOffsetLeft; i >= offsetRequired - maxOffsetRight; i--) {
		...
		if (!trial) {
			// We must insert the node before making room to it as some 
			...
			if (offsetToLeft > 0)
				jezz_AdjustPositionConsolidate(jezzRow, nodeToLeft, offsetToLeft, HEAD);

			if (offsetToRight > 0)
				jezz_AdjustPositionConsolidate(jezzRow, nodeToRight, offsetToRight, TAIL);

			// Note that after making room for the new node, node0 and node1 may
			// not be valid anymore as they may have been removed if they are 
...
void Jezz::jezz_AdjustShiftingCostToSide(
	...
	while (!jezzRow->isRoot(nodeInTheWay) && overflow > 0) {
		if (nodeInTheWay->isWhitespace()) {
			...
			for (int i = k + 1, shifts = dx; i <= numOfSitesRequired; i++, shifts += dx) {
				const DBU xDisplacement = std::abs(nodeInTheWay->x + shifts - nodeInTheWay->snapped_refx) * clsJezzSiteWidth;
				const DBU yDisplacement = std::abs(jezzRow->y - nodeInTheWay->snnaped_refy) * clsJezzRowHeight;
				const DBU displacement = xDisplacement + yDisplacement;
				const double weightedDisplacement = nodeInTheWay->weight * displacement;

				if (weightedDisplacement > sideCompoundShiftingCost[i].second) {
					sideCompoundShiftingCost[i].second = weightedDisplacement;
				} // end if
...
int Jezz::jezz_AdjustPositionConsolidate(JezzRow *jezzRow,
	...
	while (!jezzRow->isRoot(node) && overflow > 0) {
		...
		if (node->isWhitespace()) {
			...
			// Update node position globally. This should be done in a different 
			// way (maybe via a callback) as this makes the Jezz dependent on
			// the placer class. Also we don't need to keep global node
			// positions up-to-date during a full legalization. But for now 
...
bool Jezz::jezz_CheckRowConsistency_HeadToTail(JezzRow *jezzRow) {
	...
	if (w != jezzRow->width()) {
		std::cout << "[JEZZ EXCEPTION] h2t Inconsistent row ==>" << "Width: " << w << " != " << jezzRow->width() << std::endl;
		sucess = false;
	} // end if
...
bool Jezz::jezz_CheckRowConsistency(JezzRow *jezzRow) {
	...
	if (!head2tail) {
		std::cout << "Row is not consistent when traversing from head to tail.\n";
	} // end if

	if (!tail2head) {
		std::cout << "Row is not consistent when traversing from tail to head.\n";
	} // end if
...
void Jezz::jezz_StressTest() {
	...
	for (int i = 0; i < N; i++) {
		...
		if (!jezz_CheckRowConsistency(jezzNode->row))
			assert(false);
	} // end for

	for (int i = 0; i < numRows; i++) {
		if (!jezz_CheckRowConsistency(&clsJezzRows[i]))
			assert(false);
	} // end for	
} // end method
...
void Jezz::jezz_StressTest_CacheSystem() {
	...
	for (int i = 0; i < N; i++) {
		...
		JezzNode *jezzNode0 = jezz_FindOverlappingNodeNonCached(jezzRow, jezz_SnapPositionX(x));
		JezzNode *jezzNode1 = jezz_FindOverlappingNodeCached(jezzRow, jezz_SnapPositionX(x));
...
void Jezz::jezz_PrintNode(JezzNode *jezzNode, const std::string &lable) {
	...
	std::cout << "  " << "xmin" << " : " << jezzNode->xmin() << "\n";
	std::cout << "  " << "xmax" << " : " << jezzNode->xmax() << "\n";
	std::cout << "  " << "width" << " : " << jezzNode->width() << "\n";
	std::cout << "  " << "whitespace" << " : " << jezzNode->isWhitespace() << "\n";
	std::cout << "  " << "obstacle" << " : " << jezzNode->isObstacle() << "\n";
	std::cout << "  " << "fixed" << " : " << jezzNode->isFixed() << "\n";
	std::cout << "  " << "row" << " : " << jezzNode->row << "\n";
} // end method
...
void Jezz::jezz_dp_InsertNode(JezzNode *jezzNode, const DBU x, const DBU y, bool &overflow) {
	if (jezz_dp_IsLegalized(jezzNode)) {
		jezz_dp_RemoveNode(jezzNode);
	} // end if	
	...
	JezzRow *jezzRow = &clsJezzRows[row];
	jezz_InsertNode(jezzRow, jezzNode, jezz_SnapPositionX(x), false, overflow);
	if (!overflow) {
		jezz_UpdateCache(jezzNode);
	} // end if
} // end method
...
//void Jezz::jezz_dp_InsertNode(JezzNode *jezzNode, const WhitespaceDescriptor &ws) {
//	if (jezz_dp_IsLegalized(jezzNode)) {
//		jezz_dp_RemoveNode(jezzNode);
//	} // end if	
//
...
void Jezz::jezz_restoreSolutionFull(const std::string name) {
	...
	if (it == clsJezzStoredSolution.end()) {
		std::cout << "[WARNING] Stored Jezz solution '" << name << "' does not exist.\n";
		return;
	} // end if	

	std::deque<JezzNode> &storedSolution = it->second;
	if (storedSolution.empty()) {
		std::cout << "[WARNING] Jezz: Trying to restore an empty solution.\n";
		return;
	} // end if
	...
	for (int i = 0; i < numObstacles; i++) {
		...
		if (!jezzNode->row) {
			std::cout << "[WARNING] Jezz: Not restoring obstacle with null row.\n";
			continue;
		} // end if
		...
		bool overflow = false;
		jezz_dp_InsertNodeUsingJezzCoordinates(jezzNode, x, y, overflow);
		if (overflow) {
			std::cout << "[BUG] Jezz: Overflow when restoring a solution. This should not happen... Something is broken.\n";
		} // end if
	} // end for
...
void Jezz::jezz_restoreSolutionIncremental(const std::string name) {
	...
	// Remove and reinsert nodes one-by-one. It should be safe to remove and re-
	...
	for (int i = 0; i < numNodes; i++) {
		...
		if (jezzNode->x == jezzNodeStored->x && jezzNode->row == jezzNodeStored->row) {
			// Don't use y here because y is not changed if the node is 
			// un-legalized. On the other hand, row is set to nullptr when the
