...
private:
	...
	struct QualityRecord {
		...
		friend QualityRecord operator+(const QualityRecord &left, const QualityRecord &right) {
			QualityRecord result;
			result.abu = left.abu + right.abu;
			result.wns[Rsyn::EARLY] = left.wns[Rsyn::EARLY] + right.wns[Rsyn::EARLY];
			result.wns[Rsyn::LATE ] = left.wns[Rsyn::LATE ] + right.wns[Rsyn::LATE ];
			result.tns[Rsyn::EARLY] = left.tns[Rsyn::EARLY] + right.tns[Rsyn::EARLY];
			result.tns[Rsyn::LATE ] = left.tns[Rsyn::LATE ] + right.tns[Rsyn::LATE ];
	...
	struct MoveRecord {
		...
		// Total moves that succeed (do not count moves rolled back due to cost
		// increase).
		long success;
		
		// Total moves that failed (do not count moves rolled back due to cost
		// increase).
		long fail;
		
		// Count the number of moves that failed due to legalization issues.
		long fail_legalization;
		
		// Count the number of moves that failed due maximum displacement
		// problems. This should be zero in general as the target position is
		// clamped to meet the maximum displacement before the cell is actually
		// moved.
		long fail_displacement;
		
		// Count the number of times that a fixed cell was tried to be moved.
		// This should be zero in general.
		long fail_fixed;
		
		// Count the number of moves that were rolled back due to cost increase.
		long fail_cost;
		...
		long getFails(const bool ignoreMovesRolledBackDueToCost = false) const {
			return fail_displacement + fail_legalization + fail_fixed +
		...
		long getSuccesses(const bool ignoreMovesRolledBackDueToCost = false) const {
			return count - getFails(ignoreMovesRolledBackDueToCost);
		} // end method
		
		// Get the miss rate.
		double getMissRate(const bool ignoreMovesRolledBackDueToCost = false) const {
			return getFails(ignoreMovesRolledBackDueToCost) / double(count);
		} // end method
		...
		friend MoveRecord operator+(const MoveRecord &left, const MoveRecord &right) {
			MoveRecord result;
			result.count = left.count + right.count;
			result.success = left.success + right.success;
			result.fail = left.fail + right.fail;
			result.fail_legalization = left.fail_legalization + right.fail_legalization;
			result.fail_displacement = left.fail_displacement + right.fail_displacement;
	...
	Rsyn::Attribute<Rsyn::Instance, double> clsGains[Rsyn::NUM_TIMING_MODES];	
...
public:
	...
	bool isMaxDisplacementConstraintEnabled() { return clsEnableMaxDisplacementConstraint; }
...
public:
	...
	inline
	void resetBestQualityScore() { 
		std::cout << "Reseting best QS...\n";
 		clsBestQualityScore = 0;
	} // end method
		
	bool updateTDPSolution(const bool force = false, const double expectedImprovementPerIteration = 0.01);
...
private:
	...
	double computeQualityScore(const QualityRecord &originalQoR, const QualityRecord &qor) const;
...
public:
	...
	void updateGains();
	double getGain(Rsyn::Cell cell, const Rsyn::TimingMode mode) const { return clsGains[mode][cell]; }
	double getNormalizedGain(Rsyn::Cell cell, const Rsyn::TimingMode mode) const { return clsGains[mode][cell] / clsMaxGain[mode]; }
	
	void updatePriorities() { updateGains(); }
	double getPriority(Rsyn::Cell cell, const Rsyn::TimingMode mode) const { return getNormalizedGain(cell, mode); }
	
	double getPinImportance(Rsyn::Pin pin, const Rsyn::TimingMode mode) {
		return (2*clsTimer->getPinCentrality(pin, mode) +
				clsTimer->getPinCriticality(pin, mode))/3;
	} // end method		
...
public:
	...
	void computeBoundedPosition(
			const Rsyn::PhysicalCell physicalCell, 
			const DBU x, const DBU y,
			DBU &boundedx,
			DBU &boundedy) const;
	
	bool moveCell(Rsyn::PhysicalCell physicalCell, const DBU x, const DBU y, const LegalizationMethod legalization, const bool rollback = true);
	bool moveCell(Rsyn::Cell cell, const DBU x, const DBU y, const LegalizationMethod legalization, const bool rollback = true);
	bool moveCell(Rsyn::PhysicalCell physicalCell, const DBUxy pos, const LegalizationMethod legalization, const bool rollback = true);
	bool moveCell(Rsyn::Cell cell, const DBUxy pos, const LegalizationMethod legalization, const bool rollback = true);
	
	bool translateCell(Rsyn::PhysicalCell physicalCell, const DBU dx, const DBU dy, const LegalizationMethod legalization, const bool rollback = true);
	bool translateCell(Rsyn::Cell cell, const DBU dx, const DBU dy, const LegalizationMethod legalization, const bool rollback = true);
	bool translateCell(Rsyn::Cell cell, const DBUxy displacement, const LegalizationMethod legalization, const bool rollback = true);
	bool translateCell(Rsyn::PhysicalCell physicalCell, const DBUxy displacement, const LegalizationMethod legalization, const bool rollback = true);
	
	// Similar to move cell, but if the move fails, try again in a slight 
	// different position along with the straight line connecting the current
	// cell position and the target position.
	// Step size is should be in (0, 1] range.
	// Cell is kept in its current position if another position is not found.
	void moveCellTowards(Rsyn::PhysicalCell physicalCell, const DBUxy targetPos, const LegalizationMethod legalization, const double stepSize);
	void moveCellTowards(Rsyn::PhysicalCell physicalCell, const DBU x, const DBU y, const LegalizationMethod legalization, const double stepSize);
	void moveCellTowards(Rsyn::Cell cell, const DBUxy targetPos, const LegalizationMethod legalization, const double stepSize);
	void moveCellTowards(Rsyn::Cell cell, const DBU x, const DBU y, const LegalizationMethod legalization, const double stepSize);
		
	// Methods to move cells and automatically rollback if the move worsen
	// some cost metric. Use the cached version if you will try several moves
	...
	bool moveCellWithCostEvaluation(Rsyn::Cell cell, const DBU x, const DBU y, const LegalizationMethod legalization, const CostFunction costFunction, const Rsyn::TimingMode mode, double &oldCost, double &newCost);
	bool moveCellWithCostEvaluationCached(Rsyn::Cell cell, const DBU x, const DBU y, const LegalizationMethod legalization, const CostFunction costFunction, const Rsyn::TimingMode mode, const double oldCost, double &newCost);
	
	bool translateCellWithCostEvaluation(Rsyn::Cell cell, const DBU dx, const DBU dy, const LegalizationMethod legalization, const CostFunction costFunction, const Rsyn::TimingMode mode, double &oldCost, double &newCost);
	bool translateCellWithCostEvaluationCached(Rsyn::Cell cell, const DBU dx, const DBU dy, const LegalizationMethod legalization, const CostFunction costFunction, const Rsyn::TimingMode mode, const double oldCost, double &newCost);
...
private:
	...
	// Cost Computation
	double computeCost_Wirelength(Rsyn::Cell cell, const Rsyn::TimingMode mode);
	double computeCost_RC(Rsyn::Cell cell, const Rsyn::TimingMode mode);
	double computeCost_LocalDelay(Rsyn::Cell cell, const Rsyn::TimingMode mode);
...
public:
	...
	double computeCost(Rsyn::Cell cell,
...
public:
	...
	bool legalizeCell(Rsyn::PhysicalCell physicalCell, const LegalizationMethod legalization, const bool rollback = true);
	bool legalizeCell(Rsyn::Cell cell, const LegalizationMethod legalization, const bool rollback = true);
	...
	void diffJezzSolutions( std::ostream &out,
							const int numCells,
...
public:
	...
	bool checkMaxDisplacementViolation(const std::string &errorMsg = "", const bool hideMessageIfPass = true);
	bool checkUnlegalizedCells(const std::string &errorMsg = "", const bool hideMessageIfPass = true);	
...
public:	
	...
	bool isInverterOrBuffer(Rsyn::Instance cell) { return cell.getNumInputPins() == 1 && cell.getNumOutputPins() == 1 && !cell.isPort(); }
	bool isInverterOrBuffer(Rsyn::Pin pin) { return isInverterOrBuffer(pin.getInstance()); }	
	
	double computeManhattanDistance(const double x0, const double y0, const double x1, const double y1) const { 
	...
	double computeManhattanDistance(Rsyn::Pin pin0, Rsyn::Pin pin1) { 
		return computeManhattanDistance(clsPhysicalDesign.getPinPosition(pin0), clsPhysicalDesign.getPinPosition(pin1)); }		
	double computeManhattanDistanceFromInitialPosition(const PhysicalCell &ph) const { 
		return computeManhattanDistance(ph.getPosition(), ph.getInitialPosition());	}
	double computeManhattanDistanceFromInitialPosition(Rsyn::Cell cell) const { 
		return computeManhattanDistanceFromInitialPosition(clsPhysicalDesign.getPhysicalCell(cell));	}
	...
	double computePathManhattanLength(const std::vector<Rsyn::Timer::PathHop> &path);
...
public:
	...
	void reportTopCentralCells(const Rsyn::TimingMode mode, 
			const int maxNumCellsToReport, const bool showButDontCountFixedCells);
	void reportTopCriticalCells(const Rsyn::TimingMode mode, 
			const int maxNumCellsToReport, const bool showButDontCountFixedCells);
	void reportPathLengths(const std::string &label,
	...
	/* This method queries the top critical N end points plus the top M critical
	...
	void reportCountOfTopCriticalPathsOnCriticalEndPoints(std::ostream &out,
		int numEndPoints = 50,
		int numPaths = 5000);
	void reportSlackOnTopCriticalEndPoints(std::ostream &out,
		const int numEndPoints);

	void reportTotalSlackOnTopCriticalEndPoints(std::ostream &out,
		const int numPathsPerEndPoint);

	void reportSlackOnTopEndPoints(std::ostream &out, const int numEndPoints);

	void reportCurrLoadOnCriticalPath(std::ostream &out,
...
public:
	...
	DBU getMaxDisplacement() const {
		return clsMaxDisplacement * clsPhysicalDesign.getDatabaseUnits(Rsyn::DESIGN_DBU);
	} // end method
