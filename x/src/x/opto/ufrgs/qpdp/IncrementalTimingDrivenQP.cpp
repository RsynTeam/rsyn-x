...
void IncrementalTimingDrivenQP::copyCellsLocationFromLinearSystemToEngine() {
	...
	initialPositions.assign( mapIndexToCell.size(), DBUxy(
...
//		displacement.y = -displacement.y;
	...
	for (const std::tuple<Number, Rsyn::Cell, int> &e : orderedCells) {
		...
		if (moveTentatively) {
			infra->moveCellTowards(cell, newPos.convertToDbu(), legMode, 0.1);
		} else {
	...
	cout << 
		"\n\tAverage legalization disp = " << averageDisplacement << " rows\n";
...
void IncrementalTimingDrivenQP::copyCellsLocationFromLinearSystemToEngineOptimized() {
	...
	if (debugPathImprovement) {
		std::cout << "\"debugPathImprovement\" enabled. It will cause performance loss..\n"; 
		timer->updateTimingIncremental();
		if (averageDisplacement) {
			std::vector<Rsyn::Timer::PathHop> path;
			timer->queryTopCriticalPath(Rsyn::LATE, path);
//			Rsyn::Pin endpoint = design.findPinByName("A2_B4_C2_D17_o724686:d");
//			timer->queryTopCriticalPathsFromEndpoint(Rsyn::LATE, endpoint, 1, path );
...
void IncrementalTimingDrivenQP::addAnchor(const Rsyn::Cell c, const DBUxy pos, const double w) {
...
void IncrementalTimingDrivenQP::runQuadraticPlacement(const std::set<Rsyn::Cell> &movable, 
	...
	std::thread tx( [&]() {
		SolveByConjugateGradient( a, bx, px, 1000 );
	} );
	std::thread ty( [&]() {
		SolveByConjugateGradient( a, by, py, 1000 );
	} );
...
void IncrementalTimingDrivenQP::buildLinearSystem( const bool stress ) {
	...
	for (Rsyn::Net net : nset) {
		...
		if ( enableRC ) {
			const double driverResistance = driver ?
				libc->getCellMaxDriverResistance(driver.getInstance(), Rsyn::LATE) : 1;
		...
		for (Rsyn::Pin pin0 : net.allPins()) {
			...
			for (Rsyn::Pin pin1 : net.allPins()) {
				...
				double weight = R * double(1) / std::max(1, (net.getNumPins() - 1));

				if (stress) {
					weight *= 1 + timer->getNetCriticality(net, Rsyn::LATE);
				} // end if
				...
				if (index0 != -1 && index1 != -1) {
					...
					const DBUxy pos1 = phDesign.getPinPosition(pin1);
					...
					const DBUxy pos0 = phDesign.getPinPosition(pin0);
...
bool IncrementalTimingDrivenQP::canBeSetAsMovable(Rsyn::Cell cell, const bool considerCriticalCellsAsFixed) {
...
void IncrementalTimingDrivenQP::markAsMovableAllNonFixedCells(const bool considerCriticalCellsAsFixed) {
	movable.clear();
	for (Rsyn::Instance instance : module.allInstances()) {
		Rsyn::Cell cell = instance.asCell(); // TODO: hack, assuming that the instance is a cell
...
void IncrementalTimingDrivenQP::markAsMovableAllNonFixedNonCriticalCellsDrivenByCriticalNets() {
	movable.clear();
	for (Rsyn::Instance instance : module.allInstances()) {
		Rsyn::Cell cell = instance.asCell(); // TODO: hack, assuming that the instance is a cell
...
void IncrementalTimingDrivenQP::markAsMovableAllNonFixedCriticalCellsAndTheirSinks() {
	movable.clear();
	for (Rsyn::Instance instance : module.allInstances()) {
...
void IncrementalTimingDrivenQP::markAsMovableAllNonFixedCriticalCellsAndTheirNeighbors() {
	movable.clear();
	for (Rsyn::Instance instance : module.allInstances()) {
		Rsyn::Cell currentCell = instance.asCell(); // TODO: hack, assuming that the instance is a cell
		if (timer->getCellCriticality(currentCell, Rsyn::LATE) > 0) {
...
void IncrementalTimingDrivenQP::markAsMovableAllNonFixedCellsInTopCriticalPaths(const int numPaths) {
	movable.clear();
...
void IncrementalTimingDrivenQP::markAsMovableAllNonFixedCellsInTopCriticalPathsAndTheirNeighbors( const int numPaths ) {
	movable.clear();
...
void IncrementalTimingDrivenQP::runCriticalPathSmoothing() {
	...
	//timer->queryTopCriticalPathFromTopCriticalEndpoints(Rsyn::LATE, N, paths);
	...
	if (numPaths > 0) {
		...
		for (int i = 0; i < numPaths; i++) {
			...
			std::cout << std::setw(10) << oldSlack << " "
					<< std::setw(10) << newSlack 
					<< std::setw(10) << (100*(1 - newSlack/oldSlack)) << "%\n";
		} // end for
...
//	}
	...
	for ( Rsyn::Net net : module.allNets() ) {
		...
		if( criticality > 0 ) {
			Rsyn::Cell driver  = net.getAnyDriver().getInstance().asCell();
			double slack = timer->getCellWorstNegativeSlack( driver, Rsyn::LATE );
...
double IncrementalTimingDrivenQP::estimatePathSize(std::vector<Rsyn::Timer::PathHop>& pathHops) {
	double size = 0.0;
...
double IncrementalTimingDrivenQP::pathAvailableArea(std::vector<Rsyn::Timer::PathHop>& pathHops) {	
...
void IncrementalTimingDrivenQP::generateStraighteningForces_DEPRECATED(	const bool enableCriticality ) {	
	std::vector<std::vector<Rsyn::Timer::PathHop>> paths;
	std::vector<std::vector<Rsyn::Timer::PathHop>> paths2;
	
	const Number slackThresshold = - alpha * timer->getClockPeriod();
	timer->queryTopCriticalPaths( Rsyn::LATE, maxNumPaths, paths, slackThresshold );
	std::cout << "[Quadratic Placement] #Paths " << paths.size() << "\n"; 
	
	const int numCriticalEndPoints = timer->getNumCriticalEndpoints( Rsyn::LATE );
	timer->queryTopCriticalPathFromTopCriticalEndpoints( Rsyn::LATE, numCriticalEndPoints, paths2 );
...
//	paths.insert( paths.end(), paths2.begin(), paths2.end() );
	...
	for (int i = 0; i < paths.size(); i++) {
		...
		if( enableCriticality )
			criticality =
				criticalPathHops[0].getSlack() / timer->getWns( Rsyn::LATE );
		
		for (int i = 4; i < criticalPathHops.size() - 2; i += 2) {
			const Rsyn::Timer::PathHop& hop = criticalPathHops[i];
			
			const Rsyn::Cell currCell  = hop.getPin().getInstance().asCell();
			const Rsyn::Cell nextCell  = hop.getNextPin().getInstance().asCell();
			
			const bool connectionExists = 
				connections.find( std::make_pair( currCell, nextCell ) ) != connections.end();
			...
			if( enableRC ) {
				const double driverResistance =
					libc->getCellMaxDriverResistance( currCell, Rsyn::LATE );
				const double loadCapacitance = 
					routingEstimator->getRCTree( hop.getPin().getNet() ).getLumpedCap().getMax();
				RC = 0.1 * driverResistance; //  * loadCapacitance;
			}
					
			RC = std::max( RC, minimumResistance );
			
			const bool currIsMovable = canBeSetAsMovable( currCell, false );
			const bool nextIsMovable = canBeSetAsMovable( nextCell, false );			
			...
			if( !currIsMovable && nextIsMovable  ) {
				...
				//by[ index ] += RC * 5 * (pCell.lower().y + pPin.dy);
...
void IncrementalTimingDrivenQP::generateStraighteningForcesTopCriticalPaths() {
	...
	timer->queryTopCriticalEndpoints( Rsyn::LATE, 
									  timer->getNumCriticalEndpoints( Rsyn::LATE ),
									  topCriticalEndPoints, 
									  0.01f * timer->getWns( Rsyn::LATE ) );
	...
	for (int i = 0; i < topCriticalEndPoints.size(); i++) {		
		...
		timer->queryTopCriticalPathsFromEndpoint( Rsyn::LATE,
												  topCriticalEndPoints[i],
												  500,
												  criticalPaths );
		
		for( int j = 0; j < criticalPaths.size(); j++ ) {
			const std::vector<Rsyn::Timer::PathHop>& path = criticalPaths[j];
			...
			for( int k = 0; k < path.size() - 1; k++) {
				...
				const Rsyn::Cell currCell  = hop.getInstance().asCell();
				
				const double driverResistance =
						libc->getCellMaxDriverResistance( currCell, Rsyn::LATE );
				const double R = std::max( 0.1 * driverResistance, minimumResistance );
				
				const double weight = 5 * R * ( 1 + pathCriticality );
				
				const std::pair<Rsyn::Pin,Rsyn::Pin> key( hop.getPin(), 
														  hop.getNextPin() );
...
void IncrementalTimingDrivenQP::generateStraighteningForces( const bool stress ) {
	...
	for ( Rsyn::Net net : module.allNets() ) {
		...
		Rsyn::Cell driverCell  = net.getAnyDriver().getInstance().asCell();
		...
		if ( enableRC ) {
			...
			if( !libc ) {
				std::cout << "No libc specified! Aborting..." << std::endl;
				exit(1);
			}
		...
		for ( Rsyn::Pin pin : net.allPins(Rsyn::IN) ) {
			const Number pinCriticality = timer->getSmoothedCriticality( pin, Rsyn::LATE );
			//const Number pinCriticality = timer->getCellCentrality(pin.getCell(), Rsyn::LATE );
			const Number pinCentrality = timer->getPinCentrality( pin, Rsyn::LATE );
			
			if ( pinCriticality < alpha || 
					FloatingPoint::approximatelyEqual( pinCriticality, alpha) )
				continue;
			...
			double weight = //10 * RC; 
						...
						// RC * ( 1 + 0.2 * std::exp( std::pow( pinCentrality, 3) - 1 ) );
...
void IncrementalTimingDrivenQP::generateStraighteningForces_ELMORE() {
	...
	for ( Rsyn::Net net : module.allNets() ) {
		...
		double Rdrive = libc->getCellMaxDriverResistance( driverCell, Rsyn::LATE );
		Rdrive = std::max( Rdrive, 0.01 );
		
		DBUxy driverPos = phDesign.getPinPosition( driver );
		
		for ( Rsyn::Pin pin : net.allPins(Rsyn::IN) ) {
			const Number pinCriticality = timer->getPinCriticality( pin, Rsyn::LATE );
			//const Number pinCriticality = timer->getCellCentrality(pin.getCell(), Rsyn::LATE );
			const Number pinCentrality = timer->getPinCentrality( pin, Rsyn::LATE );
			...
			weight *= 1e11 * Rdrive * 0.1; // * ( 1 + pinCriticality );
...
void IncrementalTimingDrivenQP::runQuadraticPlacementStraighteningMultiplePaths(const int N) {
	std::cout << "Not implemented yet..." << std::endl;
} // end method

// -----------------------------------------------------------------------------
 
void IncrementalTimingDrivenQP::runQuadraticPlacementStraighteningSinglePathAndSideCells(const int N) {
	std::cout << "Not implemented yet..." << std::endl;
} // end method
...
void IncrementalTimingDrivenQP::generateNetWeights() {
	for (Rsyn::Net net : module.allNets() ) {
		...
		double driverCriticallity = timer->getPinCriticality( driver, Rsyn::LATE );
		
		if( driverCriticallity < alpha )
			continue;
...
//		double C = 1;
//		C = std::max( timer->getNetPinLoad( net )[Rsyn::RISE],
//				      timer->getNetPinLoad( net )[Rsyn::FALL] );
//		C = std::max( C, 0.01);
//		
		for (Rsyn::Pin pin0 : net.allPins()) {
			Rsyn::Cell cell0  = pin0.getInstance().asCell();
			...
			for (Rsyn::Pin pin1 : net.allPins()) {
				...
				double weight = R * 5 * ( 1 + driverCriticallity ) /
...
//		double currentSlack = timer->getPinWorstNegativeSlack( driver, Rsyn::LATE );
//		
...
bool IncrementalTimingDrivenQP::run(Rsyn::Engine engine, const Rsyn::Json& params) {
	...
	const std::string legModeName = params.value("legMode", "NEAREST_WHITESPACE");
