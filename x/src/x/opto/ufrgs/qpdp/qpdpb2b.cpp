...
#include "x/opto/ufrgs/qpdp/qpdpb2b.h"
...
void QpdpB2B::copyCellsLocationFromLinearSystemToEngine( 
	...
	for (const std::tuple<Number, Rsyn::Cell, int> &e : orderedCells) {
		...
		if (moveTentatively) {
			infra->moveCellTowards(cell, newPos.convertToDbu(), legMode, 0.2);
		} else {
			if( !infra->moveCell(cell, newPos.convertToDbu(), legMode, true) ) {
				if( !infra->moveCell( cell, oldPos, legMode, true ) ){
					if( !infra->moveCell( cell, oldPos, LegalizationMethod::LEG_MIN_LINEAR_DISPLACEMENT, true ) ){
						std::cout << "[WARNING]: Legalization error at " << __FILE__ << 
							": " << __LINE__ << "\n";
					} // end if
				} // end if
			} // end if
...
QpdpB2B::getDriverResistance(Rsyn::Net net) {
	...
	if (driver) {
		rDriver = libc->getCellMaxDriverResistance(driver.getInstance(), Rsyn::LATE);
...
void 
QpdpB2B::createNodeToIndexMapping(		
		Rsyn::Net net,
		const Rsyn::RCTree &tree,
		const Rsyn::Attribute<Rsyn::Instance, int> &mapCellToIndex,
		std::vector<int> &mapNodeToIndex,
		int &nextNodeIndex	
) {
	...
	for (int i = 0; i < numNodes; i++) {
		...
		if (pin) {
			mapNodeToIndex[i] = 
					mapCellToIndex[pin.getInstance()];
		} else {
			mapNodeToIndex[i] = 
					nextNodeIndex++;
		} // end else
...
void QpdpB2B::neutralizeSystemUsingAnchors(
	...
	std::vector<double> nb(position.size(), 0.0); // neutrilization b (rhs vector)
...
void QpdpB2B::neutralizeSystemUsingForces(
		const SquareCompressedRowMatrix &A,
		const std::vector<double> &b,
		const std::vector<double> &position,
		std::vector<double> &digIncrement,
	...
	for (Rsyn::Net net : nset) {
		...
		for (Rsyn::Pin pin0 : net.allPins()) {
			...
			for (Rsyn::Pin pin1 : net.allPins()) {
				...
				const double weight = weightening(pin0, pin1, edgeWeight);
				...
				if (index0 != -1 && index1 != -1) {
					...
					const double2 pos1 = double2(phDesign.getRelaxedPinPosition(pin1));
					...
					const double2 pos0 = double2(phDesign.getRelaxedPinPosition(pin0));
...
void QpdpB2B::buildLinearSystem_Add(
		const int index0, // driver
	...
	const double force1 = useForces? netWeight * (Rw * Cdown * (pos1 > pos0? -1 : +1)) : 0;  
...
void QpdpB2B::buildLinearSystemUsingBound2BoundModel_AddConnections(
	...
	// Weight scaling.
	const double rDriver = net.getAnyDriver() && enableRC? 
		getDriverResistance(net) : 1.0;	
	...
	buildLinearSystemUsingBound2BoundModel_Add(numPins, lindex, lpos, uindex, upos, 
			epsilon, weight, dscp, b, hpwl);
	
	// Add boundary-to-inner connections.
	for (int i = 1; i < numPins-1; i++) {
		const int iindex = mapCellToIndex[std::get<1>(pins[i]).getInstance()];
		const double ipos = std::get<0>(pins[i]);
		
		buildLinearSystemUsingBound2BoundModel_Add(numPins, lindex, lpos, iindex, ipos, epsilon, weight, dscp, b, hpwl);
		buildLinearSystemUsingBound2BoundModel_Add(numPins, uindex, upos, iindex, ipos, epsilon, weight, dscp, b, hpwl);
	} // end for
} // end method
...
void QpdpB2B::buildLinearSystemUsingBound2BoundModel(
	...
	for (Rsyn::Net net : nset) {
		const double netWeight = std::max(verySmallNetWeight, getNetWeight(net));
				
		relaxedPinPos->getNetPinsSortedByRelaxedPinPositions(net, X, xPins);
		relaxedPinPos->getNetPinsSortedByRelaxedPinPositions(net, Y, yPins);
		...
		buildLinearSystemUsingBound2BoundModel_AddConnections(
				mapCellToIndex, stress, epsilon, netWeight, net, xPins, dscpx, bx, netB2BHPWLx);
		buildLinearSystemUsingBound2BoundModel_AddConnections(
				mapCellToIndex, stress, epsilon, netWeight, net, yPins, dscpy, by, netB2BHPWLy);
		
		if (true) {
			const Rsyn::RCTree &tree = routingEstimator->getRCTree(net);
			
			const int prevNodeIndex = nextNodeIndex;
			createNodeToIndexMapping(net, tree, mapCellToIndex, mapping, nextNodeIndex);
			...
			const int numNodes = tree.getNumNodes();
			for (int i = 1; i < numNodes; i++) { // starts at 1 to skip root node
				const Rsyn::RCTree::Node &node = tree.getNode(i);
				const Rsyn::RCTreeNodeTag &tag0 = tree.getNodeTag(node.propParent);
				...
				const double d = std::abs(x0 - x1) + std::abs(y0 - y1);
				const double Csegment = Cw*d/2;

				const double Cdown = node.getDownstreamCap().getMax() - Csegment;
				if (Cdown < 0) {
					std::cout << "[BUG] Negative donwnstream cap.\n";
				} // end if

				buildLinearSystem_Add(index0, index1, x0, x1, Rw, Cw, Cdown, netWeight, epsilon, dscpx, bx);
				buildLinearSystem_Add(index0, index1, y0, y1, Rw, Cw, Cdown, netWeight, epsilon, dscpy, by);
				
				// Assign steiner points positions.
				if (index1 >= prevNodeIndex && index1 < nextNodeIndex) {
					px[index1] = x1;
					py[index1] = y1;
				} // end if
			} // end for
	...
	buildLinearSystemUsingBound2BoundModel_CheckConsistency("Ax", mapIndexToCell, Ax);
	buildLinearSystemUsingBound2BoundModel_CheckConsistency("Ay", mapIndexToCell, Ay);
...
void QpdpB2B::buildLinearSystemUsingBound2BoundModel_CheckConsistency(
	...
	std::cout << "  - A null diagonal creates a singular system (no unique solution) and should be considered as a bug.\n";
	std::cout << "  - An empty row (no off-diagonal elements) with no null diagonal indicates a node that is connected only to fixed nodes.\n";
...
bool QpdpB2B::canBeSetAsMovable(Rsyn::Cell cell, const bool considerCriticalCellsAsFixed) {
	...
	return !cell.isPort()
			...
			&& !cell.isLCB()
			&& !cell.isMacroBlock()
			&& !(considerCriticalCellsAsFixed && timer->getCellCriticality( cell, Rsyn::LATE ) > 0);		
} // end method

// -----------------------------------------------------------------------------

void QpdpB2B::markAsMovableAllNonFixedCells(const bool considerCriticalCellsAsFixed, std::set<Rsyn::Cell> &movable) {
	movable.clear();
	for (Rsyn::Instance instance : module.allInstances()) {
		Rsyn::Cell cell = instance.asCell(); // TODO: hack, assuming that the instance is a cell
...
void QpdpB2B::markAsMovableAllNonFixedCriticalCells(std::set<Rsyn::Cell> &movable) {
	movable.clear();
	for (Rsyn::Instance instance : module.allInstances()) {
		Rsyn::Cell cell = instance.asCell(); // TODO: hack, assuming that the instance is a cell
		if (canBeSetAsMovable(cell, false) && timer->getCellCriticality(cell, Rsyn::LATE) > 0) {
			movable.insert(cell);
		} // end if
	} // end for
} // end method
...
void QpdpB2B::markAsMovableAllNonFixedNonCriticalCellsDrivenByCriticalNets(std::set<Rsyn::Cell> &movable) {
	movable.clear();
	for (Rsyn::Instance instance : module.allInstances()) {
		Rsyn::Cell cell = instance.asCell(); // TODO: hack, assuming that the instance is a cell
		if (canBeSetAsMovable(cell, false) && 
				FloatingPoint::approximatelyZero(timer->getCellCriticality(cell, Rsyn::LATE))) {
			
			// Check if its driven by a critical net.
			bool drivenByCriticalNet = false;
			for (Rsyn::Pin pin : cell.allPins(Rsyn::IN)) {
				Rsyn::Net net = pin.getNet();
				if (net && timer->getNetCriticality(net, Rsyn::LATE) > 0) {
					drivenByCriticalNet = true;
					break;
				} // end if
			} // end for

			if (drivenByCriticalNet)
				movable.insert(cell);
		} // end if
	} // end for
} // end method
...
void QpdpB2B::markAsMovableAllNonFixedCellsConnectedToCriticalNets(std::set<Rsyn::Cell> &movable) {
	movable.clear();
	for (Rsyn::Instance instance : module.allInstances()) {
		Rsyn::Cell cell = instance.asCell(); // TODO: hack, assuming that the instance is a cell
		if (!canBeSetAsMovable(cell, false))
		...
		if (isConnectedToAtLeastOneCriticalNet)
			movable.insert(cell);
	} // end for	
} // end method
...
void QpdpB2B::markAsMovableAllNonFixedCriticalCellsAndTheirSinks(std::set<Rsyn::Cell> &movable) {
	movable.clear();
	for (Rsyn::Instance instance : module.allInstances()) {
		Rsyn::Cell currentCell = instance.asCell(); // TODO: hack, assuming that the instance is a cell
		if (timer->getCellCriticality(currentCell, Rsyn::LATE) > 0) {
			for (Rsyn::Pin driver : currentCell.allPins(Rsyn::OUT)) {
...
void QpdpB2B::markAsMovableAllNonFixedCriticalCellsAndTheirNeighbors(std::set<Rsyn::Cell> &movable) {
	movable.clear();
	for (Rsyn::Instance instance : module.allInstances()) {
		Rsyn::Cell currentCell = instance.asCell(); // TODO: hack, assuming that the instance is a cell
		if (timer->getCellCriticality(currentCell, Rsyn::LATE) > 0) {
			for (Rsyn::Pin cellPin : currentCell.allPins()) {
				...
				if (net) {
					for (Rsyn::Pin netPin : net.allPins()) {
						Rsyn::Cell cell  = netPin.getInstance().asCell();
						if (canBeSetAsMovable(cell, false)) {
						movable.insert(cell);
						} // end if
					} // end for
...
void QpdpB2B::markAsMovableAllNonFixedCellsInTopCriticalPaths(const int numPaths, std::set<Rsyn::Cell> &movable) {
	movable.clear();
...
void QpdpB2B::markAsMovableAllNonFixedCellsInKthTopCriticalPath(const int k, std::set<Rsyn::Cell> &movable) {
	movable.clear();
...
void QpdpB2B::markAsMovableAllNonFixedCellsInTopCriticalPathsFromTopCriticalEndpoints(const int numEndpoints, const int numPathsPerEndpoings, std::set<Rsyn::Cell> &movable) {
	movable.clear();
	
	std::vector<std::vector<Rsyn::Timer::PathHop>> paths;
	timer->queryTopCriticalPathsFromTopCriticalEndpointsConsolidated(Rsyn::LATE, numEndpoints, numPathsPerEndpoings, paths);
...
void QpdpB2B::markAsMovableAllNonFixedCellsInTopCriticalPathsAndTheirNeighbors(const int numPaths, std::set<Rsyn::Cell> &movable) {
	movable.clear();
...
void QpdpB2B::runPathStraightening() {
	...
	timer->getPathDelayDueToCellsAndWires(path, delayDueToCell, delayDueToWire);
	std::cout << "-------------------------------------------------------------\n";
	std::cout << "path delay (before) : " 
			<< delayDueToCell << " " 
			<< delayDueToWire << " " 
			<< (delayDueToCell + delayDueToWire) << " "
	...
	//markAsMovableAllNonFixedCellsInTopCriticalPathsFromTopCriticalEndpoints(10, 1, movable);
	//markAsMovableAllNonFixedCellsInTopCriticalPathsAndTheirNeighbors(10, movable);
	...
	if (useNeutralization) {
		if (useAnchors) {
			neutralizeSystemUsingAnchors(Ax, bx, px, anchorWeight, incDx, incBx);
			neutralizeSystemUsingAnchors(Ay, by, py, anchorWeight, incDy, incBy);
		} else {
	...
	std::thread tx([&](){ ::SolveByConjugateGradientWithDiagonalPreConditioner(Ax, bx, px, 5000); });
	std::thread ty([&](){ ::SolveByConjugateGradientWithDiagonalPreConditioner(Ay, by, py, 5000); });
...
void QpdpB2B::runPathStraighteningUsingCliqueModel() {
	...
	// Weightening
	Weightening weightening = [&](Rsyn::Pin pin0, Rsyn::Pin pin1, const double edgeWeight) {
		const double driverResistance = getDriverResistance(pin0.getNet());
		const double w0 = infra->getPinImportance(pin0, Rsyn::LATE);
		const double w1 = infra->getPinImportance(pin1, Rsyn::LATE);
		return driverResistance * std::max(verySmallNetWeight, std::max(w0, w1));
	};
	
	// Define movable cells.
	//markAsMovableAllNonFixedCellsInTopCriticalPaths(1, movable);
	//markAsMovableAllNonFixedCellsInTopCriticalPathsAndTheirNeighbors(1, movable);
	//markAsMovableAllNonFixedCriticalCells(movable);
	//markAsMovableAllNonFixedCells(false, movable);
	markAsMovableAllNonFixedCellsConnectedToCriticalNets(movable);
	...
	if (useAnchors) {
		neutralizeSystemUsingAnchors(A, bx, px, anchorWeight, incDx, incBx);
		neutralizeSystemUsingAnchors(A, by, py, anchorWeight, incDy, incBy);
	} else {
	...
	// Solve the optimization linear system.
	std::thread tx([&](){ ::SolveByConjugateGradientWithDiagonalPreConditioner(A, bx, px, 1000); });
	std::thread ty([&](){ ::SolveByConjugateGradientWithDiagonalPreConditioner(A, by, py, 1000); });
