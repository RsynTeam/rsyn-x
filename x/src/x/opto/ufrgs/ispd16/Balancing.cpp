...
bool Balancing::run(Rsyn::Engine engine, const Rsyn::Json &params) {
	...
	if (params["type"] == "cell-steiner") {
		...
		std::cout << "Warning: Invalid optimization type '" << params["type"] << "' "
...
void Balancing::runBufferFix() { 
	...
	for (Rsyn::Instance instance : module.allInstances()) {
		...
		if (!inet || inet.getNumPins() != 2)
			continue;

		if (!onet || onet.getNumPins() != 2)
			continue;

		const Number criticality = timer->getCellCriticality(cell, Rsyn::LATE);
	...
	for (int i = numBuffers - 1; i >= 0; i--) {
		...
		if (!doBufferBalancing(buffer, legalizationMethod, costFunction)) {
			failed++;
		} // end if
		movedBuffers++;
	} // end for
...
void Balancing::runCellFix() {
	...
	for (Rsyn::Instance instance : module.allInstances()) {
		Rsyn::Cell cell = instance.asCell(); // TODO: hack, assuming that the instance is a cell
		const Number criticalityLate = timer->getCellCriticality(cell, Rsyn::LATE);
		if (!FloatingPoint::approximatelyZero(criticalityLate)) {
			...
			if (2*criticalityEarly < criticalityLate) {
				if (!cell.isFixed() && !instance.isLCB() && !instance.isSequential())
					cells.push_back(std::make_tuple(infra->getPriority(cell, Rsyn::LATE), cell));
			} // end if
		} // end if
	} // end for
...
void Balancing::runCellFixSteiner() {
	...
	for (int i = numCells - 1; i >= 0; i--) {
		...
		if (!doCellBalancingSteiner(cell, legalizationMethod, costFunction)) {
			failed++;
		} // end if	
...
bool Balancing::doBufferBalancing(Rsyn::Cell buffer, 
	...
	const Rsyn::TimingTransition oedge = iedge == Rsyn::RISE? Rsyn::FALL : Rsyn::RISE; // [TODO] Make sense aware.

	Rsyn::Arc arcDriver = timer->getCriticalTimingArcToPin(driver, Rsyn::LATE, iedge);
	...
	const double R0 = libc->getDriverResistance(arcDriver, Rsyn::LATE, iedge);
	const double R1 = libc->getDriverResistance(arcBuffer, Rsyn::LATE, oedge);
	...
	const double d1 = std::min(d - a, std::max(0.0, 
...
//
//	std::cout << "\n";
//	std::cout << "Rw*d1=" << (Rw*computeManhattanDistance(getPinPosition(driver), getPinPosition(ipin))) << "\n";
//	std::cout << "Cw*d1=" << (Cw*computeManhattanDistance(getPinPosition(driver), getPinPosition(ipin))) << "\n";			
//	std::cout << "load driver net = " << clsTimer->getNetLoad(inet) << "\n";
//	std::cout << "load sink net   = " << clsTimer->getNetLoad(onet) << "\n";
//	std::cout << "driver: logical effort delay = " << clsTimer->getLogicalEffortDelay(*arcDriver, Rsyn::LATE, iedge, clsTimer->getNetLoad(inet)[iedge]) << "\n";
//	std::cout << "buffer: logical effort delay = " << clsTimer->getLogicalEffortDelay(*arcBuffer, Rsyn::LATE, oedge, clsTimer->getNetLoad(onet)[oedge]) << "\n";
//	std::cout << "\n";
//	std::cout << "driver: "; clsTimer->printArcDebug(std::cout, *arcDriver);
//	std::cout << "buffer: "; clsTimer->printArcDebug(std::cout, *arcBuffer);
//	std::cout << "\n";
//	std::cout << "start : "; clsTimer->printPinTimingDebug(std::cout, clsTimer->getFromPinOfTimingArc(*arcDriver), false, true);
//	std::cout << "driver: "; clsTimer->printPinTimingDebug(std::cout, driver, false, true);
//	std::cout << "buffer: "; clsTimer->printPinTimingDebug(std::cout, ipin, false, true);
//	std::cout << "buffer: "; clsTimer->printPinTimingDebug(std::cout, opin, false, true);
//	std::cout << "sink  : "; clsTimer->printPinTimingDebug(std::cout, sink, false, true);
//	// -------------------------------------------------------------------------

	double oldCost;
	double newCost;	
	const bool success = infra->moveCellWithCostEvaluation(buffer, 
				(DBU) px, (DBU) py, legalizationMethod, costFunction, Rsyn::LATE, oldCost, newCost);
...
bool Balancing::doCellBalancing(Rsyn::Cell cell, 
	...
	//const bool debug = false; //cell.getName() == "o618145"; unused variable
	...
	for (Rsyn::Pin opin : cell.allPins(Rsyn::OUT)) {
		...
		for (Rsyn::Pin ipin : cell.allPins(Rsyn::IN)) {
			...
			for (Rsyn::Pin sink : onet.allPins(Rsyn::SINK)) {
				...
				const double w0 = infra->getPinImportance(driver, Rsyn::LATE);
				const double w1 = infra->getPinImportance(sink, Rsyn::LATE);			
				...
				const double actualD = (Cw*w1*R1-Cw*w0*R0+Rw*w1*C2-Rw*w0*C1+(Cw*d-a*Cw)*Rw*w1)/
					(Cw*Rw*w1+Cw*Rw*w0);
				...
				//const double weight = clsTimer->getPinCriticality(driver, Rsyn::LATE) *
				//	clsTimer->getPinCriticality(sink, Rsyn::LATE);
	...
	if (numPos > 0 && totalWeight > 0) {
		...
		double oldCost;
		double newCost;
		success = infra->moveCellWithCostEvaluation(cell, 
				(DBU) avgPx, (DBU) avgPy, legalizationMethod, costFunction, Rsyn::LATE, oldCost, newCost);
	} // end if
...
bool Balancing::doCellBalancingSteiner(Rsyn::Cell cell,
											const LegalizationMethod legalizationMethod, 
											const CostFunction costFunction) {
	...
	for (Rsyn::Pin opin : cell.allPins(Rsyn::OUT)) {
		...
		for (Rsyn::Pin ipin : cell.allPins(Rsyn::IN)) {
			...
			const Rsyn::RCTree &itree = routingEstimator->getRCTree(inet);
			const Rsyn::RCTree &otree = routingEstimator->getRCTree(onet);
						
			const int iNodeIndex = routingEstimator->getRCTreeConnectingNodeIndex(itree, ipin);
			const int oNodeIndex = routingEstimator->getRCTreeConnectingNodeIndex(otree, opin);
				
			if (iNodeIndex < 0) {
				std::cout << "\n[BUG] Invalid value for iNodeIndex at pin ";
				std::cout << ipin.getInstanceName() << ":" << ipin.getName() << "\n";
				std::cout << " iNodeIndex = " << iNodeIndex << "\n";
				std::cout << " Net " << ipin.getNet().getName();
				std::cout << " has " << ipin.getNet().getNumPins();
				std::cout << " pins.\n";
				return false;
			} // end if;
						
			if (oNodeIndex < 0) {
				std::cout << "\n[BUG] Invalid value for oNodeIndex at pin ";
				std::cout << opin.getInstanceName() << ":" << opin.getName() << "\n";
				std::cout << " oNodeIndex = " << oNodeIndex << "\n";
				std::cout << " Net " << opin.getNet().getName();
				std::cout << " has " << opin.getNet().getNumPins();
				std::cout << " pins.\n";
				return false;
			} // end if;
						
			// Skip nodes added just for sake of large interconnection slicing.
			const int iindex = itree.getUpstreamBranchingNodeIndex(
					itree.getUpstreamSteinerPoint(iNodeIndex));
			const int oindex = otree.getDownstreamBranchingNodeIndex(
			...
			const Rsyn::RCTreeNodeTag &itag = itree.getNodeTag(iindex);
			const Rsyn::RCTreeNodeTag &otag = otree.getNodeTag(oindex);
						
			const double R0 = libc->getDriverResistance(arcDriver, Rsyn::LATE, iedge);
			const double R1 = libc->getDriverResistance(arcBuffer, Rsyn::LATE, oedge);

			const double Cwire = routingEstimator->getWireCapToParentNode(otree, oindex);
			
			const double Rup = inode.getUpstreamRes() + R0;
			const double Cdown = onode.getDownstreamCap()[oedge] - Cwire;
			if (Cdown < 0) {
				std::cout << "[BUG] Negative downstream capacitance. This may be due to a bug or because the downstream cap is not updated.\n";
				std::cout << "  Cdown (w/  wire) = " << onode.getDownstreamCap()[oedge] << "\n";
			...
			for (Rsyn::Pin sink : onet.allPins(Rsyn::SINK)) {
				...
				if (FloatingPoint::approximatelyZero(d)) {
					// This may happen but I'm not sure when... Well I guess the
					// input and output tree have a steiner point at same position
					// and the cell connects to these exactly steiner points.
					continue;						
				} // end if
				
				const double w0 = infra->getPinImportance(driver, Rsyn::LATE);
				const double w1 = infra->getPinImportance(opin, Rsyn::LATE);			

				if (FloatingPoint::approximatelyZero(w0 + w1))
					continue;				
				
				const double actualD = (Cw*w1*R1-Rw*w0*C1+(Cw*d-a*Cw+Cdown)*Rw*w1-Cw*Rup*w0)/
						(Cw*Rw*w1+Cw*Rw*w0);
				...
				if (debug) {
					std::cout << "\n";
					std::cout << "moving buffer " << cell.getName() << " ("
					...
					std::cout << "a=" << a << "\n";
					std::cout << "d=" << d << "\n";
					std::cout << "d1=" << d0 << " (" << (scaling) << ")" << "\n";
					std::cout << "d1 (actual)=" << actualD << " (" << (scaling) << ")" << "\n";
					
					std::cout << "\n";
					std::cout << "Rw*d1=" << (Rw*infra->computeManhattanDistance(phDesign.getPinPosition(driver), phDesign.getPinPosition(ipin))) << "\n";
					std::cout << "Cw*d1=" << (Cw*infra->computeManhattanDistance(phDesign.getPinPosition(driver), phDesign.getPinPosition(ipin))) << "\n";			
					std::cout << "load driver net = " << timer->getNetLoad(inet) << "\n";
					std::cout << "load sink net   = " << timer->getNetLoad(onet) << "\n";
					std::cout << "driver: logical effort delay = " << libc->getArcLogicalEffortDelay(arcDriver, Rsyn::LATE, iedge, timer->getNetLoad(inet)[iedge]) << "\n";
					std::cout << "buffer: logical effort delay = " << libc->getArcLogicalEffortDelay(arcBuffer, Rsyn::LATE, oedge, timer->getNetLoad(onet)[oedge]) << "\n";
					std::cout << "\n";
					std::cout << "driver: "; timer->printArcDebug(std::cout, arcDriver);
					std::cout << "buffer: "; timer->printArcDebug(std::cout, arcBuffer);
					std::cout << "\n";
					std::cout << "start : "; timer->printPinDebug(std::cout, arcDriver.getFromPin(), false, true);
					std::cout << "driver: "; timer->printPinDebug(std::cout, driver, false, true);
					std::cout << "buffer: "; timer->printPinDebug(std::cout, ipin, false, true);
					std::cout << "buffer: "; timer->printPinDebug(std::cout, opin, false, true);
					std::cout << "sink  : "; timer->printPinDebug(std::cout, sink, false, true);					
				} // end if
			} // end for (sink)
		} // end for (input pin)
	} // end for (output pin)
