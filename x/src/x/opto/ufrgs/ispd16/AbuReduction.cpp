...
bool AbuReduction::run(Rsyn::Engine engine, const Rsyn::Json &params) {
	this->engine = engine;
	...
	if (infra->getAbu() > 0) {
		...
		} while (infra->getAbu() > 0 && 
				(infra->getAbu()/previousABU <= 0.9) &&
				(routingEstimator->getTotalWirelength()/previousStWL <= 1.01));
	} // end if	
...
void AbuReduction::computeABUVioaltionCells(const double alpha ) {
	...
	for (Rsyn::Instance instance : module.allInstances()) {
		...
		if(cell.isFixed() || cell.isPort() || cell.isLCB())
			continue;
		...
		for(int i = lowerRow; i <= upperRow; i++) {
			...
			for(int j = lowerCol; j <= upperCol; j++) {
				...
				if(density > infra->getTargetUtilization()){
					clsABUViolationCells.push_back(std::make_tuple(cell, std::make_tuple(slack, binId)));
					break;
				} // end if 
			} // end for 
			if(density > infra->getTargetUtilization())
...
void AbuReduction::moveABUViolationCells() {
	...
	for (std::tuple<Rsyn::Cell, std::tuple < Number, int>> tp : clsABUViolationCells) {
		...
		if(bin_util != abu.getBinUtil(binId))
			std::cout<<"binUtil: "<<bin_util<<" obj: "<<abu.getBinUtil(binId)<<"\n";
		if (bin_util < infra->getTargetUtilization())
			continue;
		Rsyn::PhysicalCell phCell = phDesign.getPhysicalCell(cell);
		...
		bool moved = infra->moveCell(phCell, pos, LEG_NEAREST_WHITESPACE);

		if (moved) {
			if (infra->violateMaxDisplacement(phCell)) {
				infra->moveCell(phCell, initLowerPos, LEG_NEAREST_WHITESPACE);
			} else {
				movedCells.push_back(std::make_tuple(cell, initLowerPos));

				abu.removeAbuUtilization(initLowerPos, initUpperPos, isFixed);
				abu.addAbuUtilization(phCell.getCoordinate(LOWER), phCell.getCoordinate(UPPER), isFixed);
			} // end if-else
		} // end if 
	} // end for
	...
	for (std::tuple<Rsyn::Cell, DBUxy> tp : movedCells) {
		...
		if (slack < 0) {
			...
			bool move = infra->moveCell(cell, pos, LEG_NEAREST_WHITESPACE);
		} // end if 	
	} // end for 
} // end method 
