...
void ClusteredMove::runClusteredMovement(const int N) {
	...
	for( Rsyn::Instance instance : module.allInstances() ) {
		for( Rsyn::Pin pin : instance.allPins() ) {
			...
			double lSlack = timer->getPinWorstNegativeSlack( pin, Rsyn::LATE );
	...
	for( auto cell: module.allInstances() )
		moved[cell] = false;
	...
	for( int i = 0; i < num; i++ ) {
		...
		if( moved[ criticalPins[i].second.getInstance() ] )
			continue;
...
void ClusteredMove::clusterNeighborCriticalNets( Rsyn::Pin criticalPin, const bool dontMoveRegisters ) {
...
//			double currSlack = - timer->getPinWorstNegativeSlack( pin, Rsyn::LATE );
...
//			
			...
			for( Rsyn::Pin cPin : cell.allPins()  ) {
				...
				if( !(timer->getPinWorstNegativeSlack( cPin, Rsyn::LATE ) < 0) )
					continue;
	...
	for( auto cell = cells.begin(); cell != cells.end(); cell++ ) {
		for( auto pin : (*cell).allPins() ) {
			...
			for( auto neightbor : net.allPins() )  {
				const double slack = timer->getPinWorstNegativeSlack( neightbor, Rsyn::LATE );
				if( !(slack < 0) )
					continue;
				
				auto neightborCell = neightbor.getInstance();
				if( cells.find( neightborCell.asCell() ) != cells.end() )
					continue;
				
				const double weight = enableWeightening?
...
//	std::cout << "Target: " << targetPosition << std::endl;
	...
	for( std::set< Rsyn::Cell >::iterator it = cells.begin(); it != cells.end(); it++ ) {
		if (dontMoveRegisters && (*it).isSequential())
			continue;
		...
		infra->moveCellWithCostEvaluation(cell, newPos.x, newPos.y, LegalizationMethod::LEG_NEAREST_WHITESPACE,
...
Number ClusteredMove::getWorstSlack(const Rsyn::Cell cell, const Rsyn::TimingMode mode) {
	Number worstSlack = 0.0;
	
	for( auto pin : cell.allPins() )
		worstSlack = std::min( worstSlack, timer->getPinWorstSlack( pin, mode ) );
