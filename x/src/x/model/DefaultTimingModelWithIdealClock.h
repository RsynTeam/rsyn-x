...
public:	
	...
	void calculateInputDriverTiming(
		...
		clsDefaultTimingModel->calculateInputDriverTiming(port, mode, load, inputDelay, inputSlew);
		
	} // end method
	
	void calculateLibraryArcTiming(
			...
			const Number load, 
			Number& delay, 
			Number& slew) override {
			
		// HARD-CODED...
		if (libraryArc.getLibraryCell().getName().find( "CLK" ) != std::string::npos) {
			delay = 0;
			slew = 0;
			return;
		} // end if
	...
	void calculateLoadCapacitance(
		...
		clsDefaultTimingModel->calculateLoadCapacitance(pin, mode, load);
	} // end method
	
	void calculateNetArcTiming(
	...
	Rsyn::TimingSense getLibraryArcSense(const Rsyn::LibraryArc libraryArc) override {
		return clsDefaultTimingModel->getLibraryArcSense(libraryArc);
	} // end method
	
	Number getLibraryPinInputCapacitance(Rsyn::LibraryPin lpin) const override {
		return clsDefaultTimingModel->getLibraryPinInputCapacitance(lpin);
	} // end method
