...
void NetlistExtractor::parseSubcktInterface(const std::string input, Subckt& subckt) {
	 std::vector<std::string> parameters;
     boost::algorithm::split(parameters, input, boost::algorithm::is_any_of(" "));
	 
	if (parameters.size() < 3) {
		 std::cout << "[ERROR] Subckt without any name or interface...\n";
		 std::exit(1);
	} // end if
...
void NetlistExtractor::parseTransistor(const std::string input, Transistor& transistor) {
...
void NetlistExtractor::extract() {
	...
	for (Rsyn::Instance instance : clsModule.allInstances()) {
		...
		if (clsEntries.find(libCell) == clsEntries.end()) {
			std::cout << "[ERROR] Spice of cell " << libCell << " not found!\n";
			continue;
//			std::exit(1);
		} // end if
...
void NetlistExtractor::extractCellSubckt(const Rsyn::Cell cell, const Subckt& subckt) {
	...
	for (int i = 0; i < subckt.netlist.size(); i++) {
		...
		newInstance.source = getNodeName(cell, transistor.source, subckt);
		...
		if (physicalCell.getOrientation() == Rsyn::ORIENTATION_S || 
			...
			newInstance.y = physicalCell.getY() + physicalCell.getHeight() - 
		...
		if (physicalCell.getOrientation() == Rsyn::ORIENTATION_FN || 
			...
			newInstance.x = physicalCell.getX() + physicalCell.getWidth() - 
				transistor.x - transistor.l;
			newInstance.f = true;	
		} else {
...
const std::string NetlistExtractor::getNodeName(
